...
def set_wirksamkeit(
    ...
    try:
        if status is None:
            con.execute(
                """UPDATE dsgvo_tom_katalog
                   SET wirksamkeit_datum=?, wirksamkeit_ergebnis=?, updated_at=aics_now()
                   WHERE projekt_name=? AND massnahme_key=?""",
            ...
            con.execute(
                """UPDATE dsgvo_tom_katalog
                   SET wirksamkeit_datum=?, wirksamkeit_ergebnis=?, status=?, updated_at=aics_now()
                   WHERE projekt_name=? AND massnahme_key=?""",
        ...
        for entry in SEED_KATALOG:
            ...
            if key in existing:
                if force:
                    con.execute(
                        """UPDATE dsgvo_tom_katalog
                           SET ziel=?, titel=?, beschreibung=?, updated_at=aics_now()
                           WHERE projekt_name=? AND massnahme_key=?""",
