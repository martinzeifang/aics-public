...
def ensure_db(db_path: Path) -> None:
    ...
    try:
        ...
        con.execute(
            "ALTER TABLE dsgvo_dpia ADD COLUMN IF NOT EXISTS rb_projekt_id TEXT NOT NULL DEFAULT ''"
        )
        # #1105/#1106 DS5/DS6: DSFA mehrstufig — Schwellwert, Stage, Art.-36-Flag,
        # Freigabe für ältere DBs nachrüsten (idempotent).
        for _col, _ddl in (
            ('schwellwert_json', "ALTER TABLE dsgvo_dpia ADD COLUMN IF NOT EXISTS schwellwert_json TEXT NOT NULL DEFAULT '{}'"),
            ('stage', "ALTER TABLE dsgvo_dpia ADD COLUMN IF NOT EXISTS stage TEXT NOT NULL DEFAULT 'schwellwert'"),
            ('art36_required', "ALTER TABLE dsgvo_dpia ADD COLUMN IF NOT EXISTS art36_required INTEGER NOT NULL DEFAULT 0"),
            ('freigabe_durch', "ALTER TABLE dsgvo_dpia ADD COLUMN IF NOT EXISTS freigabe_durch TEXT NOT NULL DEFAULT ''"),
            ('freigabe_datum', "ALTER TABLE dsgvo_dpia ADD COLUMN IF NOT EXISTS freigabe_datum TEXT"),
        ):
            con.execute(_ddl)
        # #1101: VVT (Art. 30) Praxisfelder für ältere DBs nachrüsten (idempotent).
        for col, ddl in (
            ('rolle', "ALTER TABLE dsgvo_vvt_pflicht ADD COLUMN IF NOT EXISTS rolle TEXT NOT NULL DEFAULT 'verantwortlicher'"),
            ('art9_grundlage', "ALTER TABLE dsgvo_vvt_pflicht ADD COLUMN IF NOT EXISTS art9_grundlage TEXT NOT NULL DEFAULT ''"),
            ('datenfluss', "ALTER TABLE dsgvo_vvt_pflicht ADD COLUMN IF NOT EXISTS datenfluss TEXT NOT NULL DEFAULT ''"),
            ('loeschfrist_ref', "ALTER TABLE dsgvo_vvt_pflicht ADD COLUMN IF NOT EXISTS loeschfrist_ref TEXT NOT NULL DEFAULT ''"),
            ('tom_ref', "ALTER TABLE dsgvo_vvt_pflicht ADD COLUMN IF NOT EXISTS tom_ref TEXT NOT NULL DEFAULT ''"),
            ('dsfa_trigger', "ALTER TABLE dsgvo_vvt_pflicht ADD COLUMN IF NOT EXISTS dsfa_trigger INTEGER NOT NULL DEFAULT 0"),
        ):
            con.execute(ddl)
        con.commit()
...
def update_projekt_meta(db_path: Path, name: str, meta: dict[str, Any]) -> None:
    ...
    try:
        con.execute(
            "UPDATE dsgvo_projekte SET meta_json=?, updated_at=aics_now() WHERE name=?",
...
def save_vvt(db_path: Path, projekt_name: str, data: dict) -> int:
    ...
    try:
        ...
        if vid:
            ...
            con.execute(f"UPDATE dsgvo_vvt_pflicht SET {sets}, updated_at=aics_now() WHERE id=?",
...
def save_tom(db_path: Path, projekt_name: str, data: dict) -> int:
    ...
    try:
        ...
        if tid:
            sets = ', '.join(f"{c}=?" for c in cols)
            con.execute(f"UPDATE dsgvo_tom SET {sets}, updated_at=aics_now() WHERE id=?",
...
def save_dpia(db_path: Path, projekt_name: str, data: dict) -> int:
    ...
    try:
        ...
        if did:
            sets = ', '.join(f"{c}=?" for c in cols)
            con.execute(f"UPDATE dsgvo_dpia SET {sets}, updated_at=aics_now() WHERE id=?",
                        values + [int(did)])
...
def set_dpia_rb_projekt(db_path: Path, row_id: int, rb_projekt_id: str) -> None:
    ...
    try:
        con.execute(
            "UPDATE dsgvo_dpia SET rb_projekt_id=?, updated_at=aics_now() WHERE id=?",
...
def save_avv(db_path: Path, projekt_name: str, data: dict) -> int:
    ...
    try:
        ...
        if aid:
            sets = ', '.join(f"{c}=?" for c in cols)
            con.execute(f"UPDATE dsgvo_avv_tracker SET {sets}, updated_at=aics_now() WHERE id=?",
                        values + [int(aid)])
...
def save_panne(db_path: Path, projekt_name: str, data: dict) -> int:
    ...
    try:
        ...
        if pid:
            sets = ', '.join(f"{c}=?" for c in cols)
            con.execute(f"UPDATE dsgvo_datenpannen SET {sets}, updated_at=aics_now() WHERE id=?",
                        values + [int(pid)])
