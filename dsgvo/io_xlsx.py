"""DSGVO-Modul – Excel Fragebogen Export und Import."""
...
_EU_BLUE = "003399"
_KAPITEL_FARBEN = {
    "GDS1": {"header": "1565C0", "soft": "E3F2FD", "text": "FFFFFF"},
    "GDS2": {"header": "4A148C", "soft": "F3E5F5", "text": "FFFFFF"},
    "GDS3": {"header": "00695C", "soft": "E0F2F1", "text": "FFFFFF"},
    "GDS4": {"header": "BF360C", "soft": "FBE9E7", "text": "FFFFFF"},
    "GDS5": {"header": "E65100", "soft": "FFF3E0", "text": "FFFFFF"},
    "GDS6": {"header": "2C3E50", "soft": "ECF0F1", "text": "FFFFFF"},
}
_BEWERTUNG_FARBEN = {
...
def _safe_filename(s: str) -> str:
    ...
    return s[:100] or "DSGVO_Fragebogen"
...
def export_fragebogen(
    *,
    ...
    out_path = out_dir / f"DSGVO_Fragebogen_{_safe_filename(projekt_name)}_{ts}.xlsx"

    wb = openpyxl.Workbook()
    ws = wb.active
    ws.title = "DSGVO-Fragebogen"

    bew = bestehende_bewertungen or {}
    org_label = ORGANISATIONSTYPEN.get(organisationstyp, {}).get("label", organisationstyp)

    ws.merge_cells("A1:J1")
    c = ws["A1"]
    c.value = "DSGVO-Readiness Questionnaire"
    ...
    c = ws["A2"]
    c.value = "Verordnung (EU) 2016/679 – Datenschutz-Grundverordnung (DSGVO)  |  AI Compliance Suite"
    ...
    meta = [
        ...
        ("Organisationstyp:", org_label),
        ("Berater:", berater),
        ("Erstellt am:", datetime.now().strftime("%d.%m.%Y")),
        ("Rechtsgrundlage:", "Verordnung (EU) 2016/679 (DSGVO)"),
    ]
    ...
    audit_event("export.write", module="dsgvo", outcome="success", details={"path": str(out_path), "kind": "xlsx"})
    return out_path
...
def import_fragebogen(xlsx_path: Path) -> list[dict[str, Any]]:
    ...
    if header_row is None:
        raise ValueError("Header-Zeile mit 'ID' nicht gefunden – ist dies ein DSGVO-Fragebogen?")

    results: list[dict[str, Any]] = []
    known_ids = {req["id"] for req in DSGVO_ANFORDERUNGEN}
