...
def _ensure_row(con: sqlite3.Connection, projekt_name: str, jahr: int) -> None:
    con.execute(
        "INSERT OR IGNORE INTO dsgvo_jahresbericht (projekt_name, jahr) VALUES (?,?)",
...
def freigeben(db_path: Path, projekt_name: str, jahr: int, *, von: str) -> dict[str, Any]:
    ...
    try:
        ...
        con.execute(
            ...
            "freigabe_am=datetime('now'), updated_at=datetime('now') "
...
def signieren(db_path: Path, projekt_name: str, jahr: int, *, von: str, name: str,
    ...
    try:
        ...
        con.execute(
            ...
            "signatur_name=?, signatur_am=datetime('now'), sha256=?, pdf_path=?, "
            "updated_at=datetime('now') WHERE projekt_name=? AND jahr=?",
