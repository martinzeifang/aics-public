...
AREA_META: list[dict[str, str]] = [
    ...
    {'key': 'datenpannen', 'label': 'Datenpannen (Art. 33-34)', 'tab': 'datenpannen'},
    {'key': 'joint_controller', 'label': 'Gemeinsame Verantwortung (Art. 26)', 'tab': 'joint-controller'},
    {'key': 'eu_vertreter', 'label': 'EU-Vertreter (Art. 27)', 'tab': 'eu-vertreter'},
]
...
def _area_datenpannen(db_path: Path, projekt: str) -> dict[str, Any]:
    # #1193: Datenpannen mit Art.-33-72-h-Frist (status-aware).
    from dsgvo import datenpannen_frist as panne_frist
    pannen = panne_frist.list_pannen_mit_frist(db_path, projekt)
    total = len(pannen)
    offene = [p for p in pannen if p.get('status') not in panne_frist.TERMINAL_STATUS]
    erledigt = total - len(offene)
    pct = _pct(erledigt, total) if total else 100
    aufgaben: list[dict[str, Any]] = []
    faellig = 0
    for p in offene:
        frist = p.get('frist', {})
        overdue = bool(frist.get('overdue'))
        if overdue:
            faellig += 1
        aufgaben.append({
            'text': f"Datenpanne {p.get('panne_id') or p.get('id')} — Art.-33-Meldung offen "
                    f"(72 h, {frist.get('ampel', 'grau')})",
            'due': frist.get('due_at') or '',
            'overdue': overdue,
        })
    return {
        'reifegrad_pct': pct,
        'status': _status_from_pct(pct, has_data=bool(total)),
        'offen': len(offene),
        'faellig': faellig,
        'aufgaben': aufgaben,
    }
...
def _area_joint_controller(db_path: Path, projekt: str) -> dict[str, Any]:
    # #1216: Gemeinsame Verantwortung (Art. 26) — Ausnahmefall, 'leer' ohne Eintrag.
    return jc_db.cockpit_summary(db_path, projekt)


def _area_eu_vertreter(db_path: Path, projekt: str) -> dict[str, Any]:
    # #1219: EU-Vertreter (Art. 27) — Pflicht-Check NUR wenn Art. 3(2) einschlägig.
    return euv_db.cockpit_summary(db_path, projekt)
...
_AREA_FUNCS = {
    ...
    'dsb': _area_dsb,
    'datenpannen': _area_datenpannen,
    'joint_controller': _area_joint_controller,
    'eu_vertreter': _area_eu_vertreter,
}
