...
def list_regeln(db_path: Path, projekt_name: str) -> list[dict[str, Any]]:
    ...
    try:
        rows = con.execute(
            ...
            WHERE projekt_name = ?
            ORDER BY LOWER(datenkategorie), LOWER(regel_id)
            """,
            (projekt_name,),
        ).fetchall()
...
def get_regel(db_path: Path, regel_pk: int,
    ...
    try:
        if projekt_name is not None:
            row = con.execute(
                "SELECT * FROM dsgvo_loeschkonzept WHERE id = ? AND projekt_name = ?",
                (regel_pk, projekt_name),
            ).fetchone()
        else:
            row = con.execute(
...
def update_status(db_path: Path, regel_pk: int, status: str,
                  projekt_name: str | None = None) -> bool:
    """Setzt nur den Status (z. B. 'erledigt'). #1173 IDOR: optional projekt-scoped."""
    ensure_table(db_path)
    con = _connect(Path(db_path))
    try:
        if projekt_name is not None:
            cur = con.execute(
                "UPDATE dsgvo_loeschkonzept SET status=?, updated_at=aics_now() "
                "WHERE id=? AND projekt_name=?",
                (status, regel_pk, projekt_name),
            )
        else:
            cur = con.execute(
                "UPDATE dsgvo_loeschkonzept SET status=?, updated_at=aics_now() WHERE id=?",
                (status, regel_pk),
            )
        con.commit()
...
def delete_regel(db_path: Path, regel_pk: int,
                 projekt_name: str | None = None) -> bool:
    ...
    try:
        if projekt_name is not None:
            cur = con.execute(
                "DELETE FROM dsgvo_loeschkonzept WHERE id=? AND projekt_name=?",
                (regel_pk, projekt_name),
            )
        else:
