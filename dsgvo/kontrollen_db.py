...
CREATE TABLE IF NOT EXISTS dsgvo_kontroll_anhaenge (
    ...
    uploaded_at   TEXT NOT NULL DEFAULT (aics_now()),
...
def save_kontrolle(db_path: Path, projekt_name: str, data: dict) -> int:
    ...
    try:
        ...
        if existing:
            ...
            con.execute(
                f"UPDATE dsgvo_kontrollen SET {sets}, updated_at=aics_now() "
    ...
    try:
        if status == "freigegeben":
            con.execute(
                ...
                "freigabe_am=aics_now(), updated_at=aics_now() WHERE id=?",
                (status, freigabe_von, int(pk)))
        else:
            con.execute(
                "UPDATE dsgvo_kontrollen SET status=?, updated_at=aics_now() WHERE id=?",
                (status, int(pk)))
        con.commit()
    finally:
        con.close()
...
def dokumentieren(db_path: Path, pk: int, *, durchgefuehrt_am: str,
    ...
    try:
        ...
        con.execute(
            ...
            f"ergebnis=?{status_clause}, updated_at=aics_now() WHERE id=?",
...
def _count_anhaenge(con: Any, kontroll_pk: int) -> int:
...
def soft_delete_anhang(db_path: Path, anhang_id: int, *, by: str, reason: str) -> bool:
    ...
    try:
        ...
        if not row:
            return False
        con.execute(
            "UPDATE dsgvo_kontroll_anhaenge SET deleted_at=aics_now(), "
