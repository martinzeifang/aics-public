...
def get_antrag(db_path: Path, antrag_pk: int) -> dict[str, Any] | None:
    ensure_table(db_path)
    con = _connect(Path(db_path))
    try:
        r = con.execute(
...
def create_antrag(db_path: Path, projekt_name: str, *, typ: str,
                  ...
                  ergebnis: str = '', notizen: str = '') -> dict[str, Any]:
    ensure_table(db_path)
    if typ not in TYPEN:
...
def update_antrag(db_path: Path, antrag_pk: int,
    ...
    ensure_table(db_path)
    existing = get_antrag(db_path, antrag_pk)
    if existing is None:
        return None
    ...
    try:
        ...
        con.close()
    return get_antrag(db_path, antrag_pk)


def delete_antrag(db_path: Path, antrag_pk: int) -> bool:
    ensure_table(db_path)
    con = _connect(Path(db_path))
    try:
        cur = con.execute(
