...
def build_prompt(risk: dict, extra_prompt: str = "") -> str:
    """Build a ChatGPT prompt that requests a JSON risk assessment.

    ``extra_prompt`` (#1494): optionaler Zusatztext (z. B. ein App-Evidenz-Block),
    der ans Ende angehängt wird. Leer → Prompt bleibt byte-identisch zum bisherigen
    Verhalten.
    """
...
def build_mass_assessment_prompt(risks: list[dict], framework: str, extra_prompt: str = "") -> str:
    ...
    ``extra_prompt`` (#1494): optionaler Zusatztext (z. B. App-Evidenz-Block), der ans
    Ende angehängt wird. Leer → Prompt bleibt byte-identisch zum bisherigen Verhalten.
    """
    fw_label = FRAMEWORK_LABELS.get(framework, framework)
...
def generate_llm(
    *,
    ...
    ``extra_prompt`` (#1494): App-Evidenz-Block ans Ende des Prompts (leer → unverändert).
    """
    for ev in generate_llm_with_meta(
        risk=risk, base_url=base_url, model=model,
        timeout_s=timeout_s, cancel_event=cancel_event, extra_prompt=extra_prompt,
    ):
...
def generate_llm_with_meta(
    *,
    ...
    prompt_text = build_prompt(risk, extra_prompt)
