...
def _append_audit(con: Any, korrektur_id: int, projekt_name: str,
...
def update_korrektur(db_path: Path, korrektur_id: int, projekt_name: str,
    ...
    try:
        cur = con.execute(
            f"UPDATE cra_korrektur SET {', '.join(sets)}, updated_at=aics_now() "
        ...
        cur = con.execute(
            """UPDATE cra_korrektur
               SET behoerde_informiert=1,
                   behoerde_info_datum=COALESCE(NULLIF(?, ''), aics_now()),
                   behoerde_name=?, updated_at=aics_now()
               WHERE id=? AND projekt_name=?""",
...
def set_status(db_path: Path, korrektur_id: int, projekt_name: str,
    ...
    try:
        if neuer_status == "abgeschlossen":
            con.execute(
                "UPDATE cra_korrektur SET status=?, abgeschlossen_am=aics_now(), "
                "updated_at=aics_now() WHERE id=? AND projekt_name=?",
                (neuer_status, korrektur_id, projekt_name),
            )
        else:
            con.execute(
                "UPDATE cra_korrektur SET status=?, updated_at=aics_now() "
                "WHERE id=? AND projekt_name=?",
                (neuer_status, korrektur_id, projekt_name),
            )
