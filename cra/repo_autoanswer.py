...
_UNIVERSAL_NAME_PATTERNS: dict[str, dict[str, list[tuple[str, int, str, float, str]]]] = {
    ...
    # ── Prefill / Engine → IMPL-03 (Tools) ────────────────────────────
    "*engine*.*": {
        "cra": [
            ("IMPL-03", 3, "Automated/Engine-basierte Unterstützung für Compliance-Bewertung.",
             0.75, "Engine-Datei gefunden: automatisierte Werkzeuge für Compliance-Prozesse."),
        ],
    },
    ...
    # ── Authentication / Access → PC-C7 → AI2-04 ──────────────────────
    "*auth*.*": {
        "owasp": [
...
def full_repo_scan(
    *, provider: str, repo: str, branch: str = "", base_url: str = "", token_env: str = ""
    ...
    best: dict[str, CRARepoSuggestion] = {}
    for s in ci_cra + sec_owasp + sec_cra:
