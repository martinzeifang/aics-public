...
class GitHubPermissionDenied(RuntimeError):
    """Token hat keine Berechtigung für diesen Endpoint (HTTP 403)."""


def _gh_api_json(path: str) -> Any:
    """GitHub-API-Aufruf mit dreistufigem Fallback (Issue #559):

    1. REST mit Token (höchstes Rate-Limit 5000/h, Docker-tauglich)
    2. `gh` CLI nur wenn authentifiziert (lokale Dev-Umgebungen)
    3. Unauthentifizierte REST-API (Public Repos, 60 req/h)

    Raises:
        GitHubPermissionDenied: HTTP 403 — Token hat zu wenig Permissions
        RuntimeError: 404 / Netzwerk / sonstiger Fehler
    """
    import shutil
    ...
    if get_github_token():
        ...
        if r.status_code == 403:
            raise GitHubPermissionDenied(
                f"Token hat keine Berechtigung für {path} — "
                f"GitHub-Antwort: {r.text[:200]}"
            )
        if r.status_code >= 300:
            raise RuntimeError(f"GitHub API {r.status_code}: {r.text[:200]}")
        return r.json()

    # gh CLI nur wenn auth_status grün
    if shutil.which("gh"):
        auth = subprocess.run(
        ...
        if auth.returncode == 0:
            p = subprocess.run(
                ["gh", "api", path], check=False, capture_output=True, text=True, encoding="utf-8",
            )
            if p.returncode == 0:
                return json.loads(p.stdout)
            # auth war ok aber API-call failed → echter Fehler
            raise RuntimeError((p.stderr or p.stdout or "gh api failed").strip())

    # Unauth-Fallback: GitHub erlaubt anonyme Calls für public Repos (60/h)
    import requests
    url = f"https://api.github.com/{path.lstrip('/')}"
    ...
    if r.status_code == 403 and 'rate limit' in r.text.lower():
        raise RuntimeError(
            "GitHub Rate-Limit (60/h ohne Token) erreicht. "
            "Bitte GH_TOKEN als Env-Var im Container setzen."
        )
    if r.status_code >= 300:
        raise RuntimeError(f"GitHub API (unauth) {r.status_code}: {r.text[:200]}")
    return r.json()


_PERMISSION_DENIED_COUNTER = {'count': 0, 'last_path': ''}


def reset_permission_counter() -> None:
    _PERMISSION_DENIED_COUNTER['count'] = 0
    _PERMISSION_DENIED_COUNTER['last_path'] = ''


def get_permission_denied_count() -> tuple[int, str]:
    return _PERMISSION_DENIED_COUNTER['count'], _PERMISSION_DENIED_COUNTER['last_path']
...
def _github_path_exists(
    ...
    #563: zählt 403-Permission-Denied separat — sonst kann der Aufrufer einen
    Aggregat-Warn-Hinweis ausgeben statt jeden Pfad einzeln als "nicht da" zu
    melden.
    """
    ...
    except GitHubPermissionDenied:
        _PERMISSION_DENIED_COUNTER['count'] += 1
        _PERMISSION_DENIED_COUNTER['last_path'] = path
        return False, None
    except Exception:
        return False, None
