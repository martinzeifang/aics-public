...
def create_meldung(db_path: Path, projekt_name: str, data: dict) -> int:
    ...
    try:
        cur = con.execute(
            ...
            VALUES (?, ?, ?, ?, 'erkannt', COALESCE(NULLIF(?, ''), datetime('now')),
                    ?, ?, ?, ?, ?, datetime('now'))
            """,
...
def update_meldung(db_path: Path, meldung_id: int, projekt_name: str,
    ...
    try:
        cur = con.execute(
            f"UPDATE cra_meldung SET {', '.join(sets)}, updated_at=datetime('now') "
...
def set_stufe(db_path: Path, meldung_id: int, projekt_name: str,
    ...
    try:
        if stage_col:
            con.execute(
                f"UPDATE cra_meldung SET status=?, {stage_col}_gemeldet_am=datetime('now'), "
                "updated_at=datetime('now') WHERE id=? AND projekt_name=?",
                (neuer_status, meldung_id, projekt_name),
            )
        else:
            con.execute(
                "UPDATE cra_meldung SET status=?, updated_at=datetime('now') "
                "WHERE id=? AND projekt_name=?",
                (neuer_status, meldung_id, projekt_name),
            )
        con.commit()
...
def set_advisory(db_path: Path, meldung_id: int, projekt_name: str,
    ...
    try:
        cur = con.execute(
            "UPDATE cra_meldung SET advisory_json=?, updated_at=datetime('now') "
            "WHERE id=? AND projekt_name=?",
