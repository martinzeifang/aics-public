...
def ensure_table(db_path: Path = DB_PATH) -> None:
    ...
    try:
        ...
        cols = {r["name"] for r in con.execute("PRAGMA table_info(cra_konformitaet)")}
        for ddl_col, ddl in (
            ("freigabe_status", "ALTER TABLE cra_konformitaet ADD COLUMN freigabe_status TEXT NOT NULL DEFAULT 'entwurf'"),
            ("freigegeben_von", "ALTER TABLE cra_konformitaet ADD COLUMN freigegeben_von TEXT NOT NULL DEFAULT ''"),
            ("freigegeben_am", "ALTER TABLE cra_konformitaet ADD COLUMN freigegeben_am TEXT"),
        ):
            if ddl_col not in cols:
                con.execute(ddl)
        con.commit()
    finally:
        con.close()
    ...
    try:
        con.execute(
            """UPDATE cra_konformitaet SET doc_json=?, doc_version=?, doc_ausgestellt=1,
                      doc_ausgestellt_am=datetime('now'), ce_status='doc_ausgestellt',
                      updated_at=datetime('now')
        ...
        con.execute(
            """UPDATE cra_konformitaet
                   SET freigabe_status='freigegeben', freigegeben_von=?,
                       freigegeben_am=datetime('now'), updated_at=datetime('now')
                   WHERE projekt_name=? AND release_version=?""",
