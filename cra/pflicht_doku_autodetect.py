...
def detect_sboms(owner: str, repo: str) -> list[dict[str, Any]]:
    ...
    # Variante B: CI-erzeugte SBOM-Artefakte (#1399) — direkt auf das ECHTE SBOM
    # verweisen (Run, der es erzeugt hat) statt auf die Workflow-YAML; Version/Datum
    # aus dem Artefakt bzw. dem letzten Release statt 'workflow-detected'.
    if not findings:
        latest_tag = next((r.get('tag_name', '') for r in releases if r.get('tag_name')), '')
        try:
            arts = (_gh_api_json(
                f"repos/{owner}/{repo}/actions/artifacts?per_page=100") or {}).get('artifacts', [])
        except Exception as e:
            log.debug("artifacts fetch failed: %s", e)
            arts = []
        for art in arts:
            if art.get('expired'):
                continue
            name = art.get('name', '')
            if not any(re.search(p, name, re.IGNORECASE) for p in (r'sbom', r'cyclonedx', r'spdx')):
                continue
            run = art.get('workflow_run') or {}
            run_id = run.get('id') or ''
            created = (art.get('created_at') or '')[:10] or datetime.utcnow().strftime('%Y-%m-%d')
            findings.append({
                'release_version': latest_tag or (str(run.get('head_sha', ''))[:7]),
                'sbom_format': 'cyclonedx' if 'cyclonedx' in name.lower() else 'spdx',
                'sbom_datum': created,
                'quelle': f"auto:github:artifact:{name}",
                'blob_path': (f"https://github.com/{owner}/{repo}/actions/runs/{run_id}"
                              if run_id else art.get('archive_download_url', '')),
                'komponenten_count': 0,
                'lizenzen': [],
                'notizen': f"Auto-erkannt aus CI-Artefakt '{name}' (enthält die erzeugte SBOM).",
            })
            break

    # Variante C (Fallback): nur SBOM-Workflow vorhanden, kein konkretes Artefakt.
    if not findings:
        for wf in SBOM_WORKFLOW_PATTERNS:
            ...
            if ok:
                latest_tag = next((r.get('tag_name', '') for r in releases if r.get('tag_name')), '')
                findings.append({
                    'release_version': latest_tag,  # #1399: kein Fake 'workflow-detected'
                    'sbom_format': 'spdx',
                    'sbom_datum': '',
                    ...
                    'komponenten_count': 0,
                    'lizenzen': [],
                    'notizen': (f"Nur SBOM-Workflow erkannt ({wf}); kein konkretes SBOM-Artefakt "
                                "gefunden. Bitte SBOM-Artefakt/Release-Asset verknüpfen."),
                })
                break
...
# Dependabot-Alert-State → cra_vuln.status
_DEPENDABOT_STATUS_MAP = {
    'open': 'open',
    'fixed': 'fixed',
    'dismissed': 'wontfix',
    'auto_dismissed': 'wontfix',
}


def detect_dependabot_vulns(owner: str, repo: str, limit: int = 100) -> list[dict[str, Any]]:
    """Lädt Dependabot-Alerts (CVEs in Drittabhängigkeiten, CRA Annex I Teil II, #937).

    Benötigt ein Token mit ``security_events: read`` (bzw. ``repo`` für private Repos).
    Mapping: GHSA/CVE → cve_id, Severity → schwere, package@manifest → affected_component,
    CVSS-Score → cvss_score.
    """
    findings: list[dict[str, Any]] = []
    try:
        alerts = _gh_api_json(
            f"repos/{owner}/{repo}/dependabot/alerts?state=open,auto_dismissed&per_page={min(limit, 100)}"
        ) or []
    except Exception as e:
        log.debug("dependabot alerts fetch failed: %s", e)
        return findings

    for al in alerts[:limit]:
        ...
        sev = SEVERITY_MAP.get((adv.get('severity') or '').lower(), 'unknown')
        cvss = (adv.get('cvss') or {}).get('score') or 0
        dep = al.get('dependency') or {}
        pkg = (dep.get('package') or {}).get('name', '')
        manifest = dep.get('manifest_path', '')
        component = f"{pkg}@{manifest}" if pkg and manifest else (pkg or manifest)
        state = (al.get('state') or '').lower()
        status = _DEPENDABOT_STATUS_MAP.get(state, 'triaging')
        fixed_in = ((al.get('security_vulnerability') or {}).get('first_patched_version') or {}).get('identifier', '')
        findings.append({
            'cve_id': cve,
            'titel': (adv.get('summary') or '')[:200],
            'schwere': sev,
            'cvss_score': float(cvss) if cvss else 0.0,
            'affected_component': component,
            'fixed_in_version': fixed_in,
            'advisory_url': al.get('html_url') or adv.get('html_url', ''),
            'status': status,
            'source': 'github_dependabot',
            'fixed_at': (al.get('fixed_at') or '')[:10] or None,
        })
    return findings


def _gl_api_json(base_url: str, path: str, token_env: str = 'GITLAB_TOKEN') -> Any:
    """Minimaler GitLab-REST-Aufruf (#937). Token aus ENV ``token_env``."""
    import os
    import requests
...
# GitLab-Severity → cra_vuln.schwere
...
def detect_vulns_gitlab(
    base_url: str, project_id: str, token_env: str = 'GITLAB_TOKEN', limit: int = 100
) -> list[dict[str, Any]]:
    """Lädt GitLab Vulnerability-Findings eines Projekts (#937).

    Endpoint: ``GET /api/v4/projects/:id/vulnerabilities`` (state detected|confirmed).
    """
    findings: list[dict[str, Any]] = []
    try:
        vulns = _gl_api_json(
            base_url,
            f"projects/{project_id}/vulnerabilities?per_page={min(limit, 100)}",
            token_env,
        ) or []
    except Exception as e:
        log.debug("gitlab vulnerabilities fetch failed: %s", e)
        return findings
    ...
    for v in vulns[:limit]:
        ...
        if state not in ('detected', 'confirmed'):
            continue
        ident = ''
        for i in v.get('identifiers') or []:
            if (i.get('external_type') or '').lower() == 'cve':
                ident = i.get('name') or i.get('external_id') or ''
                break
        cve = ident or f"GL-{v.get('id')}"
        sev = _GITLAB_SEVERITY_MAP.get((v.get('severity') or '').lower(), 'unknown')
        loc = v.get('location') if isinstance(v.get('location'), dict) else {}
        findings.append({
            'cve_id': cve,
            'titel': (v.get('title') or v.get('name') or '')[:200],
            'schwere': sev,
            'affected_component': loc.get('file', ''),
            'advisory_url': v.get('web_url', ''),
            'status': 'triaging' if state == 'confirmed' else 'open',
            'source': 'gitlab',
        })
    return findings
