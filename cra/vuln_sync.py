...
def sync_vulns(
    ...
    *,
    ...
    # Letzten Sync-Stand persistieren (#947), nur bei echtem Lauf.
    if not dry_run:
        try:
            from cra.db import record_sync_state
            record_sync_state(db_path, projekt_name, report)
        except Exception:
            log.exception("record_sync_state fehlgeschlagen")
    return report
