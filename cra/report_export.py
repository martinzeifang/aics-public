...
def build_rb_risk_summary(db_path: Path | None, projekt_name: str) -> dict[str, Any]:
    """#888: Summary der verknüpften Risikoabschätzung (Modul Risikobewertung).

    Liefert immer ein Dict; ``linked=False`` wenn kein RB-Projekt verknüpft ist
    (der Report weist die fehlende Nachweis-Verknüpfung dann ausdrücklich aus —
    keine stillen Lücken). Bezug: AI1-01 (Nachweisbarkeit der Risikoabschätzung)."""
    result: dict[str, Any] = {
        'linked': False, 'rb_projekt': '', 'framework': '',
        'total': 0, 'offen': 0, 'geloest': 0, 'by_label': {}, 'top': [],
    }
    if not db_path:
        return result
    try:
        ...
        rb_proj = rb_load(rb_db, rb_name) or {}
        risiken = load_risiken(rb_db, rb_name)
        ...
        top = sorted(
            risiken, key=lambda r: (r.get('risikowert') or 0), reverse=True,
        )[:5]
        result.update({
            'linked': True,
            'rb_projekt': rb_name,
            'framework': rb_proj.get('framework', ''),
            'total': len(risiken),
            'offen': len(risiken) - geloest,
            'geloest': geloest,
            'by_label': by_label,
            'top': [{
                'name': r.get('risk_name', ''),
                'label': r.get('risiko_label', ''),
                'wert': r.get('risikowert'),
            } for r in top],
        })
    except Exception:
        # Fehler beim Laden darf den Report nicht abbrechen.
        return result
    return result
...
def export_report_docx(
    *,
    ...
    # ── 6c. Risikoabschätzung (Modul Risikobewertung) — Nachweis AI1-01 (#888) ──
    rb_sum = build_rb_risk_summary(db_path, projekt_name)
    doc.add_page_break()
    _banner(doc, "7  RISIKOABSCHÄTZUNG (MODUL RISIKOBEWERTUNG)", _EU_BLUE)
    doc.add_paragraph(
        "Die CRA-Cybersicherheits-Risikoabschätzung (Annex I, Anforderung AI1-01) "
        "wird im Modul Risikobewertung geführt und hier als Nachweis referenziert."
    ).runs[0].font.size = Pt(10)
    if not rb_sum["linked"]:
        _banner(doc, "⚠  Keine verknüpfte Risikoabschätzung – Nachweis fehlt", "#BF360C", size=10)
        doc.add_paragraph(
            "Diesem CRA-Projekt ist kein Risikobewertungs-Projekt zugeordnet. "
            "Der Nachweis der Risikoabschätzung nach AI1-01 ist damit unvollständig. "
            "Bitte im Tab „Risikoanalyse“ ein Risikobewertungs-Projekt verknüpfen."
        ).runs[0].font.size = Pt(10)
    else:
        _section_heading(
            doc, f"Verknüpftes Projekt: {rb_sum['rb_projekt']}  ·  Framework: "
                 f"{rb_sum['framework'] or '–'}", _EU_BLUE)
        kpi_tbl = doc.add_table(rows=1, cols=3)
        for i, (lbl, val) in enumerate([
            ("Risiken gesamt", str(rb_sum["total"])),
            ("offen", str(rb_sum["offen"])),
            ("gelöst", str(rb_sum["geloest"])),
        ]):
            c = kpi_tbl.cell(0, i)
            _set_shading(c, _SOFT)
            rp = c.paragraphs[0]
            r0 = rp.add_run(f"{val}\n"); r0.bold = True; r0.font.size = Pt(16)
            rp.add_run(lbl).font.size = Pt(9)
        doc.add_paragraph()
        if rb_sum["by_label"]:
            verteilung = "  ·  ".join(f"{k}: {v}" for k, v in rb_sum["by_label"].items())
            doc.add_paragraph(f"Verteilung nach Risiko-Level:  {verteilung}").runs[0].font.size = Pt(9)
        if rb_sum["top"]:
            _section_heading(doc, "Top-Risiken (nach Risikowert)", _EU_BLUE)
            top_tbl = doc.add_table(rows=1, cols=3)
            hdr = ["Risiko", "Level", "Wert"]
            for i, h in enumerate(hdr):
                c = top_tbl.cell(0, i); _set_shading(c, _EU_BLUE)
                rr = c.paragraphs[0].add_run(h); rr.bold = True
                rr.font.size = Pt(9); rr.font.color.rgb = RGBColor(255, 255, 255)
            for t in rb_sum["top"]:
                row = top_tbl.add_row().cells
                row[0].paragraphs[0].add_run(str(t["name"])).font.size = Pt(9)
                row[1].paragraphs[0].add_run(str(t["label"] or "–")).font.size = Pt(9)
                row[2].paragraphs[0].add_run(str(t["wert"] if t["wert"] is not None else "–")).font.size = Pt(9)
...
def export_report_pdf(
    *,
    ...
    # ── Risikoabschätzung (Modul Risikobewertung) — Nachweis AI1-01 (#888) ──────
    rb_sum = build_rb_risk_summary(db_path, projekt_name)
    new_page()
    banner("RISIKOABSCHÄTZUNG (MODUL RISIKOBEWERTUNG)", eu_rgb)
    y += 8
    write("CRA-Risikoabschätzung (Annex I, AI1-01) — im Modul Risikobewertung geführt, "
          "hier als Nachweis referenziert.", x=margin, font=font_xs, fill=(60, 70, 80), spacing=12)
    if not rb_sum["linked"]:
        section_banner("KEINE VERKNÜPFTE RISIKOABSCHÄTZUNG – NACHWEIS FEHLT", "#BF360C")
        write("Diesem CRA-Projekt ist kein Risikobewertungs-Projekt zugeordnet. Der Nachweis "
              "nach AI1-01 ist unvollständig. Im Tab „Risikoanalyse“ verknüpfen.",
              x=margin, font=font_xs, fill=(150, 40, 40), spacing=12)
    else:
        write(f"Verknüpftes Projekt: {rb_sum['rb_projekt']}  ·  Framework: "
              f"{rb_sum['framework'] or '-'}", x=margin, font=font_s, fill=eu_rgb, spacing=10)
        write(f"Risiken gesamt: {rb_sum['total']}   ·   offen: {rb_sum['offen']}   ·   "
              f"gelöst: {rb_sum['geloest']}", x=margin, font=font_s, fill=(20, 30, 50))
        if rb_sum["by_label"]:
            verteilung = "   ·   ".join(f"{k}: {v}" for k, v in rb_sum["by_label"].items())
            write(f"Verteilung nach Level:  {verteilung}", x=margin, font=font_xs,
                  fill=(60, 70, 80), spacing=12)
        if rb_sum["top"]:
            write("Top-Risiken (nach Risikowert):", x=margin, font=font_s, fill=eu_rgb, spacing=10)
            for t in rb_sum["top"]:
                wert = t["wert"] if t["wert"] is not None else "-"
                write(f"• {t['name']}  —  {t['label'] or '-'}  (Wert: {wert})",
                      x=margin + 20, font=font_xs, fill=(40, 50, 60), spacing=10)
