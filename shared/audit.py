...
def audit_event(
    ...
    *,
    ...
    # #1338: zusätzlich in die append-only Postgres-Tabelle mit Integritätskette.
    # Best-effort — ein DB-Ausfall darf die fachliche Aktion nie abbrechen; das Logfile
    # oben bleibt als unabhängige zweite Spur. Das HTTP-Request-Logging bleibt bewusst
    # dateibasiert (Volumen/Lock-Kontention, siehe shared/audit_db.py).
    try:
        from shared.audit_db import record_audit_event

        actor = None
        det = payload["details"]
        if isinstance(det, dict):
            actor = det.get("user_id") or det.get("actor") or det.get("user_email")
        record_audit_event(
            ts=payload["ts"],
    ...
    except Exception:  # noqa: BLE001 — Persistenz ist additiv, nie blockierend
        pass
