...
def _open_ro(db_path: Path, probe_table: str | None = None) -> Any | None:
    """Connection zum Modul-Schema; ``None`` falls Schema/Tabelle (noch) fehlt.

    Postgres-Variante des früheren Datei-Existenz-Guards: statt ``.sqlite``-Datei
    wird geprüft, ob die erwartete Tabelle im Modul-Schema existiert (``to_regclass``).
    """
    try:
    ...
    if probe_table:
        try:
            row = con.execute("SELECT to_regclass(?)", (probe_table,)).fetchone()
...
def _firmen_projekt_names(
    con: Any, projekt_table: str, firmen_id: int
) -> list[str]:
...
def collect_soc_incidents(soc_db: Path, firmen_id: int) -> list[dict[str, Any]]:
    ...
    """
    con = _open_ro(soc_db, 'soc_incidents')
    if con is None:
        return []
    try:
