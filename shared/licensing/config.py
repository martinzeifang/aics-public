...
class LicenseClientConfig:
    ...
    verify_tls: bool           # bei Self-Signed-Certs in Dev/Intranet: False
...
def _is_private_license_host(url: str) -> bool:
    """True, wenn der Lizenzserver-Host ein Intranet-/Private-Host ist
    (RFC1918-IP, localhost, *.local/.lan/.intern). Solche Server laufen
    typischerweise mit self-signed Zertifikat (#1146)."""
    ...
    if not host:
        return False
    if host == 'localhost' or host.endswith(('.local', '.lan', '.intern', '.internal')):
        return True
    try:
    ...
    *,
    ...
    if 'verify_tls' in file_settings:
        ...
        if env_raw in {'0', 'false', 'no', 'off'}:
            ...
            verify_tls = not _is_private_license_host(url)
