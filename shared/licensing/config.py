...
def get_client_config(
    *,
    ...
    if 'verify_tls' in file_settings:
        ...
        # #742: TLS-Verifikation standardmäßig AN (secure-by-default).
        # Opt-OUT für Self-Signed/Intranet via AICS_LICENSE_VERIFY_TLS=0/false/no/off.
        env_raw = os.environ.get('AICS_LICENSE_VERIFY_TLS', '').strip().lower()
        if env_raw in {'0', 'false', 'no', 'off'}:
            verify_tls = False
        else:
            verify_tls = True  # Default (auch bei {1,true,yes,on} und unset)
