...
def _connect(db_path: Path = DEFAULT_DB_PATH) -> sqlite3.Connection:
    con = connect_sqlite(Path(db_path), anchor=Path(__file__))
...
def next_version(con: sqlite3.Connection, modul: str, name: str) -> int:
    row = con.execute(
    ...
    try:
        con.execute(
            """UPDATE template_registry
               SET aktiv=0, ist_default=0, deleted_at=datetime('now'),
