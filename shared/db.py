...
class ConnWrapper:
    ...
    def execute(self, sql: str, params: Any = None) -> CurWrapper:
        cur = CurWrapper(self._conn.cursor(row_factory=_make_row_factory()))
    ...
    def cursor(self) -> CurWrapper:
        return CurWrapper(self._conn.cursor(row_factory=_make_row_factory()))
    ...
    **TRUNCATE statt DROP** — die Tabellen bleiben bestehen (App-/API-Endpunkte setzen
    ...
    try:
        ...
        with conn.cursor() as cur:
            ...
            if targets:
                ident = ", ".join(f'"{s}"."{t}"' for s, t in targets)
                try:
...
def drop_schema(db_path: Any) -> None:
    """Schema des Moduls leeren (DROP … CASCADE + neu anlegen) — für Test-Isolation."""
    schema = schema_for(db_path)
    pool = get_pool()
    conn = pool.getconn()
    try:
    ...
    with _ensured_lock:
        _ensured_schemas.add(schema)
        # #1394: Nach DROP sind die Tabellen weg — den executescript-Prozess-Cache für
        # dieses Schema invalidieren, sonst überspringt ein folgendes ensure_db die
        # Tabellen-DDL (Cache denkt "lief schon") → UndefinedTable.
        _ensured_scripts.difference_update(
            {k for k in _ensured_scripts if k[0] == schema})
