...
def cra_risk_link_candidates(projekt_name: str):
    """RB-Projekte des gleichen Kunden als Verknüpfungs-Kandidaten (#881)."""
    try:
        p = load_projekt(DB_PATH, projekt_name)
        if not p:
            return {'error': 'Projekt nicht gefunden'}, 404
        kunde = (p.get('unternehmen') or '').strip()
        candidates = []
        if kunde:
            try:
                from risikobewertung.db import list_projekte_fuer_kunde as rb_list
                for rbp in rb_list(RB_DB_PATH, kunde):
                    candidates.append({'name': rbp.get('name'),
                                       'framework': rbp.get('framework', '')})
            except Exception:
                pass
        return {'kunde': kunde, 'candidates': candidates}, 200
    except Exception as e:
        current_app.logger.exception('%s %s — %s: %s', request.method, request.path, type(e).__name__, e)
        return {'error': 'Interner Serverfehler'}, 500
...
def cra_risk_link_set(projekt_name: str):
    ...
    try:
        ...
        return {'linked_risk_projekt': rb_name, 'summary': _rb_risk_summary(rb_name)}, 200
    except Exception as e:
        current_app.logger.exception('%s %s — %s: %s', request.method, request.path, type(e).__name__, e)
        return {'error': 'Interner Serverfehler'}, 500
