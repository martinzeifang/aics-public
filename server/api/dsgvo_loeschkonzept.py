...
@dsgvo_loeschkonzept_bp.put("/projekte/<projekt_name>/regeln/<int:regel_pk>")
@jwt_required()
@require_permission(Permission.DSGVO_WRITE)
def put_regel(projekt_name: str, regel_pk: int):
    try:
        existing = get_regel(DB_PATH, regel_pk, projekt_name)
        if not existing:
        ...
        return jsonify(get_regel(DB_PATH, regel_pk, projekt_name)), 200
    except Exception as e:
        current_app.logger.exception(
            "%s %s — %s: %s", request.method, request.path, type(e).__name__, e
        )
...
@dsgvo_loeschkonzept_bp.put("/projekte/<projekt_name>/regeln/<int:regel_pk>/status")
@jwt_required()
@require_permission(Permission.DSGVO_WRITE)
def put_status(projekt_name: str, regel_pk: int):
    try:
        data = _payload()
        status = (data.get("status") or "").strip()
        if not status:
            return {"error": 'Feld "status" ist Pflicht'}, 400
        if not update_status(DB_PATH, regel_pk, status, projekt_name):
            return {"error": "Regel nicht gefunden"}, 404
        return jsonify(get_regel(DB_PATH, regel_pk, projekt_name)), 200
    except Exception as e:
        current_app.logger.exception(
...
@dsgvo_loeschkonzept_bp.delete("/projekte/<projekt_name>/regeln/<int:regel_pk>")
@jwt_required()
@require_permission(Permission.DSGVO_WRITE)
def remove_regel(projekt_name: str, regel_pk: int):
    try:
        if not delete_regel(DB_PATH, regel_pk, projekt_name):
            return {"error": "Regel nicht gefunden"}, 404
        return {"ok": True}, 200
    except Exception as e:
        current_app.logger.exception(
