...
# Framework; Risikobewertung liefert nur Katalog + Render (reuse report_export).
try:
    from shared.reports.api import register_report_routes as _register_report_routes
    from risikobewertung import berichte_provider as _rb_berichte
    _register_report_routes(
        rb_bp, modul='risikobewertung', db_path=DB_PATH,
        catalog=_rb_berichte.catalog(), render=_rb_berichte.render,
    ...
    _logging.getLogger(__name__).warning("Risikobewertungs-Berichts-Center nicht registriert: %s", _e)
...
# ── App-Evidenz für die KI-Bewertung (#1494, Sprint #40) ─────────────────────
# Standard- und Massenbewertung sollen die App-eigenen Nachweise berücksichtigen
# (verknüpfte CRA/AI-Act-Projekte, hochgeladene Firmen-Nachweise, Risiko-Cockpit).
# Best-effort: jeder Fehler liefert leeren Block → Prompt bleibt unverändert.


def _rb_evidence_extra(projekt: Dict[str, Any], risk: Dict[str, Any],
                       cache: Dict[str, Any] | None = None) -> tuple[str, list[str]]:
    """Gerenderter Evidenz-Zusatz (``evidence_section``) + Quellen für einen Prompt.

    Liefert ``("", [])``, wenn keine Evidenz erlaubt/vorhanden ist → der aufrufende
    Prompt bleibt dann byte-identisch. Das ``projekt``-Dict trägt ``unternehmen`` +
    ``meta`` (linked_cra/aiact), sodass der geteilte Aggregator Firmen-Uploads +
    Cockpit auflösen kann.
    """
    try:
        from shared.evidence_context import evidence_block_for
        from server.services.anforderung_prompt import evidence_section
        block, sources = evidence_block_for(
            'risikobewertung', projekt, risk, cache=cache,
        )
        section = evidence_section(block) if block else ""
        return section, sources
    except Exception:  # noqa: BLE001 — Evidenz darf die Bewertung nie brechen
        return "", []
...
def upload_attachment_file(projekt_name: str):
    ...
    try:
        ...
        # #1186: zentrale Inhaltsprüfung (Magic-Bytes + Zip-Bomb-Schutz) VOR dem Speichern
...
def get_risk_prompt(projekt_name: str, risk_id: int):
    ...
    try:
        ...
        extra, _src = _rb_evidence_extra(projekt, risk)  # #1494 App-Evidenz
        prompt = build_prompt(risk, extra)
...
def ollama_stream(projekt_name: str, risk_id: int):
    ...
    def _gen():
        ...
        try:
            ...
            extra, evidence_sources = _rb_evidence_extra(projekt, risk)  # #1494 App-Evidenz
            ...
            # #1494/Q3: protokolliere genutzte App-Evidenz (nur wenn welche im Prompt war)
            try:
                from shared.evidence_context import audit_assessment
                audit_assessment('risikobewertung', str(risk_id), evidence_sources, None, provider_name)
            except Exception:  # noqa: BLE001
                pass

            yield _ev('done', {
                'ok': True,
                'provider': provider_name,  # #1418 — genutzten Provider sichtbar machen
                'felder': felder,
                'risikowert': risikowert,
                'risiko_label': label,
                'detail_text': detail_text,
                ...
                'empfehlungen': parsed.get('empfehlungen', []),
                'cra_referenz': parsed.get('cra_referenz', ''),
                'bytes': total,
...
def ollama_single(projekt_name: str, risk_id: int):
    ...
    try:
        ...
        try:  # #1494/Q3: genutzte App-Evidenz protokollieren
            from shared.evidence_context import audit_assessment
            audit_assessment('risikobewertung', str(risk_id), evidence_sources, None, provider_name)
        except Exception:  # noqa: BLE001
            pass
...
def re_assessment_stream(projekt_name: str, risk_id: int):
    ...
    def _gen():
        ...
        try:
            ...
            full_text = ''.join(chunks)
            try:
                # Kanonischer, robuster Parser (Fences + eingebettetes JSON, #1361/#1421).
                from risikobewertung.prompts import parse_json_antwort
                parsed = parse_json_antwort(full_text) if full_text.strip() else {}
            except ValueError as je:
                yield _ev('done', {'ok': False,
            ...
            yield _ev('done', {
                ...
                'detail_text': detail_text,
                'bewertung_text': risk['bewertung_text'],
                # #1421 — Ergebnis-Zusammenfassung (Parität mit Auto-Bewertung #1407)
                'risiko_veraenderung': parsed.get('risiko_veraenderung', ''),
                'begruendung_veraenderung': parsed.get('begruendung_veraenderung', ''),
                'empfehlungen': parsed.get('empfehlungen', []),
                'cra_referenz': parsed.get('cra_referenz', ''),
            })
        except Exception as e:
...
def mass_prompt(projekt_name: str):
    ...
    try:
        ...
        evidence_cache: Dict[str, Any] = {}  # #1494: firmenweite Lookups je Lauf cachen
        prompts = []
        for r in risiken:
            r_with_proj = dict(r)
            ...
            try:
                extra, _src = _rb_evidence_extra(projekt, r_with_proj, cache=evidence_cache)
                prompts.append({
                    'risk_id': r.get('id'),
                    'risk_name': r.get('risk_name', ''),
                    'prompt': build_prompt(r_with_proj, extra),
                })
            except Exception as e:
...
def mass_assess_prompt(projekt_name: str):
    ...
    try:
        ...
        framework = projekt.get('framework', 'STRIDE')
        # #1494: App-Evidenz EINMAL je Lauf auflösen (firmenweite Lookups gecacht).
        # Für die Massenbewertung genügt ein gemeinsamer Block (selbe Firma/Projekt).
        cache: Dict[str, Any] = {}
        first_risk = dict(risiken[0]); first_risk['projekt_name'] = projekt_name
        first_risk['framework'] = framework
        extra, _src = _rb_evidence_extra(projekt, first_risk, cache=cache)
        prompt = build_mass_assessment_prompt(risiken, framework, extra)
        return {
            'prompt': prompt,
            'framework': framework,
            'count': len(risiken),
