...
# Einheitliches Berichts-Center (Sprint #35, #1387) — Standard-Routen über das
# geteilte Framework; DSGVO liefert nur Katalog + Render (Gesamtbericht via
# dsgvo.report_export, 8 Bereichsberichte via dsgvo.einzelberichte). Der
# Jahresbericht mit Sign-off bleibt unverändert ein eigener Tab.
try:
    from shared.reports.api import register_report_routes as _register_report_routes
    from dsgvo import berichte_provider as _dsgvo_berichte
    _register_report_routes(
        dsgvo_bp, modul='dsgvo', db_path=DB_PATH,
        catalog=_dsgvo_berichte.catalog(), render=_dsgvo_berichte.render,
        project_scoped=True, zeitraum=False,
        summary_context=_dsgvo_berichte.summary_context, modul_label='DSGVO',
    )
except Exception as _e:  # pragma: no cover — Berichts-Center optional, App startet trotzdem
    import logging as _logging
    _logging.getLogger(__name__).warning("DSGVO-Berichts-Center nicht registriert: %s", _e)
...
def _anf_evidence_block(projekt: Dict[str, Any], req: Dict[str, Any]) -> tuple[str, list]:
    """App-Nachweis-Block + Quellen für eine DSGVO-Anforderung (#1491). Best-effort."""
    try:
        from shared.evidence_context import evidence_block_for
        return evidence_block_for('dsgvo', projekt, req)
...
def anf_parse_response(projekt_name: str, req_id: str):
    ...
    try:
        ...
        _n = normalize_eval_parsed(parsed)  # #1419
        score = _n['score']
...
def _dsgvo_build_prompt(req: Dict[str, Any], projekt: Dict[str, Any], current: Dict[str, Any],
                        evidence_block: str = '') -> str:
    from server.services.anforderung_prompt import build_anforderung_prompt
    return build_anforderung_prompt(
        framework='DSGVO',
        framework_full='DSGVO / Verordnung (EU) 2016/679',
        req=req, projekt=projekt, current=current,
        evidence_block=evidence_block,
    )


@dsgvo_bp.post('/projekte/<projekt_name>/anforderungen/<req_id>/auto-bewertung')
@jwt_required()
def anf_auto_bewertung(projekt_name: str, req_id: str):
    ...
    try:
        ...
        block, sources = _anf_evidence_block(projekt, req)
        prompt = _dsgvo_build_prompt(req, projekt, load_bewertungen(DB_PATH, projekt_name).get(req_id, {}),
                                     evidence_block=block)
        try:
        ...
        save_bewertung(DB_PATH, projekt_name, req_id, res['score'], res['kommentar'], res['massnahme'])
        from shared.evidence_context import audit_assessment
        audit_assessment('dsgvo', req_id, sources, res.get('genutzte_nachweise'), res.get('provider'))
...
@dsgvo_bp.post('/projekte/<projekt_name>/anforderungen/<req_id>/auto-bewertung/stream')
@jwt_required()
...
@dsgvo_bp.post('/projekte/<projekt_name>/anforderungen/auto-bewertung-mass')
@jwt_required()
def anf_auto_bewertung_mass(projekt_name: str):
    ...
    try:
        ...
        for req in targets:
            ...
            try:
                try:
                    from shared.evidence_context import audit_assessment, evidence_block_for
                    block, sources = evidence_block_for('dsgvo', projekt, req, cache=ev_cache)
                except Exception:  # noqa: BLE001
                    block, sources = '', []
                prompt = _dsgvo_build_prompt(req, projekt, bewertungen.get(rid, {}), evidence_block=block)
                res = evaluate_prompt(prompt)
                save_bewertung(DB_PATH, projekt_name, rid, res['score'], res['kommentar'], res['massnahme'])
                if sources:
                    audit_assessment('dsgvo', rid, sources, res.get('genutzte_nachweise'), res.get('provider'))
                bewertet += 1
            except AutoBewertungError as e:
