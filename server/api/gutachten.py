...
def gerichts_docx(projekt_name: str):
    # #956: Anhang (Kap. VIII) optional ausschließen (Default an).
    include_anhang = (request.args.get('include_anhang', 'true').lower() != 'false')
    ...
    if template_id:
        try:
            from gutachten.template_render import build_template_context, render_with_template
            ...
            render_with_template(Path(tpl['datei_pfad']), ctx, out)
            data = out.read_bytes()
            try:
    ...
    try:
        doc = _ggen.build_gerichtsgutachten_docx(projekt_name, DB_PATH, include_anhang=include_anhang)
    except ValueError as e:
        return jsonify({'error': str(e)}), 400
    buf = BytesIO()
    doc.save(buf)
...
def template_upload():
    ...
    # Variablen extrahieren (nur wenn docxtpl verfügbar) + klassifizieren.
    variablen: list = []
    try:
        from gutachten.template_render import extract_variables
        from gutachten.template_schema import classify_variables
        tokens = extract_variables(dest)
        variablen = [{'token': t} for t in sorted(tokens)]
        classify = classify_variables(tokens)
    except Exception as e:
        current_app.logger.info('Variablen-Extraktion übersprungen: %s', e)
        classify = {'erkannt': [], 'frei': [], 'ungueltig': []}
    ...
    try:
        ...
        return jsonify({'error': str(e)}), 400
    return jsonify({'id': tid, 'ok': True, 'variablen': variablen, 'klassifikation': classify}), 201
