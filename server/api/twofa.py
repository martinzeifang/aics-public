...
def setup():
    ...
    # #1183: Step-up — TOTP-Setup ist eine sensible Account-Security-Aktion.
...
def verify():
    ...
    try:
        ...
        audit_event('mfa.totp.enabled', module='auth.2fa', details={'user_id': user_id})
    except Exception:
        log.exception("Audit-Event 'mfa.totp.enabled' konnte nicht geschrieben werden")
...
def disable():
    ...
    try:
        from shared.audit import audit_event
        audit_event('mfa.totp.disabled', module='auth.2fa', details={'user_id': user_id})
    except Exception:
        log.exception("Audit-Event 'mfa.totp.disabled' konnte nicht geschrieben werden")
    return jsonify({'enabled': False}), 200
