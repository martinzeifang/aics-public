...
def close_incident(incident_id: int):
    ...
    # #1454: aktualisierten Incident mitliefern → Frontend mergt statt nachzuladen.
    return jsonify({"ok": True, "incident": sdb.get_incident(DB_PATH, incident_id)})
...
# ── Geplante Berichte: GUI-Konfiguration (#1405) ────────────────────────────

def _reregister_bericht_jobs() -> None:
    """Best-effort: laufenden Scheduler die Berichts-Jobs neu registrieren lassen.
    Greift nur, wenn der In-App-Scheduler bereits läuft; sonst beim nächsten Start."""
    try:
        ...
        if sched is not None:
            register_soc_bericht_jobs(sched, current_app._get_current_object())
    except Exception:  # noqa: BLE001
        pass


@soc_bp.get("/berichte/schedule")
@jwt_required()
@require_permission(Permission.SOC_READ)
def berichte_schedule_list():
    """Liste der geplanten (automatischen) Berichte + verfügbare Presets."""
    from soc import berichte
    return jsonify({
        "schedules": berichte.list_schedules(DB_PATH),
        "typen": berichte.available_reports(),
        "perioden": [{"key": "quartal", "label": "Quartalsweise (1. des Quartals)"},
                     {"key": "jahr", "label": "Jährlich (1. Januar)"}],
    })


@soc_bp.post("/berichte/schedule")
@jwt_required()
@require_permission(Permission.SOC_EXPORT)
def berichte_schedule_add():
    from soc import berichte
    d = request.get_json(silent=True) or {}
    try:
        sid = berichte.add_schedule(DB_PATH, typ=(d.get("typ") or "").strip(),
                                    periode=(d.get("periode") or "quartal").lower(),
                                    fmt=(d.get("format") or "docx").lower())
    except ValueError as e:
        return jsonify({"error": str(e)}), 400
    _reregister_bericht_jobs()
    _audit("soc.berichte.schedule_add", typ=d.get("typ"), periode=d.get("periode"))
    return jsonify({"ok": True, "id": sid}), 201


@soc_bp.patch("/berichte/schedule/<int:sid>")
@jwt_required()
@require_permission(Permission.SOC_EXPORT)
def berichte_schedule_patch(sid: int):
    from soc import berichte
    d = request.get_json(silent=True) or {}
    berichte.set_schedule_aktiv(DB_PATH, sid, bool(d.get("aktiv")))
    _reregister_bericht_jobs()
    return jsonify({"ok": True})


@soc_bp.delete("/berichte/schedule/<int:sid>")
@jwt_required()
@require_permission(Permission.SOC_EXPORT)
def berichte_schedule_delete(sid: int):
    from soc import berichte
    berichte.delete_schedule(DB_PATH, sid)
    _reregister_bericht_jobs()
    _audit("soc.berichte.schedule_delete", sid=sid)
    return jsonify({"ok": True})
...
def get_incident(incident_id: int):
    ...
    # #1454: kein Seeding mehr bei JEDEM Read. seed_defaults ist prozess-weit
    # geguarded (höchstens einmal je DB) → der Lese-Pfad löst nach dem ersten
    # Aufruf keine COUNT/INSERT-Schreibzugriffe mehr aus (Lock-Contention/Latenz).
...
def incident_status(incident_id: int):
    ...
    # #1454: aktualisierten Incident mitliefern → Frontend kann mergen statt einen
    # zweiten (teuren) GET /incidents/<id> abzusetzen.
    return jsonify({"ok": True, "status": target, "incident": sdb.get_incident(DB_PATH, incident_id)})
