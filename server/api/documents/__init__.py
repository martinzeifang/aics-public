...
def make_documents_blueprint(modul: str, url_prefix: str, db_path: Path,
    ...
    # ── #1423: KI-Vollständigkeitsprüfung (API/Cloud oder lokale LLM) als SSE ──
    @bp.post("/<projekt>/<int:doc_id>/ki-check/stream")
    @jwt_required()
    @require_permission(perm_write)
    def ki_check_stream(projekt, doc_id):
        """Live-KI-Prüfung der Pflichtinhalte; schreibt Checklist-Status + liefert Lücken."""
        d = ddb.get_document(db_path, modul, doc_id)
        if not d or d.get("projekt") != projekt:
            return jsonify({"error": "Dokument nicht gefunden"}), 404
        soll = dcat.get_checklist(modul, d.get("doc_type", ""))
        ...
        from shared.documents import ki_check as kc
        titel = d.get("titel") or d.get("doc_type") or "Dokument"
        prompt = kc.build_check_prompt(titel, d.get("content_html") or "", soll)
        updated_by = str(get_jwt_identity() or "")

        def _finalize(full_text: str) -> dict:
            res = kc.parse_check_response(full_text, soll)
            ddb.set_checklist_status(db_path, modul, doc_id,
                                     kc.checklist_status_updates(res["items"]), updated_by=updated_by)
            _audit(modul, "ki_check", projekt=projekt, doc_id=doc_id,
                   pflicht_erfuellt=res["pflicht_erfuellt"], pflicht_gesamt=res["pflicht_gesamt"],
                   luecken=len(res["gaps"]))
            return {"gaps": res["gaps"], "erfuellt": res["erfuellt"], "gesamt": res["gesamt"],
                    "pflicht_erfuellt": res["pflicht_erfuellt"],
                    "pflicht_gesamt": res["pflicht_gesamt"], "items": res["items"]}

        return stream_ai_sse(kc._SYSTEM, prompt, finalize=_finalize, force_json=True,
