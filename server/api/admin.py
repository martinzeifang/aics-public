...
def disable_user(user_id: str):
    from server.auth.users_db import update_user as db_update_user
...
def _mask_sensitive(cfg: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if auth.get('password_hash'):
        auth['password_hash'] = '***'
    # GitHub-Token nicht zurückgeben, nur signalisieren ob gesetzt
    gh = (out.get('integrations') or {}).get('github') or {}
    if gh.get('token'):
        gh['token'] = '***'
        gh['token_set'] = True
    else:
    ...
    try:
        ...
        try:
            ...
            if users_db.exists():
                ...
                _con = sqlite3.connect(str(users_db))
