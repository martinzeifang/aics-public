...
def _filtered_cockpit(firmen_id: int) -> dict:
    ...
    soc_db = Path(current_app.config.get("SOC_DB_PATH", "data/db/soc.sqlite"))
    data = build_cockpit(int(firmen_id), rb_db=rb_db, cra_db=cra_db, soc_db=soc_db)
