...
def download_anhang(anhang_id):
    ...
    # #1180: Pfad-Containment unter dem Anhang-Store erzwingen (Path-Traversal-Schutz).
    try:
        from security_utils import ensure_within_root
        p = ensure_within_root(Path(a["stored_path"]), kdb.ATTACH_DIR)
    except ValueError:
        current_app.logger.error("download_anhang: stored_path außerhalb des Stores: %r",
                                  a.get("stored_path"))
        return jsonify({"error": "Anhang nicht gefunden"}), 404
    if not p.exists():
