...
# Framework; NIS2 liefert nur Katalog + Render (reuse nis2.report_export).
try:
    from shared.reports.api import register_report_routes as _register_report_routes
    from nis2 import berichte_provider as _nis2_berichte
    _register_report_routes(
        nis2_bp, modul='nis2', db_path=DB_PATH,
        catalog=_nis2_berichte.catalog(), render=_nis2_berichte.render,
        project_scoped=True, zeitraum=False,
        summary_context=_nis2_berichte.summary_context, modul_label='NIS2',
    )
except Exception as _e:  # pragma: no cover — Berichts-Center optional, App startet trotzdem
    import logging as _logging
    _logging.getLogger(__name__).warning("NIS2-Berichts-Center nicht registriert: %s", _e)
...
def export_report(projekt_name: str):
    ...
    # Governance-Nachweise (Art. 20, #1212) in den Readiness-Report aufnehmen.
    try:
        from nis2 import governance_db as _gov
        pflicht_doku['governance'] = _gov.list_nachweise(DB_PATH, projekt_name)
...
def _anf_evidence_block(projekt: Dict[str, Any], req: Dict[str, Any]) -> tuple[str, list]:
    """App-Nachweis-Block + Quellen für eine NIS2-Anforderung (#1489). Best-effort."""
    try:
        from shared.evidence_context import evidence_block_for
        return evidence_block_for('nis2', projekt, req)
    except Exception:  # noqa: BLE001
        return '', []
...
def _build_anforderung_prompt(req: Dict[str, Any], projekt: Dict[str, Any], current: Dict[str, Any],
    ...
    from server.services.anforderung_prompt import EVAL_AUFTRAG_FORMAT, evidence_section  # #1419/#1489
...
def _nis2_save_parsed(projekt_name: str, req_id: str, res: Dict[str, Any]) -> None:
    """Speichert ein KI-Parse-Resultat über die bestehende save_bewertung."""
    existing = load_bewertungen(DB_PATH, projekt_name).get(req_id, {})
    db_save_bewertung(
        DB_PATH,
        projekt_name=projekt_name,
...
@nis2_bp.post('/projekte/<projekt_name>/anforderungen/<req_id>/auto-bewertung')
@jwt_required()
def anf_auto_bewertung(projekt_name: str, req_id: str):
    ...
    try:
        ...
        _nis2_save_parsed(projekt_name, req_id, res)
        from shared.evidence_context import audit_assessment
        audit_assessment('nis2', req_id, sources, res.get('genutzte_nachweise'), res.get('provider'))
...
@nis2_bp.post('/projekte/<projekt_name>/anforderungen/<req_id>/auto-bewertung/stream')
@jwt_required()
def anf_auto_bewertung_stream(projekt_name: str, req_id: str):
    ...
    def _save(score, kommentar, massnahme):
        _nis2_save_parsed(projekt_name, req_id,
                          {'score': score, 'kommentar': kommentar, 'massnahme': massnahme})
    return stream_auto_bewertung(prompt, save=_save)


@nis2_bp.post('/projekte/<projekt_name>/anforderungen/auto-bewertung-mass')
@jwt_required()
        ...
        for req in targets:
            ...
            try:
                try:
                    ...
                    block, sources = evidence_block_for('nis2', projekt, req, cache=ev_cache)
                except Exception:  # noqa: BLE001
                    block, sources = '', []
                prompt = _build_anforderung_prompt(req, projekt, bewertungen.get(rid, {}), evidence_block=block)
                res = evaluate_prompt(prompt)
                _nis2_save_parsed(projekt_name, rid, res)
                if sources:
                    audit_assessment('nis2', rid, sources, res.get('genutzte_nachweise'), res.get('provider'))
                bewertet += 1
            except AutoBewertungError as e:
...
def risiken_save(projekt_name: str):
    # S7 (#1077): Manuelle N2-Risikoeinträge sind ab Sprint #21 deaktiviert.
    # Risiken werden zentral in der Risikobewertung gepflegt und im Risiko-Cockpit
    # aggregiert angezeigt; Bestands-Risiken im nis2_risiko_register bleiben lesbar
    # (NIS2 Art. 21 (2) lit. a bleibt über die Risikobewertung abgedeckt).
    # Hinweis: Der RB-Import (…/wizards/import-rb-risks/apply) schreibt direkt über
    # nis2.db.save_risiko und ist von dieser Sperre nicht betroffen.
    p, err = _require_nis2_projekt(projekt_name)
    if err: return err
    return jsonify({
        'error': 'Risiken werden ab Sprint #21 in der Risikobewertung gepflegt'
    }), 409
...
from nis2.ai_wizards import (
    ...
    # #1240 — Pflichtdokument-Generatoren (Doc-only, Art. 21(2))
    build_is_leitlinie_prompt, parse_is_leitlinie_response,
    build_incident_handling_konzept_prompt, parse_incident_handling_konzept_response,
    build_bcm_dr_plan_prompt, parse_bcm_dr_plan_response,
    build_lieferketten_richtlinie_prompt, parse_lieferketten_richtlinie_response,
    build_krypto_richtlinie_prompt, parse_krypto_richtlinie_response,
    build_zugriffskontroll_policy_prompt, parse_zugriffskontroll_policy_response,
)
...
def nis2_klass_parse(projekt_name: str):
    ...
    if _should_apply_nis2(body) and parsed.get('klasse'):
        ...
        # einrichtungsklasse-Spalte auch updaten. #1210-Fix: der Klassifikator
        # liefert EN-Rohwerte (essential/important/out-of-scope); die Spalte +
        # API-Validierung kennen aber nur {wesentlich,wichtig}. Daher kanonisch
        # mappen (essential→wesentlich, important→wichtig, sonst out-of-scope).
        from nis2.db import save_projekt as nis2_save_projekt
        from nis2.scoping_db import canonical_klasse
        klasse_db = canonical_klasse(parsed['klasse'])
        if klasse_db == 'out-of-scope':
            # out-of-scope ist kein gültiger Spaltenwert -> bestehende Klasse halten.
            klasse_db = p.get('einrichtungsklasse', 'wesentlich') or 'wesentlich'
        nis2_save_projekt(DB_PATH, p['name'], p.get('unternehmen', ''),
                          klasse_db, p.get('beschreibung', ''),
                          p.get('berater', ''), meta)
        applied = True
...
def nis2_incident_parse(projekt_name: str):
    ...
    if _should_apply_nis2(body) and parsed.get('early_warning'):
        ...
        db_save_ir(DB_PATH, projekt_name, ir)
        # #1194: bei vorhandener incident_id alle drei Stufen an einen Vorfall binden.
        incident_pk = None
        if parsed.get('incident_id'):
            incident_pk = _bind_incident_meldung(
                projekt_name, parsed, typ='24h', text=parsed['early_warning'],
                default_titel='Vorfall-Meldepaket')
            if parsed.get('notification'):
                _bind_incident_meldung(projekt_name, parsed, typ='72h',
                                       text=parsed['notification'])
            if parsed.get('final_report'):
                _bind_incident_meldung(projekt_name, parsed, typ='1M',
                                       text=parsed['final_report'])
        applied = True
        current_app.logger.info('wizard.applied kind=nis2-incident-notification project=%r', projekt_name)
        return jsonify({**parsed, 'applied': applied, 'incident_pk': incident_pk})
    return jsonify({**parsed, 'applied': applied})
...
# ════════════════════════════════════════════════════════════════════
# #1240 — Pflichtdokument-Generatoren (Doc-only, Art. 21(2) NIS2)
#
# Je Pflichtdokument ein Copy/Paste-Generator: GET …/prompt liefert einen
# kontext-bewussten Prompt (Vorbefüllung aus N1/N3/N4/N5, keine Doppel-
# erfassung), POST …/parse liefert NUR Vorschau + Plausibilitätshinweise
# (applied=False). Die Persistenz erfolgt bewusst über „Als Dokument
# speichern" (#1235) → editier-/freigabe-/exportierbares managed_doc.
# ════════════════════════════════════════════════════════════════════

@nis2_bp.get('/projekte/<projekt_name>/wizards/is-leitlinie/prompt')
@jwt_required()
def nis2_is_leitlinie_prompt(projekt_name: str):
    p, err = _require_nis2_projekt(projekt_name)
    if err:
        return err
    assets = db_list_assets(DB_PATH, projekt_name)
    return jsonify({'prompt': build_is_leitlinie_prompt(p, assets)})


@nis2_bp.post('/projekte/<projekt_name>/wizards/is-leitlinie/parse')
@jwt_required()
def nis2_is_leitlinie_parse(projekt_name: str):
    p, err = _require_nis2_projekt(projekt_name)
    if err:
        return err
    body = request.get_json(silent=True) or {}
    parsed = parse_is_leitlinie_response(body.get('response', ''))
    current_app.logger.info('wizard.parsed kind=nis2-is-leitlinie project=%r text_len=%d',
                            projekt_name, len(parsed.get('doc_text', '')))
    return jsonify({**parsed, 'applied': False})


@nis2_bp.get('/projekte/<projekt_name>/wizards/incident-handling-konzept/prompt')
@jwt_required()
def nis2_incident_konzept_prompt(projekt_name: str):
    p, err = _require_nis2_projekt(projekt_name)
    if err:
        return err
    ir = db_load_ir(DB_PATH, projekt_name) or {}
    return jsonify({'prompt': build_incident_handling_konzept_prompt(p, ir)})


@nis2_bp.post('/projekte/<projekt_name>/wizards/incident-handling-konzept/parse')
@jwt_required()
def nis2_incident_konzept_parse(projekt_name: str):
    p, err = _require_nis2_projekt(projekt_name)
    if err:
        return err
    body = request.get_json(silent=True) or {}
    parsed = parse_incident_handling_konzept_response(body.get('response', ''))
    current_app.logger.info('wizard.parsed kind=nis2-incident-handling-konzept project=%r text_len=%d',
                            projekt_name, len(parsed.get('doc_text', '')))
    return jsonify({**parsed, 'applied': False})


@nis2_bp.get('/projekte/<projekt_name>/wizards/bcm-dr-plan/prompt')
@jwt_required()
def nis2_bcm_dr_prompt(projekt_name: str):
    p, err = _require_nis2_projekt(projekt_name)
    if err:
        return err
    bcp = db_load_bcp(DB_PATH, projekt_name) or {}
    return jsonify({'prompt': build_bcm_dr_plan_prompt(p, bcp)})


@nis2_bp.post('/projekte/<projekt_name>/wizards/bcm-dr-plan/parse')
@jwt_required()
def nis2_bcm_dr_parse(projekt_name: str):
    p, err = _require_nis2_projekt(projekt_name)
    if err:
        return err
    body = request.get_json(silent=True) or {}
    parsed = parse_bcm_dr_plan_response(body.get('response', ''))
    current_app.logger.info('wizard.parsed kind=nis2-bcm-dr-plan project=%r text_len=%d',
                            projekt_name, len(parsed.get('doc_text', '')))
    return jsonify({**parsed, 'applied': False})


@nis2_bp.get('/projekte/<projekt_name>/wizards/lieferketten-richtlinie/prompt')
@jwt_required()
def nis2_lieferkette_prompt(projekt_name: str):
    p, err = _require_nis2_projekt(projekt_name)
    if err:
        return err
    vendors = db_list_vendors(DB_PATH, projekt_name)
    return jsonify({'prompt': build_lieferketten_richtlinie_prompt(p, vendors)})


@nis2_bp.post('/projekte/<projekt_name>/wizards/lieferketten-richtlinie/parse')
@jwt_required()
def nis2_lieferkette_parse(projekt_name: str):
    p, err = _require_nis2_projekt(projekt_name)
    if err:
        return err
    body = request.get_json(silent=True) or {}
    parsed = parse_lieferketten_richtlinie_response(body.get('response', ''))
    current_app.logger.info('wizard.parsed kind=nis2-lieferketten-richtlinie project=%r text_len=%d',
                            projekt_name, len(parsed.get('doc_text', '')))
    return jsonify({**parsed, 'applied': False})


@nis2_bp.get('/projekte/<projekt_name>/wizards/krypto-richtlinie/prompt')
@jwt_required()
def nis2_krypto_prompt(projekt_name: str):
    p, err = _require_nis2_projekt(projekt_name)
    if err:
        return err
    return jsonify({'prompt': build_krypto_richtlinie_prompt(p)})


@nis2_bp.post('/projekte/<projekt_name>/wizards/krypto-richtlinie/parse')
@jwt_required()
def nis2_krypto_parse(projekt_name: str):
    p, err = _require_nis2_projekt(projekt_name)
    if err:
        return err
    body = request.get_json(silent=True) or {}
    parsed = parse_krypto_richtlinie_response(body.get('response', ''))
    current_app.logger.info('wizard.parsed kind=nis2-krypto-richtlinie project=%r text_len=%d',
                            projekt_name, len(parsed.get('doc_text', '')))
    return jsonify({**parsed, 'applied': False})


@nis2_bp.get('/projekte/<projekt_name>/wizards/zugriffskontroll-policy/prompt')
@jwt_required()
def nis2_zugriff_prompt(projekt_name: str):
    p, err = _require_nis2_projekt(projekt_name)
    if err:
        return err
    assets = db_list_assets(DB_PATH, projekt_name)
    return jsonify({'prompt': build_zugriffskontroll_policy_prompt(p, assets)})


@nis2_bp.post('/projekte/<projekt_name>/wizards/zugriffskontroll-policy/parse')
@jwt_required()
def nis2_zugriff_parse(projekt_name: str):
    p, err = _require_nis2_projekt(projekt_name)
    if err:
        return err
    body = request.get_json(silent=True) or {}
    parsed = parse_zugriffskontroll_policy_response(body.get('response', ''))
    current_app.logger.info('wizard.parsed kind=nis2-zugriffskontroll-policy project=%r text_len=%d',
                            projekt_name, len(parsed.get('doc_text', '')))
    return jsonify({**parsed, 'applied': False})
...
def nis2_rb_risks_list(projekt_name: str):
    ...
    con = _sdb.connect(str(RB_DB_PATH))
    try:
    ...
    # Bereits importierte erkennen (#1365: primär über source_risk_id,
    # backward-compat über den alten source_rb_risk_id-Hack in notizen)
    already_imported_ids: set[int] = set()
    for r in db_list_risiken(DB_PATH, projekt_name):
        if (r.get('source_modul') or '') == 'risikobewertung':
            try:
                already_imported_ids.add(int(r.get('source_risk_id') or 0))
...
def nis2_rb_risks_import(projekt_name: str):
    ...
    # #1365: bereits importierte RB-Risiken über echte Quell-Felder erkennen
    existing_source_ids: set[int] = set()
    for r in existing_risiken:
        if (r.get('source_modul') or '') == 'risikobewertung':
            try:
                existing_source_ids.add(int(r.get('source_risk_id') or 0))
            except (ValueError, TypeError):
                pass
    ...
    for r in rb_rows:
        ...
        if nis2_risiko_id in existing_ids or rb['id'] in existing_source_ids:
        ...
        payload = {
            ...
            # #1365: echte Quell-Verlinkung (source_modul/source_ref/source_risk_id)
            'source_modul': 'risikobewertung',
            'source_ref': rb.get('projekt_name', ''),
            'source_risk_id': str(rb['id']),
            'notizen': (
...
def _bind_incident_meldung(projekt_name: str, parsed: dict, *, typ: str,
                           text: str, default_titel: str = '') -> int | None:
    """N-INC (#1194): Wizard-Ergebnis an einen **konkreten Vorfall** binden.

    Upsert von ``nis2_incident`` (anhand der vom Wizard erzeugten ``incident_id``)
    + Speichern der jeweiligen Melde-Stufe (24h/72h/1M) in ``nis2_incident_meldung``.
    Ersetzt das alleinige Anhängen an ``ir.kommunikationsplan`` (das als Audit-Spur
    erhalten bleibt). Gibt den Vorfall-PK zurück (oder None bei fehlender ID).
    """
    incident_id = str(parsed.get('incident_id') or '').strip()
    if not incident_id:
        return None
    from nis2 import incident_db as _idb
    schwere = (parsed.get('schweregrad') or 'mittel')
    schwere = schwere if schwere in _idb.SCHWEREGRADE else 'mittel'
    existing = None
    for inc in _idb.list_incidents(DB_PATH, projekt_name):
        if inc.get('incident_id') == incident_id:
            existing = inc
            break
    payload = {
        'incident_id': incident_id,
        'titel': (existing or {}).get('titel') or default_titel or incident_id,
        'kenntnis_zeitpunkt': (existing or {}).get('kenntnis_zeitpunkt')
            or parsed.get('kenntnis_zeitpunkt', ''),
        'schweregrad': schwere if not existing else (existing.get('schweregrad') or schwere),
        'grenzueberschreitend': bool(parsed.get('grenzueberschreitend')
                                     or (existing or {}).get('grenzueberschreitend')),
        'status': (existing or {}).get('status', 'offen'),
    }
    pk = _idb.save_incident(DB_PATH, projekt_name, payload)
    _idb.save_meldung(DB_PATH, pk, {
        'typ': typ, 'status': 'uebermittelt',
        'ist_zeitpunkt': _idb_now_iso(),
        'text': text,
        'bsi_referenz': parsed.get('bsi_referenz', ''),
    })
    return pk


def _idb_now_iso() -> str:
...
def nis2_incident_24h_parse(projekt_name: str):
    ...
    if _should_apply_nis2(body) and parsed.get('kurztext'):
        ...
        # #1194: zusätzlich an einen konkreten Vorfall binden (Lifecycle-Register).
        incident_pk = _bind_incident_meldung(
            projekt_name, parsed, typ='24h', text=summary,
            default_titel='24h-Frühwarnung')
        applied = True
...
def nis2_incident_72h_parse(projekt_name: str):
    ...
    if _should_apply_nis2(body) and parsed.get('ersteinschaetzung'):
        ...
        # #1194: an konkreten Vorfall binden (72h-Stufe → Basis der 1M-Frist).
        incident_pk = _bind_incident_meldung(
            projekt_name, parsed, typ='72h', text=summary,
            default_titel='72h-Vorfallmeldung')
        applied = True
...
def nis2_incident_final_parse(projekt_name: str):
    ...
    if _should_apply_nis2(body) and parsed.get('report_text'):
        ...
        # #1194: an konkreten Vorfall binden (1M-Abschluss-Stufe).
        incident_pk = _bind_incident_meldung(
            projekt_name, parsed, typ='1M', text=parsed.get('report_text', ''),
            default_titel='1-Monats-Abschlussbericht')
        applied = True
...
# ════════════════════════════════════════════════════════════════════
# Sprint #21 — N1–N5 Repo-Scan Auto-Fill (#1072/#1074/#1075/#1076)
# + N1 Asset-Inventar-Wizard, N2 Read-only-Prep (#1073)
# Spiegelt das AI-Act A1/A2-Suggest-Muster (server/api/aiact.py).
# Token-aware (#1064): Token aus Projekt-VCS-Konfig via vcs_token().
# ════════════════════════════════════════════════════════════════════

def _nis2_resolve_repo(p: Dict[str, Any], data: Dict[str, Any]) -> tuple[str, str]:
    """(repo, branch) aus Projekt-VCS-Konfig; Request-`repo`/`branch` als Override."""
    vcs = _project_vcs(p)
    repo = resolve_repo(vcs, data.get('repo'))
...
# ─── N1: Asset-Inventar — Repo-Scan-Vorschläge (#1072) ──────────────

@nis2_bp.post('/projekte/<projekt_name>/assets/suggest')
@jwt_required()
def assets_suggest(projekt_name: str):
    """Asset-Vorschläge aus Infrastruktur-Manifesten + GitHub-Topics.

    Body: {repo?, branch?}. Ohne repo wird das gespeicherte vcs_publish.repo
    genutzt. Schreibt nichts — Anwender übernimmt Vorschläge manuell."""
    p, err = _require_nis2_projekt(projekt_name)
    if err:
        return err
    data = request.get_json(silent=True) or {}
    repo, branch = _nis2_resolve_repo(p, data)
    if not repo:
        return jsonify({'error': 'Kein Repo verknüpft. Bitte Repo-Konfiguration speichern.'}), 400
    from nis2 import repo_autofill as ra
    try:
        token = vcs_token(_project_vcs(p))  # #1064: HTTP-API-Token für private Repos
        items = ra.suggest_assets(repo, branch, token=token)
    except Exception as e:
        current_app.logger.warning('nis2 assets suggest failed (%s): %s', repo, e)
        return jsonify({'error': 'Vorschläge konnten nicht erzeugt werden'}), 502
    return jsonify({'suggestions': ra.assets_to_dicts(items), 'count': len(items), 'repo': repo})


# ─── N1: Asset-Inventar — manueller Wizard (#1072) ──────────────────

@nis2_bp.get('/projekte/<projekt_name>/wizards/asset-inventory/prompt')
@jwt_required()
def nis2_asset_wizard_prompt(projekt_name: str):
    p, err = _require_nis2_projekt(projekt_name)
    if err:
        return err
    from nis2.ai_wizards import build_asset_inventory_prompt
    return jsonify({'prompt': build_asset_inventory_prompt(p)})


@nis2_bp.post('/projekte/<projekt_name>/wizards/asset-inventory/parse')
@jwt_required()
def nis2_asset_wizard_parse(projekt_name: str):
    p, err = _require_nis2_projekt(projekt_name)
    if err:
        return err
    from nis2.ai_wizards import parse_asset_inventory_response
    body = request.get_json(silent=True) or {}
    parsed = parse_asset_inventory_response(body.get('response', ''))
    assets = parsed.get('assets', [])
    applied = 0
    if _should_apply_nis2(body) and assets:
        for a in assets:
            try:
                db_save_asset(DB_PATH, projekt_name, a)
                applied += 1
            except Exception as e:  # noqa: BLE001 — einzelnes Asset überspringen
                current_app.logger.warning('asset wizard save failed: %s', e)
        current_app.logger.info('wizard.applied kind=nis2-asset-inventory project=%r count=%d',
                                projekt_name, applied)
    return jsonify({**parsed, 'applied': applied})


# ─── N3: Incident-Response — CSIRT-Defaults aus SECURITY.md (#1074) ──

@nis2_bp.post('/projekte/<projekt_name>/incident-response/suggest')
@jwt_required()
def ir_suggest(projekt_name: str):
    """CSIRT-Kontakt-Defaults aus SECURITY.md. Body: {repo?, branch?}."""
    p, err = _require_nis2_projekt(projekt_name)
    if err:
        return err
    data = request.get_json(silent=True) or {}
    ...
    from nis2 import repo_autofill as ra
    try:
        token = vcs_token(_project_vcs(p))  # #1064
        suggestion = ra.suggest_incident_response(repo, branch, token=token)
    except Exception as e:
        current_app.logger.warning('nis2 ir suggest failed (%s): %s', repo, e)
        return jsonify({'error': 'Vorschläge konnten nicht erzeugt werden'}), 502
    return jsonify({'suggestion': suggestion, 'count': len(suggestion), 'repo': repo})


# ─── N4: Supply-Chain — Vendors aus SBOM/Manifest (#1075) ───────────

@nis2_bp.post('/projekte/<projekt_name>/vendors/suggest')
@jwt_required()
def vendors_suggest(projekt_name: str):
    """Vendor-Vorschläge aus SBOM + package.json/requirements.txt.
    Body: {repo?, branch?}."""
    p, err = _require_nis2_projekt(projekt_name)
    if err:
        return err
    ...
    try:
        token = vcs_token(_project_vcs(p))  # #1064
        items = ra.suggest_vendors(repo, branch, token=token)
    except Exception as e:
        current_app.logger.warning('nis2 vendors suggest failed (%s): %s', repo, e)
        return jsonify({'error': 'Vorschläge konnten nicht erzeugt werden'}), 502
    return jsonify({'suggestions': ra.vendors_to_dicts(items), 'count': len(items), 'repo': repo})


# ─── N5: BCP — Backup-Hinweise aus docker-compose (#1076) ───────────

@nis2_bp.post('/projekte/<projekt_name>/bcp/suggest')
@jwt_required()
def bcp_suggest(projekt_name: str):
    """Backup-Hinweise aus docker-compose (Volumes/Datenbank-Services).
    Body: {repo?, branch?}."""
    p, err = _require_nis2_projekt(projekt_name)
    if err:
        return err
    ...
    try:
        token = vcs_token(_project_vcs(p))  # #1064
        suggestion = ra.suggest_bcp(repo, branch, token=token)
    except Exception as e:
        current_app.logger.warning('nis2 bcp suggest failed (%s): %s', repo, e)
        return jsonify({'error': 'Vorschläge konnten nicht erzeugt werden'}), 502
    return jsonify({'suggestion': suggestion, 'count': len(suggestion), 'repo': repo})
