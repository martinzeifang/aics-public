...
def build_login_response(user: dict) -> dict:
    ...
    # #1185: Objekt-RBAC — Firmen-Whitelist. Admins (admin-Rolle) sind nie eingeschränkt
    # (None = alle Firmen); für andere wird die User-Whitelist übernommen.
    is_admin = 'admin' in (user.get('roles') or [])
    allowed_firmen = None if is_admin else user.get('allowed_firmen')
    try:
    ...
    access_token = create_access_token(
        ...
        additional_claims={
            ...
            'allowed_firmen': allowed_firmen,  # #1185 Objekt-RBAC (None = alle)
...
def login():
    ...
    if ldap_auth:
        ...
        if ldap_user_info:
            ...
            if not roles:
                # #1184: fail-closed — KEIN impliziter Modulzugriff ohne Gruppen-Mapping.
                # Ein expliziter Default-Rollen-Opt-in (Risikoakzeptanz des Admins) ist
                # nur über LDAP_DEFAULT_ROLE möglich; ohne den wird der Login verweigert.
                from shared.redaction import mask_email  # lokal (Import unten erst später)
                default_role = (os.getenv('LDAP_DEFAULT_ROLE') or '').strip()
                if default_role:
                    roles = [default_role]
                    _audit_auth('auth.ldap.default_role', outcome='success',
                                ip=client_ip, email=mask_email(email),
                                groups=ldap_user_info.get('groups', []), role=default_role)
                else:
                    _audit_auth('auth.ldap.no_role_mapping', outcome='fail',
                                ip=client_ip, email=mask_email(email),
                                groups=ldap_user_info.get('groups', []))
                    return {'error': 'LDAP-Login erfolgreich, aber keine Rolle zugeordnet. '
                            'Bitte Gruppen-Mapping (LDAP_GROUP_MAPPING) durch einen '
                            'Administrator konfigurieren.'}, 403
