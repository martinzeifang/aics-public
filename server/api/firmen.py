...
ALLOWED_MODULES = {'risikobewertung', 'gutachten', 'cra', 'dsgvo', 'nis2', 'ai_act', 'wiba'}
ALLOWED_RB_FRAMEWORKS = {'STRIDE', 'Finanzinstitute', 'HEAVENS', 'OCTAVE', 'TARA'}
ALLOWED_GUTACHTEN_FRAMEWORKS = {'NIS2', 'CRA', 'ISO27001', 'DSGVO', 'AI_ACT', 'BSI'}
...
def _serialize_firma(firma: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        'id': firma.get('name'),
        # #1185: numerische PK (firmen.id) zusätzlich exponieren — Objekt-RBAC/firmen_link
        # keyen auf diese; das Legacy-`id`-Feld bleibt der Name (URL-Adressierung).
        'firmen_id': firma.get('id'),
        'name': firma.get('name', ''),
        ...
        'modules': {
            ...
            'wiba': bool(firma.get('module_wiba', 1)),
        },
...
_WIBA_DB = Path('data/db/wiba.sqlite')
from wiba.db import (
    save_projekt as wiba_save_projekt,
    load_projekt as wiba_load_projekt,
)
...
def _ensure_module_projects(firma: Dict[str, Any]) -> List[str]:
    ...
    if bool(firma.get('module_wiba', 1)):
        _safe_save(
            'wiba',
            lambda: wiba_load_projekt(_WIBA_DB, name),
            lambda: wiba_save_projekt(
                _WIBA_DB, name=name, unternehmen=unternehmen,
                beschreibung=beschreibung, berater=berater,
            ),
        )
...
def list_all_firmen():
    ...
    try:
        ...
        # #1185: Objekt-RBAC — Liste auf die für den Nutzer freigegebenen Firmen filtern.
        try:
            from flask_jwt_extended import get_jwt
            from server.middleware.firma_acl import filter_allowed_firmen_ids
            claims = get_jwt()
            if isinstance(claims.get('allowed_firmen'), list):
                allowed = set(filter_allowed_firmen_ids(
                    claims, [f.get('firmen_id') for f in out if f.get('firmen_id') is not None]))
                out = [f for f in out if f.get('firmen_id') in allowed]
        except Exception:
            pass
        return out, 200
...
def add_evidence_url(firma_name: str):
    ...
    try:
        ...
        # #1025: Inhalt SSRF-sicher abrufen, dann mit korrekter Signatur speichern
        # (title + text Pflicht; store_dir/max_pages gibt es nicht → war 500-Bug).
        from evidence.web_fetch import fetch_page, WebFetchError
        try:
            res = fetch_page(url)
        except WebFetchError as e:
            current_app.logger.warning('firmen evidence url fetch failed (%s): %s', url, e)
            return {'error': f'URL konnte nicht geladen werden: {e}'}, 400
