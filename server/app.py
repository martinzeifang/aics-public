...
def create_app(config_file: str | None = None) -> Flask:
    ...
    def _enforce_read_only():
        ...
        try:
            ...
            LIC_MODULES = {'cra', 'nis2', 'aiact', 'dsgvo', 'risikobewertung', 'gutachten', 'soc'}
...
def _configure_swagger(app: Flask):
    ...
    swagger_template = {
        ...
        'info': {
            ...
            'description': 'REST API für Multi-Modul-Compliance-Verwaltung (CRA, NIS2, AI-Act, Risikobewertung, Firmen).',
            'version': '1.0.0',
            'contact': {
                'name': 'Martin Zeifang',
...
def _configure_security(app: Flask):
    ...
    _guard_at_rest_encryption(app)


def _guard_at_rest_encryption(app: Flask):
    """#1189: At-Rest-Verschlüsselung erfordert einen SEPARATEN Schlüssel.

    Ist Verschlüsselung aktiv (security.at_rest_encryption.enabled/encrypt_evidence/
    encrypt_backups), aber nur ``JWT_SECRET_KEY`` ohne separaten ``AICS_AT_REST_KEY``
    vorhanden, koppelt der Fallback Signatur- und Verschlüsselungsschlüssel
    (Datenschutz-Risiko). Produktivstart → fail-closed (RuntimeError); Dev → Warnung.
    """
    try:
        from ai_compliance_suite.config import load_config
        sec = ((load_config() or {}).get('security', {}) or {}).get('at_rest_encryption', {}) or {}
    except Exception:
        sec = {}
    # Nur der Master-Flag entscheidet, ob Verschlüsselung aktiv ist — encrypt_backups/
    # encrypt_evidence sind im Verschlüsselungs-Code ohnehin durch `enabled` gegated
    # (evidence/db.py). Sonst würde der Default `encrypt_backups: true` (bei enabled:false)
    # den Prod-Start ohne Key fälschlich blockieren.
    enabled = bool(sec.get('enabled'))
    if not enabled or (os.getenv('AICS_AT_REST_KEY') or '').strip():
        return
    msg = ('At-Rest-Verschlüsselung ist aktiv, aber kein separater AICS_AT_REST_KEY gesetzt — '
           'der Fallback auf JWT_SECRET_KEY koppelt Signatur- und Verschlüsselungsschlüssel '
           '(Datenschutz-Risiko, #1189). Bitte AICS_AT_REST_KEY setzen.')
    is_prod = os.getenv('FLASK_ENV', '').lower() not in ('development', 'testing')
    if is_prod:
        raise RuntimeError(msg + ' Produktivstart abgebrochen (fail-closed).')
    app.logger.warning('⚠ %s', msg)
...
def _register_blueprints(app: Flask):
    ...
    # Benutzerhandbuch-PDF (#1473) — aus dem Daten-Volume über den Hilfe-Dialog
    from server.api.handbook import handbook_bp
    app.register_blueprint(handbook_bp, url_prefix='/api/handbook')
