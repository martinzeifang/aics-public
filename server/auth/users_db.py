...
def ensure_db(db_path: Path = DEFAULT_DB_PATH) -> None:
    ...
    cur.execute("""
        CREATE TABLE IF NOT EXISTS users (
            ...
            allowed_firmen_json TEXT DEFAULT 'null',
    ...
    # #1185: Objekt-RBAC — Firmen-Whitelist (NULL = alle Firmen, [] = keine, [ids] = nur diese)
    _add_column_if_missing(cur, 'users', 'allowed_firmen_json', "allowed_firmen_json TEXT DEFAULT 'null'")
...
def _create_demo_users(con: Any) -> None:
    ...
    demo_users = [
        ...
        # #1411 (SOC-Portal): Demo-SOC-Operator für das SOC-Operations-Portal.
        ('user-soc', 'soc@example.com', 'soc-password', ['soc_operator']),
    ]
...
def _row_to_user(row: dict | Any) -> dict[str, Any]:
    ...
    try:  # #1185: Firmen-Whitelist (Liste von firmen_id oder None = alle)
        af = json.loads(user.get('allowed_firmen_json') or 'null')
        user['allowed_firmen'] = [int(x) for x in af] if isinstance(af, list) else None
    except Exception:
        user['allowed_firmen'] = None
    try:
...
def create_user(
    *,
    ...
    allowed_firmen: list[int] | None = None,
    ...
    con.execute(
        '''INSERT INTO users (id, email, password_hash, roles_json,
                              allowed_modules_json, allowed_firmen_json,
                              extra_permissions_json, display_name)
           VALUES (?, ?, ?, ?, ?, ?, ?, ?)''',
        (uid, email, generate_password_hash(password),
         json.dumps(roles),
         json.dumps(allowed_modules) if allowed_modules is not None else 'null',
         json.dumps(allowed_firmen) if allowed_firmen is not None else 'null',
...
def update_user(
    ...
    *,
    ...
    allowed_firmen: list[int] | None | object = ...,
    extra_permissions: list[str] | None = None,
    ...
    if allowed_firmen is not ...:  # #1185
        fields.append('allowed_firmen_json=?')
        params.append(json.dumps(allowed_firmen) if allowed_firmen is not None else 'null')
