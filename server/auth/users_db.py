...
def _connect(db_path: Path = DEFAULT_DB_PATH) -> sqlite3.Connection:
    ...
    # busy_timeout MUSS als erstes gesetzt werden: das Umschalten auf WAL nimmt
    # auf einer frischen DB einen Write-Lock. Mehrere gunicorn-Worker rufen
    # ensure_db() beim Boot gleichzeitig auf — ohne vorher aktiven busy_timeout
    # scheitert der WAL-Switch des zweiten Workers sofort mit "database is
    # locked" (#837). 30s warten statt crashen.
    con.execute('PRAGMA busy_timeout=30000')
    # WAL persistiert im DB-Header; sobald irgendein Worker umgeschaltet hat,
    # bleibt es bestehen. Den Switch lock-tolerant machen, falls ein paralleler
    # Worker ihn gerade hält.
    try:
...
def _add_column_if_missing(cur: sqlite3.Cursor, table: str, column: str, ddl: str) -> None:
    """Fügt eine Spalte idempotent + race-tolerant hinzu.

    Mehrere gunicorn-Worker booten parallel und rufen ensure_db() gleichzeitig
    auf. Die klassische Prüfung "PRAGMA table_info → fehlt? → ALTER TABLE" ist
    eine TOCTOU-Race: beide Worker sehen die Spalte fehlen, beide führen das
    ALTER aus, der zweite crasht mit "duplicate column name" → Worker failed to
    boot → Container exit 3 (nicht-deterministischer Deploy-Fehler, #837).
    Lösung: das erwartete "duplicate column" abfangen und als Erfolg werten.
    """
    ...
    try:
        ...
        if 'duplicate column name' in str(exc).lower():
            return  # anderer Worker war schneller — gewünschter Endzustand
        raise
...
def _create_initial_admin(con: sqlite3.Connection, db_path: Path = DEFAULT_DB_PATH) -> None:
...
def _row_to_credential(row: sqlite3.Row) -> dict[str, Any]:
    return {
        'id': row['id'],
        'user_id': row['user_id'],
...
def update_webauthn_sign_count(
    ...
    con = _connect(db_path)
    con.execute(
        'UPDATE webauthn_credentials SET sign_count=?, last_used_at=CURRENT_TIMESTAMP '
        'WHERE credential_id=?',
...
def set_mfa_grace_until(user_id: str, until_ts: int, db_path: Path = DEFAULT_DB_PATH) -> None:
    ...
    con = _connect(db_path)
    con.execute(
        'UPDATE users SET mfa_grace_until=?, updated_at=CURRENT_TIMESTAMP WHERE id=?',
