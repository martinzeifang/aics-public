...
def test_new_columns_present(db_path):
    ...
    try:
        cols = _columns(con, "dsgvo_betroffenenrechte")
    finally:
        con.close()
    assert {"empfaenger_status", "empfaenger_liste", "empfaenger_datum"} <= cols


def test_migration_adds_columns_to_legacy_table(db_path, pg):
    # Lege eine ALTE Tabelle ohne Art.-19-Spalten an.
    pg.drop_schema(db_path)
    con = _sdb.connect(db_path)
    try:
        con.execute(
            "CREATE TABLE dsgvo_betroffenenrechte ("
            "id BIGINT GENERATED ALWAYS AS IDENTITY PRIMARY KEY, projekt_name TEXT NOT NULL, "
