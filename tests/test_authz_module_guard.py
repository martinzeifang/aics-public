...
class TestModuleAuthz:
    ...
    def test_firmen_viewer_write_blocked(self, client, app):
        # 'viewer' hat firmen:read aber nicht firmen:write
        r = client.post('/api/firmen', headers=_h(app, ['viewer']), json={'name': 'x'})
        assert r.status_code == 403
