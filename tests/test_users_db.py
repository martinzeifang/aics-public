...
class TestDemoUsersIdempotent:
    """ENABLE_DEMO_USERS stellt Testuser idempotent sicher — auch bei nicht-leerer DB."""

    def test_demo_users_ensured_on_nonempty_db(self, tmp_path, monkeypatch):
        from server.auth.users_db import ensure_db, list_users, get_user_by_email, verify_password
        db = tmp_path / 'users.sqlite'
        # 1) ohne Flag, leere DB → Initial-Admin, KEIN Demo
        monkeypatch.setenv('ENABLE_DEMO_USERS', 'false')
        ensure_db(db)
        emails = {u['email'] for u in list_users(db_path=db)}
        assert 'admin@example.com' not in emails
        # 2) Flag an, DB NICHT leer → Demo-User werden ergänzt
        monkeypatch.setenv('ENABLE_DEMO_USERS', 'true')
        ensure_db(db)
        ensure_db(db)  # idempotent — kein IntegrityError
        emails = {u['email'] for u in list_users(db_path=db)}
        assert {'admin@example.com', 'editor@example.com'} <= emails
        admin = get_user_by_email('admin@example.com', db_path=db)
