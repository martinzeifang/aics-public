...
Abgedeckt:
  ...
  (c) Fake-MIME-Upload an einen echten Endpunkt (firmen Evidence) wird abgelehnt;
...
def _create_firma(client, auth_headers) -> str:
    name = f'wp10-{uuid.uuid4().hex[:8]}'
    resp = client.post('/api/firmen', json={'name': name}, headers=auth_headers)
...
def test_evidence_upload_rejects_fake_pdf(client, auth_headers):
    """firmen ist NICHT lizenzgated → kein 423. Fake-PDF muss 400 liefern."""
    name = _create_firma(client, auth_headers)
    data = {
        'file': (io.BytesIO(b'this is definitely not a pdf'), 'evil.pdf'),
    }
    resp = client.post(
        f'/api/firmen/{name}/evidence/file',
        data=data,
