...
def test_legacy_kunden_db_migration(fresh_repo_db):
    ...
    new = fresh_repo_db('firmenmig')
    legacy = new.with_name('kunden.sqlite_' + new.stem)  # eindeutige Legacy-Datei
    # realistisches Alt-Schema durch Rückbenennung erzeugen
    fdb.ensure_db(new)
    con = sqlite3.connect(new)
    con.execute("INSERT INTO firmen (id,name,produkt) VALUES (1,'Acme GmbH','')")
    con.execute("INSERT INTO produkte (id,firmen_id,name) VALUES (1,1,'Produkt-A')")
    con.commit()
    con.execute("ALTER TABLE produkte RENAME COLUMN firmen_id TO kunden_id")
    con.execute("ALTER TABLE firmen RENAME TO kunden")
    con.commit(); con.close()
    # Datei in den von der Migration erwarteten Legacy-Namen umbenennen
    for s in ('', '-wal', '-shm'):
        p = new.with_name(new.name + s)
        if p.exists():
            p.rename(new.with_name('kunden.sqlite' + s))
    try:
        fdb.ensure_db(new)  # echte Migration
        con = sqlite3.connect(new)
        tables = {r[0] for r in con.execute("SELECT name FROM sqlite_master WHERE type='table'")}
        assert 'firmen' in tables and 'kunden' not in tables
        cols = {r[1] for r in con.execute("PRAGMA table_info(produkte)")}
        assert 'firmen_id' in cols and 'kunden_id' not in cols
        assert con.execute("SELECT name FROM firmen WHERE id=1").fetchone()[0] == 'Acme GmbH'
        assert con.execute("SELECT name FROM produkte WHERE firmen_id=1").fetchone()[0] == 'Produkt-A'
        con.close()
        fdb.ensure_db(new)  # idempotent
    finally:
        for s in ('', '-wal', '-shm'):
            for base in (new, new.with_name('kunden.sqlite')):
                p = base.with_name(base.name + s)
                try:
                    p.unlink()
                except FileNotFoundError:
                    pass
...
def test_legacy_evidence_kunden_id_migration(fresh_repo_db):
    ...
    edb.ensure_db(ev)
    con = sqlite3.connect(ev)
    cols = {r[1] for r in con.execute("PRAGMA table_info(evidence_documents)")}
    assert 'firmen_id' in cols and 'kunden_id' not in cols
