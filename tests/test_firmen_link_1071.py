...
def test_ensure_column_idempotent(dbs):
    ...
    cols = [r[0] for r in con.execute(
        ...
        "WHERE table_schema=current_schema() AND table_name='rb_projekte'")]
    con.close()
...
def test_backfill_matches_and_reports_unmatched(dbs):
    ...
    con = _sdb.connect(module)
    rows = {r[0]: r[1] for r in con.execute("SELECT name, firmen_id FROM rb_projekte").fetchall()}
    con.close()
