...
def _alltext(doc):
    """Volltext inkl. Tabellenzellen (Glossar ist seit #1001 eine Tabelle)."""
...
def test_standard_export_renders_glossar(db):
    ...
    doc = gen.build_gerichtsgutachten_docx('GE', db)
    txt = _alltext(doc)
    assert 'Glossar' in txt and 'ISO/IEC 27037' in txt
    # #1001: Glossar als Tabelle unter Anhang → Glossar
    heads = [(p.style.name, p.text) for p in doc.paragraphs]
    assert ('Heading 2', 'Glossar') in heads
    assert any(s == 'Heading 1' and 'Anhang' in t for s, t in heads)
    # toggle off → keine Glossar-Überschrift (ISO-Referenz steht zwar im Beurteilungstext)
    off_doc = gen.build_gerichtsgutachten_docx('GE', db, export_options={'include_glossar': False})
    off_heads = [(p.style.name, p.text) for p in off_doc.paragraphs]
    assert ('Heading 2', 'Glossar') not in off_heads
...
def test_glossar_auto_appended_when_no_placeholder(db):
    """#998: Vorlage ohne [glossar]/[gutachten_volltext] → Glossar trotzdem am Ende."""
    _seed_projekt(db, 'GP')
    gdb.generate_glossar(db, 'GP')
    from gutachten.template_render import build_template_context, render_with_bracket_mapping, suggest_mapping, detect_bracket_placeholders
    ctx = build_template_context(db, 'GP')
    src = Path('data/gutachten/_gp_in.docx'); d = Document()
    d.add_paragraph('AZ: [projekt.aktenzeichen]'); d.add_paragraph('Anhang'); d.save(str(src))
    out = Path('data/gutachten/_gp_out.docx')
    render_with_bracket_mapping(src, suggest_mapping(detect_bracket_placeholders(src)), ctx, out)
    txt = _alltext(Document(str(out)))
    src.unlink(missing_ok=True); out.unlink(missing_ok=True)
    assert 'Glossar' in txt and 'ISO/IEC 27037' in txt


def test_glossar_no_double_with_placeholder(db):
    _seed_projekt(db, 'GP2')
    gdb.generate_glossar(db, 'GP2')
    from gutachten.template_render import build_template_context, render_with_bracket_mapping
    ctx = build_template_context(db, 'GP2')
    src = Path('data/gutachten/_gp2_in.docx'); d = Document(); d.add_paragraph('[glossar]'); d.save(str(src))
    out = Path('data/gutachten/_gp2_out.docx')
    render_with_bracket_mapping(src, {'[glossar]': 'glossar'}, ctx, out)
    txt = _alltext(Document(str(out)))
    src.unlink(missing_ok=True); out.unlink(missing_ok=True)
    assert txt.count('ISO/IEC 27037') == 1   # kein Duplikat


def test_glossar_toggle_off_in_template(db):
    _seed_projekt(db, 'GP3')
    gdb.generate_glossar(db, 'GP3')
    from gutachten.template_render import build_template_context, render_with_bracket_mapping, suggest_mapping, detect_bracket_placeholders
    ctx = build_template_context(db, 'GP3'); ctx['include_glossar'] = False
    src = Path('data/gutachten/_gp3_in.docx'); d = Document(); d.add_paragraph('AZ: [projekt.aktenzeichen]'); d.save(str(src))
    out = Path('data/gutachten/_gp3_out.docx')
    render_with_bracket_mapping(src, suggest_mapping(detect_bracket_placeholders(src)), ctx, out)
    txt = _alltext(Document(str(out)))
    src.unlink(missing_ok=True); out.unlink(missing_ok=True)
    assert 'ISO/IEC 27037' not in txt
