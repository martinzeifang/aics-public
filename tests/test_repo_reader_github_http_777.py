...
def _fake_http(path, token=None):
...
def test_explicit_token_used_in_header(monkeypatch):
    captured = {}
    ...
    monkeypatch.delenv('GH_TOKEN', raising=False)
    monkeypatch.setattr(rr.urllib.request, 'urlopen', _fake_urlopen)
    rr._github_http_api('repos/o/r', token='explicit-tok')
    assert captured['auth'] == 'Bearer explicit-tok'


def test_rate_limit_403_without_token_gives_clear_error(monkeypatch):
    import urllib.error
    ...
    with pytest.raises(RuntimeError) as ei:
        rr._github_http_api('repos/o/r')
    assert 'Token' in str(ei.value) or 'Limit' in str(ei.value)


def test_token_prefers_http_over_gh(monkeypatch):
    # Mit Token: gh-CLI darf NICHT versucht werden (deterministische HTTP-API).
    import shutil
    monkeypatch.setattr(shutil, 'which', lambda _n: '/usr/bin/gh')
    def _boom(_p):
        raise AssertionError('gh darf mit Token nicht aufgerufen werden')
    monkeypatch.setattr(rr, '_gh_api', _boom)
    monkeypatch.setattr(rr, '_github_http_api', _fake_http)
    ctx = rr.fetch_repo_context('owner/repo', token='tok')
    assert ctx.description == 'Mein Tool'
