...
# ════════════════════════════════════════════════════════════════════
# #1116 — RB-Projektname URL-sicher (kein '/') + Legacy-Migration
# ════════════════════════════════════════════════════════════════════

def test_dsfa_rb_name_ist_url_sicher_ohne_slash():
    """Der verknüpfte RB-Name darf kein '/' enthalten — sonst 404 in den
    Risikobewertungs-Routen (string-Converter matcht keine Slashes)."""
    nm = _dsfa_rb_name("Acme / Tochter", "DSFA-001")
    assert "/" not in nm
    assert "DSFA-001" in nm


def test_rename_projekt_migriert_projekt_und_risiken():
    from risikobewertung.db import (
        save_projekt as rb_save_projekt, rename_projekt,
        load_projekt as rb_load2, load_risiken, delete_projekt as rb_del2,
    )
    old, new = "ZZ-1116-old", "ZZ-1116-new"
    rb_del2(_RB_DB_PATH, old); rb_del2(_RB_DB_PATH, new)
    rb_save_projekt(_RB_DB_PATH, name=old, framework="DSGVO-DSFA", beschreibung="x")
    rb_save_risiko(_RB_DB_PATH, {"projekt_name": old, "risk_name": "R1",
                                 "framework": "DSGVO-DSFA", "felder": {}})
    try:
        assert rename_projekt(_RB_DB_PATH, old, new) is True
        assert rb_load2(_RB_DB_PATH, old) is None
        assert rb_load2(_RB_DB_PATH, new) is not None
        assert len(load_risiken(_RB_DB_PATH, new)) == 1
        # Ziel existiert bereits → kein Clobber
        rb_save_projekt(_RB_DB_PATH, name=old, framework="DSGVO-DSFA", beschreibung="y")
        with pytest.raises(ValueError):
            rename_projekt(_RB_DB_PATH, old, new)
    finally:
        rb_del2(_RB_DB_PATH, old); rb_del2(_RB_DB_PATH, new)


def test_risk_link_migriert_legacy_slash_namen(client, auth_headers, projekt):
    """Alt-Verknüpfung mit '/' wird beim risk-link auf den slashfreien Namen
    umgezogen — inkl. Erhalt der im RB gepflegten Risiken."""
    from server.api.dsgvo import _legacy_dsfa_rb_name
    from risikobewertung.db import (
        save_projekt as rb_save_projekt, load_projekt as rb_load2,
        delete_projekt as rb_del2,
    )
    rid = save_dpia(DB_PATH, projekt, {"dpia_id": "DSFA-LEG", "titel": "Legacy"})
    legacy = _legacy_dsfa_rb_name(projekt, "DSFA-LEG")
    canonical = _dsfa_rb_name(projekt, "DSFA-LEG")
    assert "/" in legacy and "/" not in canonical
    rb_del2(_RB_DB_PATH, legacy); rb_del2(_RB_DB_PATH, canonical)
    rb_save_projekt(_RB_DB_PATH, name=legacy, framework="DSGVO-DSFA", beschreibung="x",
                    meta={"linked_dsgvo_projekt": projekt, "linked_dsgvo_dpia_id": "DSFA-LEG"})
    rb_save_risiko(_RB_DB_PATH, {"projekt_name": legacy, "risk_name": "Altes Risiko",
                                 "framework": "DSGVO-DSFA", "felder": {"massnahme": "M"}})
    set_dpia_rb_projekt(DB_PATH, rid, legacy)
    ...
    data = r.get_json()
    assert data["rb_projekt_id"] == canonical
    assert "/" not in data["rb_projekt_id"]
    assert len(data["risiken"]) == 1                          # Risiken erhalten
    assert get_dpia(DB_PATH, rid)["rb_projekt_id"] == canonical  # persistiert
    assert rb_load2(_RB_DB_PATH, legacy) is None              # Alt-Projekt umbenannt
    rb_del2(_RB_DB_PATH, canonical)
