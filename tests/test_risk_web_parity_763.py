...
class TestRepoConfigEndpoints:
    ...
    def test_token_value_encrypted_and_used(self, client, auth_headers, projekt):
        # #784: eingegebener Token-Wert wird verschlüsselt gespeichert, nie
        # ausgeliefert, aber für den Fetch entschlüsselt verwendet.
        secret = 'dummy-pat-not-a-real-secret'
        r = client.put(f'{BASE}/projekte/{projekt}/repo-config', headers=auth_headers,
                       json={'vcs_publish': {'provider': 'github', 'repo': 'o/r',
                                             'token': secret}})
        assert r.status_code == 200
        body = r.get_json()['vcs_publish']
        assert body.get('has_token') is True
        assert 'token' not in body and 'token_enc' not in body  # nie ausgeliefert

        # GET liefert ebenfalls keinen Token-Wert
        ...
        # Klartext nicht in der DB; _vcs_token entschlüsselt korrekt
        from server.api.risikobewertung import _vcs_token, DB_PATH
        from risikobewertung.db import load_projekt
        ...
        assert _vcs_token(vcs) == secret

    def test_token_preserved_on_partial_update(self, client, auth_headers, projekt):
        client.put(f'{BASE}/projekte/{projekt}/repo-config', headers=auth_headers,
                   json={'vcs_publish': {'provider': 'github', 'repo': 'o/r', 'token': 'dummy-token-keep-me'}})
        # anderes Feld ändern, OHNE token mitzuschicken
        client.put(f'{BASE}/projekte/{projekt}/repo-config', headers=auth_headers,
                   json={'vcs_publish': {'provider': 'github', 'repo': 'o/r2'}})
        from server.api.risikobewertung import _vcs_token, DB_PATH
        from risikobewertung.db import load_projekt
        vcs = ((load_projekt(DB_PATH, projekt) or {}).get('meta') or {}).get('vcs_publish') or {}
        assert _vcs_token(vcs) == 'dummy-token-keep-me'  # Token nicht verloren
