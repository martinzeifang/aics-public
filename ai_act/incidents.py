...
def update_incident(db_path: Path, projekt_name: str, incident_id: int,
    ...
    try:
        ...
        cur = con.execute(
            f"UPDATE aiact_incidents SET {sets}, updated_at=datetime('now') "
...
def attach_report(db_path: Path, projekt_name: str, incident_id: int,
    ...
    try:
        cur = con.execute(
            "UPDATE aiact_incidents SET report_text=?, updated_at=datetime('now') "
            "WHERE id=? AND projekt_name=?",
...
def _row(r: sqlite3.Row) -> dict[str, Any]:  # pragma: no cover - convenience
    return dict(r)
