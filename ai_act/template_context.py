...
AIACT_VARIABLES: list[dict[str, Any]] = [
    ...
    {"key": "dokumente", "typ": "liste", "beschreibung": "Finalisierte/freigegebene gemanagte Dokumente des Projekts (titel, doc_type, rechtsgrundlage, status, version, stand)", "pflicht": False},
]
...
def _dokumente(db_path: Path, projekt_name: str) -> list[dict[str, Any]]:
    ...
    Best-effort: bricht den Bericht nie ab — bei Fehlern leere Liste.
    """
    try:
        ...
        for d in list_documents(db_path, _DOC_MODUL, projekt_name):
            ...
            mode = _s(d.get("doc_mode")) or "inapp"
            out.append({
                ...
                # #1233 additiv: extern → URL statt Volltext; in-app → leere URL.
                "doc_mode": mode,
                "external_url": _s(d.get("external_url")) if mode == "extern" else "",
            })
        return out
    except Exception:
        return []
...
def build_aiact_context(db_path: Path | str, projekt_name: str) -> dict[str, Any]:
    ...
    return {
        ...
        "meta": meta_ctx,
        "dokumente": _dokumente(db_path, name),
    }
