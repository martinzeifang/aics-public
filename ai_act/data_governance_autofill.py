...
def _fetch_first(owner: str, name: str, branch: str,
    ...
    for path in candidates:
        text = github_fetch_text(owner, name, path, branch, token=token)
...
def suggest_data_governance(repo: str, branch: str = "",
    ...
    ``token`` (#1064) für private Repos im Container.
    """
    ...
    dataset = _fetch_first(owner, name, branch, _DATASET_FILES, token)
    modelcard = _fetch_first(owner, name, branch, _MODELCARD_FILES, token)
    bias_doc = _fetch_first(owner, name, branch, _BIAS_FILES, token)
    readme = _fetch_first(owner, name, branch, _README_FILES, token)
    ...
    if "data_collection_method" not in out:
        dvc = _fetch_first(owner, name, branch, _DVC_FILES, token)
        if dvc:
            dvc_path, _ = dvc
