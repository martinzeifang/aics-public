...
def _fetch_first(owner: str, name: str, candidates: tuple[str, ...],
    ...
    for path in candidates:
        content = github_fetch_text(owner, name, path, branch)
...
def suggest_system_doku(repo: str, branch: str = "") -> dict[str, FieldSuggestion]:
    """Vorschläge für ``aiact_system_doku`` aus den Dateien eines GitHub-Repos.

    Gibt nur Felder mit gefundenem Inhalt zurück.
    """
    ...
    readme = _fetch_first(owner, name, _README_CANDIDATES, branch)
    if readme:
        path, content = readme
    ...
    arch = _fetch_first(owner, name, _ARCHITECTURE_CANDIDATES, branch)
    if arch:
        path, content = arch
    ...
    card = _fetch_first(owner, name, _MODEL_CARD_CANDIDATES, branch)
    if card:
        path, content = card
    ...
    sec = _fetch_first(owner, name, _SECURITY_CANDIDATES, branch)
    if sec:
        path, content = sec
        body = _first_paragraph(content, after_heading=True)
        if not body:
