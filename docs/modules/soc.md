...
## SOC-Operations-Portal (eigener Zugang)

Optional lässt sich das SOC als **eigenständiges, gebrandetes Portal** betreiben —
eigener Login, eigene URL/Port, nach der Anmeldung **nur das SOC-Modul** sichtbar.
Gleiche Codebasis/Image, aktiviert über das Runtime-Flag **`AICS_PORTAL=soc`**
(separater Stack, z. B. via `scripts/create_soc_stack.py`).

- **Login:** eigenständige, dunkle „SOC Operation Center"-Seite; Passwort + Passkey
  wie in der Suite. Passkey: `WEBAUTHN_RP_ORIGIN` muss den Portal-Origin enthalten
  (RP-ID = Domain, port-unabhängig).
- **Nach Login:** nur SOC navigierbar (Redirect auf `/soc`); andere Module sind
  ausgeblendet — **auch für Benutzer, die in der Suite Zugriff auf mehr Module haben.**
- **Admin-Schalter:** Das Portal kann in den Einstellungen (`⚙️ → 🛡️ SOC-Operations-Portal`)
  aktiviert/deaktiviert werden. Deaktiviert: Login gesperrt + Hinweis-Seite.
- **Gemeinsame Datenbank:** Im Produktivbetrieb zeigt das Portal auf die Suite-DB
  (gemeinsame Benutzer/Passkeys/Incidents), damit sich jeder mit seinem Suite-Account
  anmelden kann.
- **Kein Zugriff:** Meldet sich ein Benutzer ohne `soc:read` am Portal an, erscheint
  ein freundlicher „Kein Zugriff auf das SOC"-Hinweis.

### Alternative ohne Portal: reine SOC-Anwender per Rechte

Soll ein Benutzer **generell nur SOC** nutzen (überall, nicht nur über eine eigene
URL), genügt die Rechte-Konfiguration — ohne Portal:

- Rolle **`soc_operator`** vergeben (bündelt die SOC-Berechtigungen), und/oder
- in der Benutzerverwaltung **`allowed_modules = ['soc']`** setzen → die Modul-Navigation
  zeigt dann nur SOC.

Unterschied: Rechte wirken **pro Benutzer** (überall gleich); das Portal wirkt **pro
Zugang** (eigene URL, SOC-only auch für Voll-Zugriff-Nutzer). Beides ist kombinierbar.
