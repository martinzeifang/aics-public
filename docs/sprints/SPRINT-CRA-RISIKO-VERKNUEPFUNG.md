...
- `risikobewertung/db.py:227` – `list_projekte_fuer_kunde()`.
...
- `frontend/src/views/risikobewertung/RisikobewertungView.vue:665` – Deep-Link `?projekt=<name>` (Issue #433); `?kunde=`/`?neu=` ebenfalls vorhanden.
...
- `cra/db.py:264` – `list_projekte_fuer_kunde()`.
...
### Gemeinsame Klammer
- Beide Module nutzen `unternehmen` (= `kunden.name`) als Kunden-Schlüssel.
- `server/api/kunden.py:55,73` importiert beide `list_projekte_fuer_kunde` (cra/rb).
...
- **Kundenbasierte Projektliste** in beiden Modulen → Link-Kandidaten gratis.
...
`GET candidates` (gleicher Kunde), `POST/DELETE risk-link` mit beidseitiger Konsistenz,
Kundengrenzen-Validierung, `@jwt_required`, meta-merge zwingend.
