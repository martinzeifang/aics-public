...
Diese Anleitung beschreibt, wie der AICS-Server mit einem lokalen
Ollama-Modell-Server arbeitet — sowohl in Docker als auch in der
lokalen Entwicklung.
...
Der AICS-Server spricht Ollama über die Umgebungsvariable
`OLLAMA_BASE_URL` an (default `http://ollama:11434` in Docker,
`http://localhost:11434` lokal).

## Docker (empfohlen)

`docker-compose.yml` hat ein Profil `ai`, das den Ollama-Container
startet:
...
Beim ersten Start zieht der Init-Container das Default-Modell
...
Default-Modell pullen + Server starten:

```bash
ollama serve &        # Hintergrund-Daemon
...
Bei „Verbindung verweigert": prüfen, ob `OLLAMA_BASE_URL` auf die
korrekte Adresse zeigt (in Docker: **`http://ollama:11434`**, nicht
`localhost`).
