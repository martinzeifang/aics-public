...
const routes: RouteRecordRaw[] = [
  ...
  // Kunden Module
  {
    path: '/kunden',
    name: 'Kunden',
    component: () => import('../views/kunden/KundenView.vue'),
...
router.beforeEach((to, from, next) => {
  ...
  //   - 'kunden' immer erlaubt
  //   - 'gutachten' nur bei expliziter Nennung
  //   - '*'-Wildcard deckt alle anderen Module
  //   - leere Liste = keine Lizenz → alles außer 'kunden' gesperrt
  ...
  if (LICENSED_PATHS.has(moduleSlug)) {
    ...
    if (Array.isArray(licMods)) {  // strict-Modus nur bei explizitem Array
      ...
      if (!allowed) {
        console.warn(`[router] Modul '${moduleSlug}' nicht lizenziert — Redirect zu /kunden`)
        next('/kunden')
        return
      }
    }
