<template>
  ...
  >
    ...
    <template v-if="store.selectedProjektObj">
      <!-- Tab: Dashboard — einheitlich (#1250/#1501) -->
      <div v-if="activeTab === 'dashboard'" class="tab-content">
        <ModuleDashboard
          :gesamt="{ percent: dashboardGesamtPct, ampel: wibaAmpel }"
          ...
          @open-bereich="activeTab = 'prueffragen'"
          @open-luecken="activeTab = 'prueffragen'"
          @open-punkt="activeTab = 'prueffragen'"
      ...
      <div v-if="activeTab === 'berichte'" class="tab-content">
        <!-- Sprint #35: einheitliches Berichts-Center -->
        <ReportCenterPanel
          v-if="store.selectedProjekt"
          :api-base="`/wiba/projekte/${encodeURIComponent(store.selectedProjekt)}`"
          title="WiBA-Berichts-Center"
        />
      </div>
    </template>
  </ModuleShell>
...
// Einheitliches Dashboard (#1250/#1501) — WiBA-Reifegrad auf die geteilte
// ModuleDashboard-Komponente gemappt (Reifegrad je Thema → Bereiche,
// „Nein"/„Offen"-Prüffragen → offene Punkte mit Score 0).
const { dokFertig, dokGesamt, risiko, risikoLoading, loadAll: loadDashboardExtras } =
  useModuleDashboard('wiba')

const dashboardGesamtPct = computed(() => Number(store.reifegrad?.gesamt_pct ?? 0))
const wibaAmpel = computed(() => {
  const p = dashboardGesamtPct.value
  return p >= 80 ? 'gruen' : p >= 50 ? 'orange' : 'rot'
})
const dashboardStats = computed(() => {
  const r = store.reifegrad
  const t = { ja: 0, nein: 0, offen: 0, nicht_relevant: 0 }
  for (const v of Object.values(r?.themen || {}) as any[]) {
    t.ja += v.ja ?? 0; t.nein += v.nein ?? 0; t.offen += v.offen ?? 0; t.nicht_relevant += v.nicht_relevant ?? 0
  }
  return [
    `${r?.bewertet ?? 0} / ${r?.in_scope ?? 0} Prüffragen bewertet`,
    `✅ ${t.ja}  ❌ ${t.nein}  ⏳ ${t.offen}  ➖ ${t.nicht_relevant}`,
  ]
})
const dashboardBereiche = computed(() =>
  Object.entries(store.reifegrad?.themen || {}).map(([key, v]: [string, any]) => ({
    id: key,
    title: v.titel || key,
    percent: Number(v.pct ?? 0),
    bewertet: (v.ja ?? 0) + (v.nein ?? 0) + (v.nicht_relevant ?? 0),
    gesamt: v.total ?? 0,
  })),
)
const dashboardLuecken = computed(() => {
  const out: { id: string; titel: string; bewertung: number }[] = []
  for (const th of store.themen) {
    for (const pf of th.prueffragen || []) {
      if (pf.status === 'nein' || pf.status === 'offen' || !pf.status)
        out.push({ id: pf.control_id, titel: `${pf.nr} ${pf.frage}`, bewertung: 0 })
    }
  }
  return out
})
...
// Gruppierte Navigation (Sprint #34, #1375) — gemeinsame Taxonomie
const navGroups = [
