<template>
  <div class="soc-view">
    ...
    <section v-show="tab === 'incidents'" class="panel">
      ...
      <div v-else class="inc-detail">
        ...
        <div v-if="editMode" class="edit-box">
          ...
          <table v-if="(store.currentIncident.pir_actions || []).length" class="grid mini">
          ...
          <table v-if="evidence.length" class="grid mini">
    ...
    <section v-show="tab === 'massnahmen'" class="panel">
      ...
      <table v-if="store.pirActions.length" class="grid">
    ...
    <section v-show="tab === 'betrieb'" class="panel">
      ...
      <table v-if="store.handovers.length" class="grid mini">
      ...
      <table v-if="store.raci.length" class="grid mini">
    ...
    <section v-show="tab === 'detektion'" class="panel">
      ...
      <table v-if="store.usecases.length" class="grid mini">
      ...
      <table v-if="store.rules.length" class="grid mini">
    ...
    <section v-show="tab === 'threatintel'" class="panel">
      ...
      <table v-if="store.iocs.length" class="grid mini">
      ...
      <table v-if="store.iocAlerts.length" class="grid mini">
    ...
    <section v-show="tab === 'logquellen'" class="panel">
      ...
      <table v-if="store.logHealth && store.logHealth.sources.length" class="grid mini">
      ...
      <table v-if="store.logHealth && store.logHealth.register.length" class="grid mini">
    ...
    <section v-show="tab === 'hunting'" class="panel">
      ...
      <details class="supp-box" open>
        ...
        <table v-if="huntResult && huntResult.ok && huntResult.hits.length" class="grid mini">
    ...
    <section v-show="tab === 'berichte'" class="panel">
      ...
      <table class="grid">
        ...
        <tbody>
          ...
          <tr v-if="!store.berichtRuns.length"><td colspan="7" class="empty">Noch keine automatisch erzeugten Berichte. Scheduler in <code>soc.config.json → berichte</code> aktivieren.</td></tr>
        </tbody>
      </table>
    </section>
    ...
    <section v-show="tab === 'assistenten'" class="panel">
      ...
      <div v-if="assistant==='syslog'" class="assistant-panel">
        ...
        <table v-if="syslogSources.length" class="grid mini" style="margin-top:8px">
          <thead><tr>
...
async function select(id: string) {
  ...
  if (id === 'berichte') await store.fetchBerichte()
...
.kpi { background: #fff; border: 1px solid #e1e6ec; border-radius: 8px; padding: 18px; text-align: center; transition: box-shadow .15s; }
.kpi.clickable:hover { box-shadow: 0 2px 10px rgba(21,101,192,.15); }
...
.sev-row { display: flex; align-items: center; gap: 6px; background: #f5f7fa; border: 1px solid #e1e6ec; border-radius: 8px; padding: 6px 12px; }
...
.sla-tbl input { border: 1px solid #cfd8dc; border-radius: 4px; padding: 5px; }
...
.filterbar { display: flex; gap: 8px; margin-bottom: 12px; flex-wrap: wrap; align-items: center; background: #f5f7fa; border: 1px solid #e1e6ec; border-radius: 6px; padding: 10px 12px; }
.form-actions { display: flex; gap: 8px; margin-bottom: 12px; flex-wrap: wrap; }
.filterbar select, .filterbar input { padding: 6px 8px; border: 1px solid #cfd8dc; border-radius: 4px; font-size: 13px; }
.grid { width: 100%; border-collapse: collapse; font-size: 13px; background: #fff; border: 1px solid #e1e6ec; border-radius: 8px; overflow: hidden; }
...
.pb-assign select { padding: 6px; border: 1px solid #ccc; border-radius: 4px; flex: 1; }
.playbook { background: #fbfdff; border: 1px solid #e1e6ec; border-radius: 6px; padding: 10px 12px; margin-bottom: 8px; }
...
.add-alerts-box { margin: 10px 0; border: 1px solid #e0e0e0; border-radius: 6px; padding: 8px 12px; }
...
.esc-step { display: flex; align-items: center; gap: 10px; background: #f5faff; border: 1px solid #e0e0e0; border-radius: 6px; padding: 6px 10px; }
...
.ueb-card { border: 1px solid #e0e0e0; border-radius: 8px; margin: 10px 0; padding: 10px 14px; }
...
.lc-step { font-size: 11px; padding: 3px 10px; border-radius: 12px; background: #eceff1; color: #607d8b; border: 1px solid #cfd8dc; }
...
.ueb-tab th, .ueb-tab td { border: 1px solid #e0e0e0; padding: 4px 6px; text-align: left; vertical-align: top; }
...
/* Management-Report (#1325) */
.report-preview { display: flex; gap: 18px; flex-wrap: wrap; font-size: 13px; margin: 6px 0 8px; color: #37474f; background: #f5f7fa; border: 1px solid #e1e6ec; border-radius: 6px; padding: 12px 16px; }
/* Berichts-Center (#1350/#1357) */
.bericht-zeitraum { display: flex; gap: 16px; align-items: center; flex-wrap: wrap;
  background: #f5f7fa; border: 1px solid #e1e6ec; border-radius: 8px; padding: 14px 16px; margin-bottom: 18px; }
...
.bericht-zeitraum input, .bericht-zeitraum select { padding: 6px 8px; border: 1px solid #cfd8dc; border-radius: 4px; font-size: 13px; }
.report-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(260px, 1fr)); gap: 14px; margin-bottom: 8px; }
.report-card { background: #fff; border: 1px solid #e1e6ec; border-radius: 8px; padding: 16px;
  display: flex; flex-direction: column; transition: box-shadow .15s, border-color .15s; }
.report-card:hover { box-shadow: 0 2px 12px rgba(21,101,192,.12); border-color: #90caf9; }
...
.evi-add input, .pir-action-add input { border: 1px solid #cfd8dc; border-radius: 4px; }
