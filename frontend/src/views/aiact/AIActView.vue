<template>
  <div class="aiact-view">
    ...
    <div v-if="!store.selectedProjektObj" class="empty-state">
      <h3>{{ store.projekte.length === 0 ? 'Noch keine AI-Act-Bewertungen' : 'Kunde wählen' }}</h3>
      <p v-if="store.projekte.length === 0">
        AI Act ist <strong>kundenbezogen</strong> — pro Kunde eine Bewertung.
        Lege einen Kunden mit aktiviertem AI-Act-Modul in der Kundenverwaltung an;
        die AI-Act-Bewertung wird dann automatisch erzeugt.
      </p>
      <p v-else>Wähle links einen Kunden aus der Sidebar.</p>
      <router-link to="/kunden" class="btn-primary">→ Zur Kundenverwaltung</router-link>
    </div>
    ...
    <template v-else>
      ...
      <div v-if="activeTab === 'bericht'" class="tab-content">
        <div class="report-card">
          <h3>📄 AI-Act-Compliance-Bericht</h3>
          <p>Vollständiger Bericht mit Reifegrad, Anforderungs-Bewertungen, OWASP-LLM-Mapping
            sowie Pflicht-Doku-Nachweisen (A1-A5 + A6 Risk-Tier). Verfügbare Formate:</p>
          <div class="report-buttons">
            ...
            <DownloadButton :endpoint="stripApi(exportUrl('md'))" class="report-btn">📃 Markdown</DownloadButton>
          </div>
        </div>
      </div>

      <!-- Tab: Anforderungen (Bestehender Inhalt) -->
...
async function bulkCreateIssues() {
  if (!store.selectedProjekt) return
  const repo = prompt('GitHub-Repo (owner/repo):')
  if (!repo) return
  ...
  try {
    ...
    const r = await api.post(
      ...
      { provider: 'github', repo, only_gaps: true, skip_linked: true },
      { timeout: 120000 }
    )
...
const onReassignKunde = async (newKunde: string) => {
  ...
  try {
    await store.updateProjekt(store.selectedProjekt, { organisation: newKunde } as any)
...
.report-buttons { display: flex; gap: 10px; margin-top: 16px; flex-wrap: wrap; }
.report-btn { padding: 10px 20px; background: #1565c0; color: white; border: none; border-radius: 4px; font-size: 14px; }
</style>
