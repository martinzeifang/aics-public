<template>
  <div class="modal-overlay" @mousedown.self="onCancel">
    <div class="modal-content">
      <h3>♻️ Gelöschte Firmen</h3>
      <p class="hint">
        Soft-gelöschte Firmen können reaktiviert oder endgültig entfernt werden.
        Endgültige Löschung kann <strong>nicht rückgängig gemacht</strong> werden.
      </p>

      <div v-if="firmen.deletedFirmen.length === 0" class="empty">
        Keine gelöschten Firmen gefunden.
      </div>
      ...
      <table v-else class="deleted-table">
        ...
        <tbody>
          <tr v-for="d in firmen.deletedFirmen" :key="d.name">
            <td>{{ d.name }}</td>
            <td>{{ d.unternehmen || '—' }}</td>
...
const onRestore = async (name: string) => {
  busy.value = true
  const ok = await firmen.restoreFirma(name)
  busy.value = false
  if (ok) {
...
const onHardDelete = async (name: string) => {
  if (!confirm(`Firma "${name}" ENDGÜLTIG löschen? Diese Aktion ist NICHT umkehrbar.`)) return
  busy.value = true
  const ok = await firmen.hardDeleteFirma(name)
  busy.value = false
  if (ok) {
...
onMounted(async () => {
  await firmen.fetchDeletedFirmen()
})
</script>
