<template>
  <div class="pflicht-doku">
    ...
    <details class="section">
      ...
      <div class="section-body">
        ...
        <table v-if="store.risiken.length">
          <thead><tr><th>ID</th><th>Titel</th><th>Asset</th><th>Score</th><th>Status</th></tr></thead>
          <tbody>
    ...
    <details class="section wizards" open>
      ...
      <div class="section-body">
        <div class="assist-hint">
          🤖 Alle NIS2-Assistenten sind jetzt gebündelt im eigenen Tab.
    ...
    <details class="section wizards-d" open>
      <summary><strong>🚨 Incident-Meldungen + Awareness</strong> — 24h/72h/1M + Cyberhygiene-Quiz + Vendor-Tiering</summary>
      <div class="section-body">
        <div class="assist-hint">
          🤖 Incident-Meldungs- und Awareness-Assistenten finden Sie auch im Assistenten-Tab.
          <button class="btn-link-inline" @click="goToAssistenten">Zum Assistenten →</button>
        </div>

        <!-- N14 24h-Erstmeldung -->
        <div class="wizard-card">
          <h4>N14 — 24h-Erstmeldung (Art. 23 Abs. 4 lit. a) <span class="badge">#513</span></h4>
          <p><strong>Output:</strong> Kurzmeldung im BSI-Portal-Format — bloß die Mitteilung „es ist was passiert",
          ...
          <button class="btn-primary" :disabled="!phaseD.incident24.summary?.trim()"
                  @click="openWizard('incident-24h', phaseD.incident24)">📝 Prompt generieren</button>
        </div>

        <!-- N15a 72h-Aktualisierung -->
        <div class="wizard-card">
          <h4>N15 — 72h-Aktualisierung (Art. 23 Abs. 4 lit. b) <span class="badge">#514</span></h4>
          <p><strong>Output:</strong> Folgemeldung mit Ersteinschätzung Schweregrad + IoCs +
          ...
          <button class="btn-primary" :disabled="!phaseD.incident72.incident_id?.trim()"
                  @click="openWizard('incident-72h', phaseD.incident72)">📝 Prompt generieren</button>
        </div>

        <!-- N15b 1-Monats-Final -->
        <div class="wizard-card">
          <h4>N15 — 1-Monats-Abschlussmeldung (Art. 23 Abs. 4 lit. c) <span class="badge">#514</span></h4>
          <p><strong>Output:</strong> Vollständiger Final-Report — Vorfall-Beschreibung + Root-Cause +
          ...
          <button class="btn-primary" :disabled="!phaseD.incidentFinal.incident_id?.trim()"
                  @click="openWizard('incident-final', phaseD.incidentFinal)">📝 Prompt generieren</button>
        </div>

        <!-- N16 Cyberhygiene-Quiz -->
        <div class="wizard-card">
          <h4>N16 — Cyberhygiene-Quiz für Mitarbeiter <span class="badge">#515</span></h4>
          <p><strong>Output:</strong> 10-Fragen-Quiz (MC/True-False) für Awareness-Tests — Phishing,
          ...
          <div class="row">
            ...
            <button class="btn-primary" @click="openWizard('cyberhygiene-quiz', phaseD.quiz)">📝 Quiz erzeugen</button>
          </div>
        </div>

        <!-- N17 Vendor-Tiering -->
        <div class="wizard-card">
          <h4>N17 — Lieferanten-Risiko-Bewertung (Tiering) <span class="badge">#516</span></h4>
          <p><strong>Output:</strong> Klassifizierung kritisch/wichtig/normal + tier-spezifische
            Kontroll-Empfehlungen (z.B. Audit-Rechte ab kritisch, Pen-Tests ab wichtig).</p>
          <div v-if="store.vendors.length === 0" class="info-no-vendor">
            ⚠️ Noch keine Vendoren in N4 — bitte einen Vendor anlegen.
          </div>
          <div v-else class="row">
            ...
            <button class="btn-primary" :disabled="!phaseD.tieringVendorId" @click="openTieringWizard">📝 Tiering</button>
          </div>
        </div>
      </div>
    </details>
    ...
    <div v-if="rbImportModal.open" class="wizard-modal-overlay" @mousedown.self="closeRbImport">
      <div class="wizard-modal">
        ...
        <div v-else class="hint">Keine importierbaren Risiken für diesen Firmen gefunden.</div>
...
const openWizard = async (kind: 'klassifikator' | 'incident-notification' | 'supply-chain' | 'incident-24h' | 'incident-72h' | 'incident-final' | 'cyberhygiene-quiz' | 'vendor-tiering', extra: any = {}) => {
...
.wizards-d { background: #fff3e0; border-color: #ffb74d; }
.wizards-d .wizard-card { border-left-color: #e65100; }
.wizards-d .wizard-card h4 { color: #bf360c; }
.badge { background: #e65100; color: white; font-size: 11px; padding: 1px 7px;
