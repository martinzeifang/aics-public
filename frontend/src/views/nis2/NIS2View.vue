<template>
  <ModuleShell
    class="nis2-view"
    title="NIS2 – Cybersicherheit"
    subtitle="NIS2-Richtlinie · 30+ Anforderungen in 5 Kapiteln · 0-5-Bewertung mit Gewichtung"
    module-name="nis2"
    :tabs="store.selectedProjektObj ? tabs : []"
    :groups="store.selectedProjektObj ? navGroups : []"
    persist-key="nis2"
    v-model="activeTab"
  >
    <HelpDialog
    ...
    <div v-if="!store.selectedProjektObj" class="empty-state">
      ...
      <p v-if="store.projekte.length === 0">
        ...
        Lege eine Firma mit aktiviertem NIS2-Modul in der Firmenverwaltung an;
        die NIS2-Bewertung wird dann automatisch erzeugt.
      </p>
      <p v-else>Wähle links eine Firma aus der Sidebar.</p>
    ...
    <template v-if="store.selectedProjektObj">
      ...
      <div v-if="activeTab === 'dashboard'" class="tab-content">
        <ModuleDashboard
          :gesamt="{ percent: store.reifegrad?.gesamt?.prozent ?? 0, ampel: store.reifegrad?.gesamt?.ampel }"
          ...
          @open-punkt="(id) => { activeTab = 'anforderungen'; editAnforderungById(id) }"
          @open-fristen="activeTab = 'fristen'"
      ...
      <div v-if="activeTab === 'pflichtdoku'" class="tab-content">
        <NIS2PflichtDokuPanel @open-assistenten="activeTab = 'assistenten'" />
      </div>

      <!-- Tab: Assistenten (#1095) -->
      <div v-if="activeTab === 'assistenten'" class="tab-content">
        <AssistentenPanel
          v-if="store.selectedProjekt"
          @applied="onAssistentenApplied"
          @open-in-register="onOpenInRegister"
        />
        <div v-else class="cockpit-hint">Bitte zuerst ein NIS2-Projekt auswählen.</div>
      </div>
      ...
      <div v-if="activeTab === 'dokumente'" class="tab-content">
        <DokumenteRegister
          ...
          :auto-open-id="registerAutoOpenId"
      ...
      <!-- Tab: Vorfälle (Art. 23 · #1194) -->
      <div v-if="activeTab === 'incidents'" class="tab-content">
        <IncidentRegisterPanel :projekt-name="store.selectedProjekt" />
      </div>

      <!-- Tab: Scoping (Art. 2/3 + Art. 26 · #1210/#1211) -->
      <div v-if="activeTab === 'scoping'" class="tab-content">
        <ScopingPanel :projekt-name="store.selectedProjekt" />
      </div>

      <!-- Tab: Registrierung (Art. 27 · #1203) -->
      <div v-if="activeTab === 'registrierung'" class="tab-content">
        <RegistrierungPanel :projekt-name="store.selectedProjekt" />
      </div>

      <!-- Tab: Audits (Art. 32 · #1204) -->
      <div v-if="activeTab === 'audits'" class="tab-content">
        <AuditRegisterPanel :projekt-name="store.selectedProjekt" />
      </div>

      <!-- Tab: Governance (Art. 20 · #1212) -->
      <div v-if="activeTab === 'governance'" class="tab-content">
        <GovernancePanel :projekt-name="store.selectedProjekt" />
      </div>

      <!-- Tab: Fristen & Wiedervorlagen (Art. 21(2)f/27(4) · #1213) -->
      <div v-if="activeTab === 'fristen'" class="tab-content">
        <FristenPanel :projekt-name="store.selectedProjekt" />
      </div>

      <!-- Tab: DVO 2024/2690 (#1220 B) -->
      <div v-if="activeTab === 'dvo'" class="tab-content">
        <DvoPanel :projekt-name="store.selectedProjekt" />
      </div>

      <!-- Tab: Bericht (B2 #1093) -->
      <div v-if="activeTab === 'berichte'" class="tab-content">
        <!-- Sprint #35: einheitliches Berichts-Center (Vorbild SOC/CRA) -->
        <ReportCenterPanel
          v-if="store.selectedProjekt"
          :api-base="`/nis2/projekte/${encodeURIComponent(store.selectedProjekt)}`"
          title="NIS2-Berichts-Center"
        />
        <div v-if="store.selectedProjekt" class="report-card import-card">
    ...
    <template v-if="store.selectedProjektObj && store.selectedProjekt" #repo-config>
      <RepoConfigPanel :api-base="'/nis2'" :projekt-name="store.selectedProjekt" />
    </template>
...
import AutoBewertungMassButton from '../../components/shared/AutoBewertungMassButton.vue'
...
import IncidentRegisterPanel from './IncidentRegisterPanel.vue'
import ScopingPanel from './ScopingPanel.vue'
import RegistrierungPanel from './RegistrierungPanel.vue'
import AuditRegisterPanel from './AuditRegisterPanel.vue'
import GovernancePanel from './GovernancePanel.vue'
import FristenPanel from './FristenPanel.vue'
import DvoPanel from './DvoPanel.vue'
...
// Gruppierte Navigation (Sprint #34, #1373) — gemeinsame Taxonomie
const navGroups = [
  { id: 'ueberblick', label: '📊 Überblick', tabs: [
    { id: 'dashboard', label: '📊 Dashboard' },
    { id: 'cockpit', label: '📊 Risiko-Cockpit' },
  ] },
  { id: 'register', label: '📋 Compliance & Register', tabs: [
    { id: 'anforderungen', label: '✅ Anforderungen' },
    { id: 'scoping', label: '🎯 Scoping' },
    { id: 'registrierung', label: '📝 Registrierung' },
    { id: 'dvo', label: '📜 DVO 2690' },
...
// #1095: Der Assistenten-Tab hat einen Wizard angewendet (z.B. Klassifikator,
// Sektor-Template) — Projektliste + Pflicht-Doku-abgeleitete Daten neu laden.
const onAssistentenApplied = async () => {
  await store.fetchProjekte()
  await reloadProjekt()
}

// #1240/#1235: „Als Dokument speichern" → Dokumente-Register, Dokument öffnen.
const registerAutoOpenId = ref<number | null>(null)
const onOpenInRegister = async (id: number) => {
  registerAutoOpenId.value = null  // Reset, damit der Watcher auch bei gleicher ID feuert
  activeTab.value = 'dokumente'
  await nextTick()
  registerAutoOpenId.value = id
}

// Risiko-Cockpit-Tab (#1082): Firmen-Zuordnung des Projekts auflösen.
...
const loadCockpitFirma = async () => {
  ...
  try {
    const res = await apiClient.get(
      `/risk-cockpit/by-projekt/nis2/${encodeURIComponent(store.selectedProjekt)}`,
    )
...
// #1250: Modul-Daten → einheitliche Dashboard-Props
const dashboardStats = computed(() => [
  `${store.reifegrad?.gesamt?.punkte_aktuell ?? 0} / ${store.reifegrad?.gesamt?.punkte_max ?? 0} Punkte`,
  `${store.anforderungen.length} Anforderungen`,
])
const dashboardBereiche = computed(() =>
  Object.entries(store.reifegrad?.kapitel || {}).map(([kapitel, data]: [string, any]) => ({
    id: kapitel,
    percent: data.prozent ?? 0,
...
const dashboardLuecken = computed(() =>
  (store.reifegrad?.luecken || []).map((l: any) => ({
    id: l.id, titel: l.titel, bewertung: l.bewertung,
  })),
)
...
.import-card { margin-top: 16px; color: #555; font-size: 13px; max-width: 700px; }

.assistenten-intro {
  background: #ede7f6; border-left: 4px solid #7b1fa2; padding: 14px 18px;
  border-radius: 8px; margin-bottom: 16px;
}
.assistenten-intro h3 { margin: 0 0 6px; color: #4a148c; }
.assistenten-intro p { margin: 0; color: #444; font-size: 13px; }

.cockpit-hint {
  background: white; border: 1px solid #ddd; border-radius: 8px;
  padding: 24px; color: #555; font-size: 14px;
}
.cockpit-hint.warn {
  background: #fff3e0; border-color: #ffb74d; color: #e65100;
}
</style>
