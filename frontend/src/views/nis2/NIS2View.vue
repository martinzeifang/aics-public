<template>
  <div class="nis2-view">
    ...
    <div v-if="!store.selectedProjektObj" class="empty-state">
      <h3>{{ store.projekte.length === 0 ? 'Noch keine NIS2-Bewertungen' : 'Kunde wählen' }}</h3>
      <p v-if="store.projekte.length === 0">
        NIS2 ist <strong>kundenbezogen</strong> — pro Kunde eine Bewertung.
        Lege einen Kunden mit aktiviertem NIS2-Modul in der Kundenverwaltung an;
...
async function bulkCreateIssues() {
  ...
  try {
    ...
    alert(e?.response?.data?.error || 'Issue-Anlage fehlgeschlagen.')
