<template>
  <div class="nis2-view">
    ...
    <template v-else>
      ...
      <template v-if="activeTab === 'assistenten'">
        <div class="assistenten-intro">
          <h3>🤖 NIS2-Assistenten</h3>
          <p>Geführte KI- und Anlege-Assistenten für die NIS2-Pflicht-Dokumentation. Wähle einen
            Assistenten — du wirst zum passenden Bereich der Pflicht-Doku geführt.</p>
        </div>
        <AssistentenKachelGrid :wizards="nis2Wizards" grouped @open="onAssistentOpen" />
      </template>

      <!-- Tab: Risiko-Cockpit (#1082) -->
      <template v-if="activeTab === 'cockpit'">
...
type Nis2Tab = 'pflichtdoku' | 'anforderungen' | 'assistenten' | 'bericht' | 'cockpit'
const activeTab = ref<Nis2Tab>('pflichtdoku')

// Assistenten-Tab (#1082): NIS2-Wizards als WizardDescriptor-Liste.
const nis2Wizards: WizardDescriptor[] = buildWizardList([
  {
...
// Alle NIS2-Wizard-Formulare leben im Pflicht-Doku-Panel; die Kacheln führen
// dorthin (analog DSGVO #1083).
const onAssistentOpen = (_id: string) => {
