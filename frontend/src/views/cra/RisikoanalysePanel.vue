  ...
  <div class="risikoanalyse-panel" v-if="projektName">
    ...
    <section class="link-section">
      ...
      <div v-else class="link-form">
        <p v-if="candidates.length === 0" class="hint">
          Keine Risikobewertungs-Projekte für den Firmen <code>{{ firma || '—' }}</code> gefunden.
          Lege im Modul Risikobewertung ein Projekt für diesen Firmen an.
        </p>
        <template v-else>
    ...
    <section v-if="linked && reqStatus.length" class="nachweis-section">
      ...
      <div class="nachweis-foot">
        <button v-if="anyDiffers || applying" class="btn-primary" :disabled="applying" @click="applyAll">
          {{ applying ? 'Übernehme…' : 'Alle Vorschläge übernehmen' }}
        </button>
        <span v-else class="msg ok">✓ Alle Vorschläge sind bereits übernommen — die Bewertungen entsprechen dem aktuellen Stand.</span>
