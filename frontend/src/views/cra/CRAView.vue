<template>
  <div class="cra-view">
    ...
    <div v-else-if="creating" class="form-card">
      ...
      <!-- Issue #427/#429: Kunden-Dropdown statt Freitext -->
      <div class="form-row">
        <label>Kunde</label>
        ...
        <small class="hint">Verknüpft das Projekt mit einem Kunden aus der Kundenverwaltung.</small>
      </div>
      <div class="form-row">
        <label>Produkt</label>
...
const activeTab = ref<'dashboard' | 'requirements' | 'owasp' | 'pflichtdoku' | 'fragebogen' | 'bericht'>('dashboard')
...
async function bulkCreateIssues() {
  if (!store.selectedProjekt) return
  const repo = prompt('GitHub-Repo (owner/repo):', defaultRepoFromMeta.value)
  if (!repo) return
  ...
  try {
    ...
    alert(e?.response?.data?.error || 'Massen-Issue-Anlage fehlgeschlagen.')
...
const onReassignKunde = async (newKunde: string) => {
  ...
  try {
    await store.updateProjekt(store.selectedProjekt as string, { unternehmen: newKunde } as any)
...
onMounted(async () => {
  ...
  // Issue #435: Deep-Link via ?projekt=<name> aus KundenView
