<template>
  <ModuleShell
    ...
    subtitle="Verordnung (EU) 2024/2847 · 32 Anforderungen + 10 OWASP Proactive Controls"
    module-name="cra"
  ...
  >
    ...
    <template v-if="store.selectedProjektObj">
      <!-- Tab: Dashboard (F5e) — einheitlich (#1250) -->
      <div v-if="activeTab === 'dashboard'" class="tab-content">
        <ModuleDashboard
          ...
          @open-bereich="(id) => { activeTab = 'requirements'; filterKapitel = id }"
          @open-luecken="activeTab = 'requirements'"
          @open-punkt="editAnforderungById"
          @open-dokumente="activeTab = 'dokumente'"
          @open-risiken="activeTab = 'risikocockpit'"
        />

        <!-- OWASP Status (CRA-spezifisch, additiv) -->
      ...
      <!-- Tab: Meldungen Art. 14 (Milestone #28 / #1192) -->
      <div v-if="activeTab === 'meldungen'" class="tab-content">
        <MeldungPanel v-if="store.selectedProjekt" :projekt="store.selectedProjekt" />
        <div v-else class="action-card">
          <p class="hint">Bitte zuerst ein CRA-Projekt auswählen.</p>
        </div>
      </div>

      <!-- Tab: Konformitätsbewertung (Milestone #28 / #1201) -->
      <div v-if="activeTab === 'konformitaet'" class="tab-content">
        <KonformitaetPanel v-if="store.selectedProjekt" :projekt="store.selectedProjekt" />
        <div v-else class="action-card">
          <p class="hint">Bitte zuerst ein CRA-Projekt auswählen.</p>
        </div>
      </div>

      <!-- Tab: Wirtschaftsakteure Art. 19-22 (Milestone #28 / #1200) -->
      <div v-if="activeTab === 'akteure'" class="tab-content">
        <AkteurePanel v-if="store.selectedProjekt" :projekt="store.selectedProjekt" />
        <div v-else class="action-card">
          <p class="hint">Bitte zuerst ein CRA-Projekt auswählen.</p>
        </div>
      </div>

      <!-- Tab: Korrekturmaßnahmen/Rückruf Art. 13(19)-(22) (Milestone #28 / #1202) -->
      <div v-if="activeTab === 'korrektur'" class="tab-content">
        <KorrekturPanel v-if="store.selectedProjekt" :projekt="store.selectedProjekt" />
        <div v-else class="action-card">
          <p class="hint">Bitte zuerst ein CRA-Projekt auswählen.</p>
        </div>
      </div>

      <!-- Tab: Traceability + Annex-VII-Matrix Art. 13(1) (Milestone #28 / #1217) -->
      <div v-if="activeTab === 'traceability'" class="tab-content">
        <TraceabilityPanel v-if="store.selectedProjekt" :projekt="store.selectedProjekt" />
        <div v-else class="action-card">
          <p class="hint">Bitte zuerst ein CRA-Projekt auswählen.</p>
        </div>
      </div>
      ...
      <!-- Tab: Bericht (F5h / B2 #1093) -->
      <div v-if="activeTab === 'bericht'" class="tab-content">
        <!-- Sprint #35: einheitliches Berichts-Center (Vorbild SOC) -->
        <ReportCenterPanel
          v-if="store.selectedProjekt"
          :api-base="`/cra/projekte/${encodeURIComponent(store.selectedProjekt)}`"
          title="CRA-Berichts-Center"
        />
      </div>
    </template>
...
import MeldungPanel from './MeldungPanel.vue'
import KonformitaetPanel from './KonformitaetPanel.vue'
import AkteurePanel from './AkteurePanel.vue'
import KorrekturPanel from './KorrekturPanel.vue'
...
// #1250: einheitliches Dashboard — Dokumente-Soll-Ist + Risiko-Kurzübersicht
const { dokFertig, dokGesamt, risiko, risikoLoading, loadAll: loadDashboardExtras } =
  useModuleDashboard('cra')
...
const tabs = [
  ...
  { id: 'konformitaet', label: '✅ Konformität' },
  { id: 'akteure', label: '🏷️ Akteure' },
  { id: 'korrektur', label: '↩️ Korrekturmaßnahmen' },
...
// Gruppierte Navigation (Sprint #34, #1371) — gemeinsame Taxonomie
const navGroups = [
...
// #1250: Modul-Daten → einheitliche Dashboard-Props normalisieren
const dashboardStats = computed(() => [
  `${store.reifegrad?.gesamt?.punkte_aktuell ?? 0} / ${store.reifegrad?.gesamt?.punkte_max ?? 0} Punkte`,
  `${store.anforderungen.length} Anforderungen · ${store.owaspControls.length} OWASP`,
])
...
// #1235: Sprung „Als Dokument speichern" → Dokumente-Register, Dokument öffnen.
