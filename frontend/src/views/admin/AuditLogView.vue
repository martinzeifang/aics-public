<template>
  <div class="audit-view">
    ...
    <div class="toolbar">
      ...
      <button class="btn-secondary" @click="onVerify" :disabled="verifying">🔐 Integrität prüfen</button>
      <span v-if="verifyResult" :class="['verify-pill', verifyResult.ok ? 'ok' : 'bad']">
        {{ verifyResult.ok
          ? `✓ Kette intakt (${verifyResult.count} Einträge)`
          : `⚠ Kette gebrochen bei id ${verifyResult.broken_at}` }}
      </span>
...
const verifyResult = ref<{ ok: boolean; count: number; broken_at: number | null } | null>(null)

const onVerify = async () => {
  verifying.value = true
  verifyResult.value = null
  try {
    verifyResult.value = await admin.verifyAuditChain()
  } catch {
    verifyResult.value = { ok: false, count: 0, broken_at: null }
  } finally {
