  ...
  <div class="license-view">
    ...
    <div class="card">
      ...
      <div v-else-if="status" class="status-grid">
        ...
        <div v-if="status.customer"><strong>Firma:</strong> {{ status.customer }}</div>
...
// #1176: TLS-Verify secure-by-default (true)
const serverCfg = ref<ServerCfg>({ server_url: '', verify_tls: true, request_timeout: 15, has_file_override: false })
const editServer = ref(false)
const serverDraft = ref<ServerCfg>({ server_url: '', verify_tls: true, request_timeout: 15, has_file_override: false })
