    ...
    <div v-if="editing" class="modal-overlay" @mousedown.self="editing = null">
      <div class="modal-content modal-wide">
        ...
        <div class="modal-body">
          ...
          <!-- #1185: Objekt-RBAC — Firmen-Whitelist -->
          <fieldset class="fset">
            <legend>Sichtbare Firmen (Objekt-Whitelist)</legend>
            <label class="check-row">
              <input type="checkbox" :checked="editing.allowed_firmen === null"
                     @change="onAllFirmenToggle($event)" />
              <strong>Alle Firmen freischalten (keine Einschränkung)</strong>
            </label>
            <div v-if="editing.allowed_firmen !== null" class="check-grid mt-1">
              <label v-for="f in firmenList" :key="f.id" class="check-row">
                <input type="checkbox" :value="f.id"
                       :checked="(editing.allowed_firmen || []).includes(f.id)"
                       @change="toggle(editing.allowed_firmen, f.id, $event)" />
                <strong>{{ f.name }}</strong>
              </label>
              <p v-if="!firmenList.length" class="hint">Keine Firmen vorhanden.</p>
            </div>
            <p class="hint">Beschränkt den Zugriff auf Projekte/Daten der gewählten Firmen
              (serverseitig erzwungen). Admins sind nie eingeschränkt.</p>
          </fieldset>
...
const firmenList = ref<{ id: number; name: string }[]>([])  // #1185
...
const reload = async () => {
  ...
  try {
    const [u, c, f] = await Promise.all([
      ...
      apiClient.get('/firmen').catch(() => ({ data: [] })),  // #1185: Firmen für Whitelist
    ])
    users.value = u.data
    Object.assign(catalog, c.data)
    firmenList.value = (f.data || []).map((x: any) => ({ id: x.firmen_id, name: x.name }))
...
const openEdit = (u: User) => {
  editing.value = {
    ...
    allowed_firmen: u.allowed_firmen === null ? null : [...(u.allowed_firmen || [])],
...
const toggle = (list: (string | number)[] | null, value: string | number, e: Event) => {
  if (list === null) return
...
const onAllFirmenToggle = (e: Event) => {  // #1185
  if (!editing.value) return
  const checked = (e.target as HTMLInputElement).checked
  editing.value.allowed_firmen = checked ? null : []
}
...
const onSave = async () => {
  ...
  const payload: any = {
    ...
    allowed_firmen: e.allowed_firmen,  // #1185
