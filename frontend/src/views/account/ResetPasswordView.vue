<template>
  <div class="reset-wrap">
    <div class="card">
      <h2>🔐 Neues Passwort setzen</h2>

      <p v-if="!token" class="fail">Kein Token in der URL. Bitte den vollständigen Reset-Link öffnen.</p>

      <form v-if="token && !done" @submit.prevent="submit">
        <label>
          Neues Passwort
          <input v-model="pw1" type="password" autocomplete="new-password" required />
        </label>
        <label>
          Wiederholen
          <input v-model="pw2" type="password" autocomplete="new-password" required />
        </label>
        <button class="btn-primary" :disabled="loading || !canSubmit">
          {{ loading ? 'Setze…' : 'Passwort setzen' }}
        </button>
      </form>

      <div v-if="done" class="ok">
        ✓ Passwort gesetzt. Du kannst dich jetzt anmelden.
        <p style="margin-top: 12px;"><router-link to="/login">→ Zum Login</router-link></p>
      </div>

      <div v-if="error" class="fail">⚠ {{ error }}</div>
    </div>
  </div>
</template>

<script setup lang="ts">
import { ref, computed } from 'vue'
import { useRoute } from 'vue-router'
import axios from 'axios'

const route = useRoute()
const token = ref(String(route.query.token || ''))
const pw1 = ref('')
const pw2 = ref('')
const loading = ref(false)
const done = ref(false)
const error = ref('')

const canSubmit = computed(() => pw1.value.length >= 8 && pw1.value === pw2.value)

async function submit() {
  loading.value = true
  error.value = ''
  try {
    await axios.post('/api/auth/password/reset', {
      token: token.value,
      new_password: pw1.value,
    })
    done.value = true
  } catch (e: any) {
    error.value = e?.response?.data?.error || 'Fehler beim Setzen des Passworts.'
  } finally {
    loading.value = false
  }
}
</script>

<style scoped>
.reset-wrap { display: flex; justify-content: center; padding: 60px 20px; }
.card { max-width: 480px; background: var(--color-surface, #fff); padding: 32px; border-radius: 8px; box-shadow: 0 2px 8px rgba(0,0,0,0.08); }
label { display: block; margin-bottom: 12px; }
label input { display: block; width: 100%; padding: 8px; margin-top: 4px; box-sizing: border-box; }
.btn-primary { background: var(--color-primary, #1565c0); color: #fff; border: 0; padding: 10px 18px; border-radius: 6px; cursor: pointer; }
.btn-primary:disabled { opacity: 0.6; cursor: not-allowed; }
.ok { color: #2e7d32; margin-top: 12px; }
.fail { color: #c62828; margin-top: 12px; }
</style>
