<template>
  <div class="login-split" :class="{ 'login-soc': isSocPortal }">
    <aside class="login-brand">
      ...
      <div class="login-brand-top">
        <div class="brand-lockup">
          ...
          <div>
            <div class="brand-name">{{ isSocPortal ? 'SOC Operation Center' : 'AI Compliance Suite' }}</div>
            <div class="brand-tag">{{ isSocPortal ? 'Security Operations Center' : 'Compliance Management Platform' }}</div>
          </div>
        </div>
      </div>
      <div v-if="isSocPortal" class="login-brand-mid">
        <div class="login-eyebrow">Detektion · Triage · Incident-Response</div>
        <h2 class="login-headline">
          Security Operations, <span class="hl">rund um die Uhr</span> im Blick.
        </h2>
        <p class="login-sub">
          Wazuh-Alarm-Triage, Incident-Management und Meldepflichten (NIS2 / DSGVO) —
          fokussierte Arbeitsumgebung für das SOC-Team.
        </p>
        <ul class="login-frameworks">
          <li>ISO/IEC 27035</li><li>NIST SP 800-61</li><li>BSI DER.2</li><li>NIS2 Art. 23</li>
        </ul>
      </div>
      <div v-else class="login-brand-mid">
    ...
    <main class="login-pane">
      ...
      <!-- #1474: Portal vom Admin deaktiviert → Hinweis statt Login -->
      <div v-if="portalDisabled" class="portal-disabled">
        <div class="pd-icon" aria-hidden="true">🛑</div>
        <h1>SOC-Portal derzeit deaktiviert</h1>
        <p>Das SOC-Operations-Portal wurde durch einen Administrator vorübergehend deaktiviert.
           Bitte später erneut versuchen oder die Administration kontaktieren.</p>
      </div>

      <form v-if="!portalDisabled && step === 'password'" class="login-form2" @submit.prevent="handleLogin">
        ...
        <footer v-if="publicCfg.demo_users_enabled" class="login-form-foot">
          <span>Demo:</span>
          <button v-if="isSocPortal" type="button" class="demo-chip" @click="fillDemo('soc')">SOC-Operator</button>
          <button type="button" class="demo-chip" @click="fillDemo('admin')">Admin</button>
          <button v-if="!isSocPortal" type="button" class="demo-chip" @click="fillDemo('editor')">Editor</button>
        </footer>
      </form>

      <form v-else-if="!portalDisabled" class="login-form2" @submit.prevent="handleTotpVerify">
        <header class="login-form-hdr">
      ...
      <div class="login-pane-foot">
        ...
        <!-- #1051: Datenschutz/Impressum vorerst ausgeblendet (Seiten existieren noch nicht).
             Nicht löschen — bei Verfügbarkeit der Seiten v-if entfernen und href setzen. -->
        <template v-if="false">
        ...
        <span class="mono-tiny">v{{ appVersion }}</span>
      </div>
    </main>
  </div>
</template>
...
interface PublicCfg {
  demo_users_enabled: boolean; sso_enabled: boolean; env: string
  portal?: 'suite' | 'soc'; portal_name?: string; portal_enabled?: boolean
}
const publicCfg = ref<PublicCfg>({ demo_users_enabled: false, sso_enabled: false, env: 'production' })
// #1413: SOC-Portal-Login-Branding + #1474 Admin-Schalter (deaktiviert → Hinweis statt Login).
const isSocPortal = computed(() => publicCfg.value.portal === 'soc')
const portalDisabled = computed(() => isSocPortal.value && publicCfg.value.portal_enabled === false)
...
const fillDemo = (kind: 'admin' | 'editor' | 'soc') => {
  if (kind === 'admin') {
    email.value = 'admin@example.com'
    password.value = 'admin-password'
  } else if (kind === 'soc') {
    email.value = 'soc@example.com'
    password.value = 'soc-password'
  } else {
...
/* #1413: SOC-Operations-Portal — eigenständiges, dunkles Security-Theme. */
.login-soc .login-brand {
  background: linear-gradient(140deg, #0b1220 0%, #111b2e 55%, #0e3a3a 130%);
}
.login-soc .login-headline .hl { color: #2dd4bf; }
.login-soc .login-eyebrow { color: #5eead4; }
.login-soc .brand-mark { color: #2dd4bf; border-color: rgba(45, 212, 191, 0.5); }
.login-soc .login-frameworks li { border-color: rgba(45, 212, 191, 0.35); }
.portal-disabled {
  margin: auto; max-width: 420px; text-align: center; padding: 32px 24px;
}
.portal-disabled .pd-icon { font-size: 44px; margin-bottom: 12px; }
.portal-disabled h1 { font-size: 1.4rem; color: #b71c1c; margin: 0 0 10px; }
.portal-disabled p { color: #546e7a; line-height: 1.5; }
