<template>
  <div class="dora-view">
    ...
    <template v-else-if="store.selectedProjektObj">
      ...
      <!-- Tabs (Sprint #34: gruppierte Navigation) -->
      <GroupedModuleNav :groups="navGroups" :model-value="activeTab" persist-key="dora"
                        @update:model-value="(id: string) => activeTab = id" />
          ...
          <table v-if="store.tpps.length > 0">
            ...
            <tbody>
              <tr v-for="t in store.tpps" :key="t.id" @click="openTPPDialog(t)">
                ...
                <td>{{ t.status }}</td>
                <td>
                  <a v-if="t.vertrag_url" :href="safeUrl(t.vertrag_url)" target="_blank" rel="noopener noreferrer" @click.stop>📄</a>
                  <span v-else class="muted">—</span>
                </td>
                <td>
      ...
      <!-- Tab: Bericht (Sprint #35: einheitliches Berichts-Center) -->
      <div v-if="activeTab === 'bericht'" class="tab-content">
        <ReportCenterPanel
          v-if="store.selectedProjekt"
          :api-base="`/dora/projekte/${encodeURIComponent(store.selectedProjekt)}`"
          title="DORA-Berichts-Center"
        />
      </div>
    </template>
    ...
    <!-- TPP-Dialog -->
    <div v-if="tppDialog.open" class="modal-overlay" @mousedown.self="tppDialog.open = false">
    ...
    <!-- Test-Dialog -->
    <div v-if="testDialog.open" class="modal-overlay" @mousedown.self="testDialog.open = false">
      <div class="modal-content">
        <div class="modal-header">
