<template>
  ...
  >
    ...
    <template v-else>
        ...
        <div class="dashboard">
          ...
          <div class="chapters-grid">
            <ChapterCard
              ...
              :key="kap"
              :id="String(kap)"
              :title="kapitelTitle(String(kap))"
              :percent="Math.round(pct ?? 0)"
              :bewertet="kapitelCounts(String(kap)).bewertet"
              :gesamt="kapitelCounts(String(kap)).gesamt"
              @click="activeTab = 'anforderungen'; filterKapitel = String(kap)"
            />
          </div>
        </div>
      ...
      <!-- DSMS-Bereiche (Sprint #23) -->
      <div v-if="activeTab === 'dsms-cockpit'" class="tab-content">
        <DsmsCockpitPanel v-if="store.selectedProjekt" @open-tab="activeTab = $event" />
...
import DsmsCockpitPanel from './DsmsCockpitPanel.vue'
...
const tabs = [
  ...
  { id: 'dsms-cockpit', label: '📊 DSMS-Cockpit' },
