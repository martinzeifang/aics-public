<template>
  <div class="dsgvo-view">
    <div class="header">
      ...
      <ModuleHelpButton module="dsgvo" />
    </div>

    <div v-if="store.error" class="alert alert-error">{{ store.error }}</div>

    <div v-if="!store.selectedProjektObj" class="empty-state">
      <h3>{{ store.projekte.length === 0 ? 'Noch keine DSGVO-Bewertungen' : 'Kunde wählen' }}</h3>
      <p v-if="store.projekte.length === 0">
        DSGVO ist <strong>kundenbezogen</strong> — pro Kunde eine Bewertung.
        Lege einen Kunden mit aktiviertem DSGVO-Modul in der Kundenverwaltung an;
    ...
    <template v-else>
      ...
      <RepoConfigPanel v-if="store.selectedProjekt" api-base="/dsgvo" :projekt-name="store.selectedProjekt" />
...
async function syncIssues() {
  ...
  try {
    ...
    const r = await api.post(`/dsgvo/projekte/${encodeURIComponent(store.selectedProjekt)}/issues/sync`, {}, { timeout: 120000 })
    const d = r.data || {}
...
const activeTab = ref<'pflichtdoku' | 'anforderungen' | 'tom' | 'privacy' | 'training'>('pflichtdoku')
...
onMounted(async () => {
  ...
  await store.fetchProjekte()
  // Issue #434: Deep-Link via ?projekt=<kunde> aus KundenView
...
async function bulkCreateIssues() {
  ...
  try {
    const res = await api.post(`/dsgvo/projekte/${encodeURIComponent(store.selectedProjekt)}/issues/bulk`, {})
    const d = (res && res.data) ? res.data : {}
    alert(`Issues angelegt: ${d.created ?? 0} · übersprungen: ${d.skipped ?? 0} · fehlgeschlagen: ${d.failed ?? 0}`)
    if (typeof store.loadAnforderungen === 'function') {
      try { await store.loadAnforderungen(store.selectedProjekt) } catch (e) { /* optional */ }
    }
  } catch (e) {
