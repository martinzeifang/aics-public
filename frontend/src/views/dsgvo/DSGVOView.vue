  ...
  >
    ...
    <div v-if="!store.selectedProjektObj" class="empty-state">
      ...
      <p v-if="store.projekte.length === 0">
        ...
        Lege eine Firma mit aktiviertem DSGVO-Modul in der Firmenverwaltung an;
    ...
    <template v-else>
      ...
      <div v-if="activeTab === 'berichte'" class="tab-content">
        <!-- Sprint #35 (#1387): einheitliches Berichts-Center (Vorbild SOC/CRA);
             deckt Gesamtbericht + 8 Bereichsberichte als Berichtstypen ab. -->
        <ReportCenterPanel
          v-if="store.selectedProjekt"
          :api-base="`/dsgvo/projekte/${encodeURIComponent(store.selectedProjekt)}`"
          title="DSGVO-Berichts-Center"
        />
      </div>
        ...
        <div class="modal-content modal-wide">
          ...
          <div class="modal-body">
            ...
            <div style="display:flex; gap:10px; align-items:center;">
              ...
              <button class="btn-mini" :disabled="aiPromptModal.running" @click="runDsgvoWizard">⚡ Direkt mit KI ausführen</button>
            </div>
            <textarea readonly :value="aiPromptModal.prompt" rows="14" class="prompt-textarea"></textarea>
            <!-- #1366: Direkt über den Provider (lokal/Cloud) ausführen -->
            <div v-if="aiPromptModal.running" class="ki-run">
              <KiStreamView :url="`/api/ai/run-stream`" :body="{ prompt: aiPromptModal.prompt }"
                            @done="onDsgvoRunDone" @error="onDsgvoRunError" />
            </div>
...
// #1366: DSGVO-Assistenten-Prompt direkt über den Provider (lokal/Cloud) ausführen.
const runDsgvoWizard = () => { aiPromptModal.value.running = true }
const onDsgvoRunDone = (p: any) => { aiPromptModal.value.running = false; aiResponseRaw.value = (p?.text || '').trim() }
const onDsgvoRunError = () => { aiPromptModal.value.running = false }
...
// Gruppierte Navigation (Sprint #34, #1374) — DSGVO hat die meisten Tabs (24),
// daher domänenspezifische Register-Gruppen (je ≤8 Tabs).
const navGroups = [
  { id: 'ueberblick', label: '📊 Überblick', tabs: [
    { id: 'dashboard', label: '📊 Dashboard' },
    { id: 'cockpit', label: '📊 Risiko-Cockpit' },
  ] },
  { id: 'pflichten', label: '📋 Pflichten & Rechtsgrundlagen', tabs: [
    { id: 'anforderungen', label: '✅ Anforderungen' },
    { id: 'einwilligung', label: '✍️ Einwilligungen' },
    { id: 'lia', label: '⚖️ LIA-Register' },
    { id: 'zweckaenderung', label: '🔄 Zweckänderung' },
  ...
  { id: 'schutz', label: '🔐 Schutz & Betroffene', tabs: [
    { id: 'tom-katalog', label: '🔐 TOM-Katalog' },
    { id: 'tom', label: '🔒 TOM-Generator' },
    { id: 'loeschkonzept', label: '🗑️ Löschkonzept' },
    { id: 'betroffenenrechte', label: '📨 Betroffenenrechte' },
  ...
  { id: 'organisation', label: '🏛️ Organisation', tabs: [
    { id: 'dsgvo-dsb', label: '🛡️ DSB' },
    { id: 'eu-vertreter', label: '🇪🇺 EU-Vertreter' },
  ] },
