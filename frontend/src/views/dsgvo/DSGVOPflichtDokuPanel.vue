<template>
  <div class="pflicht-doku">
    <div class="info-banner">
      <h3>📋 DSGVO-Dokumentation — Hier starten</h3>
      ...
      <div class="workflow">
        ...
        <ol>
          ...
          <li><strong>D3 DSFA</strong> (Art. 35) — Folgenabschätzung bei hohem Risiko</li>
    ...
    <details class="section" open>
      ...
      <div class="section-body">
        <div class="help-box">
          ...
          Rolle (Art. 30(1) Verantwortlicher / 30(2) Auftragsverarbeiter), Zweck, Rechtsgrundlage (Art. 6,
          ggf. Art. 9), Betroffene, Datenkategorien, Empfänger, Datenfluss, Drittland, Löschfrist + Verweis,
          TOM-Verweis, DSFA-Trigger (Art. 35). Wizard <strong>D6</strong> bestimmt automatisch die Rechtsgrundlage.
        </div>
        ...
        <!-- #1101: Rollen-Filter Art. 30(1)/30(2) -->
        <div class="vvt-rolle-filter">
          <span class="filter-label">Rolle:</span>
          <button
            class="pill-btn" :class="{ active: store.vvtRolleFilter === '' }"
            @click="store.setVvtRolleFilter('')">Alle</button>
          <button
            class="pill-btn" :class="{ active: store.vvtRolleFilter === 'verantwortlicher' }"
            @click="store.setVvtRolleFilter('verantwortlicher')">Verantwortlicher (30(1))</button>
          <button
            class="pill-btn" :class="{ active: store.vvtRolleFilter === 'auftragsverarbeiter' }"
            @click="store.setVvtRolleFilter('auftragsverarbeiter')">Auftragsverarbeiter (30(2))</button>
        </div>
        <table v-if="store.vvt.length">
          <thead><tr><th>ID</th><th>Rolle</th><th>Name</th><th>Zweck</th><th>Rechtsgrundlage</th><th>DSFA</th><th></th></tr></thead>
          <tbody>
            <tr v-for="v in store.vvt" :key="v.id">
              <td><code>{{ v.vvt_id }}</code></td>
              <td>
                <span class="rolle-pill" :class="(v.rolle || 'verantwortlicher')">
                  {{ (v.rolle || 'verantwortlicher') === 'auftragsverarbeiter' ? 'AV (30(2))' : 'V (30(1))' }}
                </span>
              </td>
              <td>{{ v.name }}</td>
              <td>{{ truncate(v.zweck, 80) }}</td>
              <td>{{ v.rechtsgrundlage }}<template v-if="v.art9_grundlage"> · {{ v.art9_grundlage }}</template></td>
              <td>
                <span v-if="Number(v.dsfa_trigger)" class="dsfa-flag" title="DSFA-Pflicht (Art. 35)">⚠️ DSFA</span>
    ...
    <!-- D2 TOM — Übersicht + Verweis auf TOM-Katalog (#1115) -->
    <details class="section">
      <summary><strong>D2 — TOM (Art. 32)</strong> (im TOM-Katalog gepflegt)</summary>
      <div class="section-body">
        <div class="help-box">
          <strong>Worum geht's?</strong> Technische und Organisatorische Maßnahmen werden jetzt im
          strukturierten <strong>TOM-Katalog</strong> gepflegt — entlang der SDM-Gewährleistungsziele +
          Art. 32, mit Reifegrad (0–5), Soll/Ist und <strong>Wirksamkeitsüberprüfung</strong>.
          Hier in der Pflicht-Doku gibt es nur die Übersicht — keine manuelle Einzel-Eingabe mehr.
        </div>
        <div class="d2-jump">
          <button class="btn-primary" @click="emit('open-tab', 'tom-katalog')">🔐 Zum TOM-Katalog (Art. 32 / SDM) →</button>
        </div>
        <details v-if="store.tom.length" class="legacy">
          <summary>Alt-TOM-Einträge (read-only Legacy, {{ store.tom.length }})</summary>
          <table>
            <thead><tr><th>Kategorie</th><th>Maßnahme</th><th>Status</th><th>Verantwortlich</th></tr></thead>
            <tbody>
    ...
    <!-- D3 DSFA -->
    <details class="section">
      <summary><strong>D3 — DSFA (Art. 35)</strong> ({{ store.dpia.length }})</summary>
      <div class="section-body">
        <div class="help-box">
          <strong>Worum geht's?</strong> Eine Datenschutz-Folgenabschätzung (Art. 35) ist ein komplexer,
          mehrstufiger Prozess — kein einfaches Formular. Die Tabelle unten ist nur die <strong>Übersicht</strong>;
          jede DSFA wird im <strong>geführten DSFA-Assistenten</strong> bearbeitet
          (Schwellwertanalyse → Beschreibung → Notwendigkeit → Risiko (aus Risikobewertung) →
          Maßnahmen/Restrisiko → Konsultation Art. 36 → Freigabe).
        </div>
        <div class="d3-actions">
          <button class="btn-primary" @click="startDsfaAssistent">➕ Neue DSFA im Assistenten starten</button>
        </div>
        <table>
          ...
          <tbody>
            <tr v-for="d in store.dpia" :key="d.id">
              ...
              <td>{{ d.status }}</td>
              <td>
                <button class="btn-link" @click="openDsfaDetail(d)" title="DSFA-Details (Art. 35(7) a–d)">🔎</button>
                <button class="btn-link" @click="store.deleteDpia(d.id)">🗑️</button>
              </td>
            </tr>
          </tbody>
        </table>

        <!-- DSFA-Detailansicht: Art. 35(7)a+b lokal + Risiken(c)/Maßnahmen(d) read-only (#1085) -->
        <DSFADetailPanel
          v-if="selectedDsfa"
          :dpia="selectedDsfa"
          @close="closeDsfaDetail"
        />
      </div>
    </details>
    ...
    <!-- Phase B: KI-Wizards — jetzt im Assistenten-Tab (#1083) -->
    <details class="section wizards" open>
      ...
      <div class="section-body">
        <p class="assistenten-pointer">
          Die geführten KI-Assistenten (D6 Rechtsgrundlage, D7 Branchen-Template, D8 Datenpannen-Meldung,
          D9 Betroffenenrechte) sowie TOM-, Datenschutzerklärungs- und Schulungs-Generator findest du
          gebündelt im Tab <strong>🤖 Assistenten</strong>.
        </p>
        <button class="btn-primary" @click="emit('open-assistenten')">🤖 Zum Assistenten →</button>
      </div>
    </details>

    <!-- VVT-Anlege-Wizard (#587) -->
    <div v-if="vvtWizard.open" class="wizard-modal-overlay" @mousedown.self="closeVvtWizard">
      <div class="wizard-modal vvt-modal">
        ...
        <fieldset>
          <legend>🧭 Rolle (Art. 30 DSGVO)</legend>
          <small class="hint">Art. 30(1) gilt für <strong>Verantwortliche</strong>, Art. 30(2) für <strong>Auftragsverarbeiter</strong> — die Pflichtfelder unterscheiden sich.</small>
          <div class="rolle-toggle">
            <label class="rolle-opt" :class="{ active: vvtWizard.data.rolle === 'verantwortlicher' }">
              <input type="radio" value="verantwortlicher" v-model="vvtWizard.data.rolle" />
              Verantwortlicher (Art. 30(1))
            </label>
            <label class="rolle-opt" :class="{ active: vvtWizard.data.rolle === 'auftragsverarbeiter' }">
              <input type="radio" value="auftragsverarbeiter" v-model="vvtWizard.data.rolle" />
              Auftragsverarbeiter (Art. 30(2))
            </label>
          </div>
        </fieldset>
        ...
        <fieldset>
          ...
          <div class="vvt-grid">
            <label>VVT-ID <span class="req">*</span>
              <small>Eindeutige Kennzeichnung, z.B. <code>VVT-001</code>. Wird in DSFA + AVV referenziert.</small>
              <input v-model="vvtWizard.data.vvt_id" placeholder="VVT-001" />
            </label>
            <label>Name der Verarbeitung <span class="req">*</span>
              <small>Kurze, sprechende Bezeichnung (z.B. „Firmenstammdaten-Verwaltung im CRM").</small>
              <input v-model="vvtWizard.data.name" placeholder="z.B. CRM-Firmenstammdaten" />
            </label>
          </div>
          <label class="full">Zweck der Verarbeitung <span class="req">*</span>
            ...
            <textarea v-model="vvtWizard.data.zweck" rows="3"
              placeholder="z.B. Vertragsabwicklung mit Firmen, Rechnungserstellung, Firmenpflege per E-Mail." />
          </label>
        </fieldset>
        ...
        <fieldset>
          ...
          <div class="vvt-grid">
            ...
            <label>Art. 9-Grundlage (besondere Kategorien)
              <small>Nur bei besonderen Datenkategorien (Gesundheit, Religion, Biometrie …) — Art. 9 Abs. 2 lit.</small>
              <select v-model="vvtWizard.data.art9_grundlage">
                <option value="">— Keine besondere Kategorie —</option>
                <option value="Art. 9(2)(a)">Art. 9(2)(a) — Ausdrückliche Einwilligung</option>
                <option value="Art. 9(2)(b)">Art. 9(2)(b) — Arbeitsrecht/Soziales</option>
                <option value="Art. 9(2)(c)">Art. 9(2)(c) — Lebenswichtige Interessen</option>
                <option value="Art. 9(2)(g)">Art. 9(2)(g) — Erhebliches öffentl. Interesse</option>
                <option value="Art. 9(2)(h)">Art. 9(2)(h) — Gesundheitsvorsorge</option>
                <option value="Art. 9(2)(i)">Art. 9(2)(i) — Öffentliche Gesundheit</option>
                <option value="Art. 9(2)(j)">Art. 9(2)(j) — Archiv/Forschung/Statistik</option>
              </select>
            </label>
          </div>
        </fieldset>
        ...
        <fieldset>
          ...
          <label class="full">Kategorien betroffener Personen
            <small>Wer ist betroffen? z.B. Firmen, Mitarbeiter, Interessenten, Bewerber, Lieferanten.</small>
            <textarea v-model="vvtWizard.data.betroffene_kategorien" rows="2"
              placeholder="Firmen (B2B + B2C), Interessenten, ehemalige Firmen für 6 Monate" />
          </label>
        ...
        <fieldset>
          ...
          <label class="full">Datenfluss / Schnittstellen
            <small>Woher kommen die Daten, wohin fließen sie? Systeme, Schnittstellen, Übergabepunkte (für Transparenz + DSFA).</small>
            <textarea v-model="vvtWizard.data.datenfluss" rows="2"
              placeholder="z.B. Webformular → CRM (PostgreSQL) → Buchhaltung (DATEV-Export) → Steuerberater (SFTP)" />
          </label>
        </fieldset>
        ...
        <fieldset>
          ...
          <div class="vvt-grid">
            <label>Verweis Löschkonzept
              <small>Strukturierter Verweis auf den Eintrag im Löschkonzept (z.B. Lösch-ID / Dokumentabschnitt).</small>
              <input v-model="vvtWizard.data.loeschfrist_ref" placeholder="z.B. LK-2024 §3.2 / Lösch-Regel LR-07" />
            </label>
            <label>Verweis TOM (Art. 32)
              <small>Strukturierte Referenz auf die zugeordnete(n) TOM-ID(s) in D2.</small>
              <input v-model="vvtWizard.data.tom_ref" placeholder="z.B. TOM-003, TOM-011" />
            </label>
          </div>
          <label class="full dsfa-trigger-row">
            <input type="checkbox" :checked="!!Number(vvtWizard.data.dsfa_trigger)"
              @change="vvtWizard.data.dsfa_trigger = ($event.target as HTMLInputElement).checked ? 1 : 0" />
            <span>
              <strong>DSFA-Trigger (Art. 35)</strong> — voraussichtlich hohes Risiko für Rechte/Freiheiten
              (z.B. systematisches Profiling, besondere Datenkategorien in großem Umfang, Überwachung).
              Markiert die Verarbeitung als DSFA-pflichtig → siehe D3.
            </span>
          </label>
        </fieldset>
        ...
        <fieldset>
          ...
          <label class="full">Notizen
            ...
            <textarea v-model="vvtWizard.data.notizen" rows="3"
              placeholder="z.B. Verzicht auf Einwilligungspflicht wegen Vertragserfüllung. DSFA durchgeführt (DSFA-001)." />
          </label>
        </fieldset>
...
import DSFADetailPanel from './DSFADetailPanel.vue'

const store = useDsgvoStore()

// #1083: Navigation zum Assistenten-Tab in der DSGVOView.
const emit = defineEmits<{ (e: 'open-assistenten'): void; (e: 'open-tab', tab: string): void }>()

// DSFA-Detailansicht (#1085): zeigt Art. 35(7)a+b lokal + Risiken(c)/Maßnahmen(d) read-only
const selectedDsfa = ref<any | null>(null)
const openDsfaDetail = (d: any) => { selectedDsfa.value = d }
const closeDsfaDetail = () => { selectedDsfa.value = null }

const vvtForm = ref<any>({})
// #1115: D3 — DSFA über den geführten Assistenten anlegen (statt simples Formular)
const startDsfaAssistent = async () => {
  const n = (store.dpia?.length || 0) + 1
  const id = 'DSFA-' + String(n).padStart(3, '0')
  await store.saveDpia({ dpia_id: id, titel: 'Neue DSFA', restrisiko: 'niedrig' })
  await store.fetchPflichtDokuStatus()
  const rec = (store.dpia || []).find((d: any) => d.dpia_id === id)
  if (rec) selectedDsfa.value = rec
}
...
const statusItems = computed(() => {
  ...
  return [
    ...
    { key: 'dpia', label: 'DSFA', ok: s.dpia?.ok, detail: `${s.dpia?.count || 0}` },
...
const vvtWizardEmpty = () => ({
  ...
  rolle: 'verantwortlicher', art9_grundlage: '', datenfluss: '',
  loeschfrist_ref: '', tom_ref: '', dsfa_trigger: 0,
})
const vvtRolleLabel = (r: string) => r === 'auftragsverarbeiter'
  ? 'Auftragsverarbeiter (Art. 30(2))'
  : 'Verantwortlicher (Art. 30(1))'
...
// Wizards — die Tabellen-Zeilenaktionen D6 (Rechtsgrundlage) + D8 (Datenpannen-
// Meldung) bleiben kontextbezogen hier; die eigenständigen Launcher (D7/D9) sind
// in den Assistenten-Tab umgezogen (#1083).
...
.assistenten-pointer { margin: 0 0 12px; color: #4a148c; font-size: 13px; line-height: 1.55; }
...
/* #1101: Rollen-Toggle / -Filter, DSFA-Flag */
.rolle-toggle { display: flex; gap: 12px; flex-wrap: wrap; margin-top: 8px; }
.rolle-opt {
  flex-direction: row !important; align-items: center; gap: 6px;
  border: 1px solid #ccc; border-radius: 6px; padding: 8px 12px; cursor: pointer;
  font-weight: 500 !important; background: #fff; margin-bottom: 0 !important;
}
.rolle-opt.active { border-color: #1565c0; background: #e3f2fd; color: #1565c0; font-weight: 600 !important; }
.rolle-opt input { width: auto; }

.vvt-rolle-filter {
  display: flex; align-items: center; gap: 8px; flex-wrap: wrap; margin: 4px 0 12px;
}
.vvt-rolle-filter .filter-label { font-weight: 600; color: #333; font-size: 13px; }
.pill-btn {
  border: 1px solid #c5cae9; background: #fff; color: #3949ab; border-radius: 999px;
  padding: 4px 12px; font-size: 12px; cursor: pointer;
}
.pill-btn.active { background: #1565c0; border-color: #1565c0; color: #fff; }

.rolle-pill {
  display: inline-block; border-radius: 4px; padding: 2px 6px; font-size: 11px; font-weight: 600;
}
.rolle-pill.verantwortlicher { background: #e3f2fd; color: #1565c0; }
.rolle-pill.auftragsverarbeiter { background: #fff3e0; color: #e65100; }
.dsfa-flag { color: #c62828; font-weight: 600; font-size: 12px; }

.dsfa-trigger-row {
  flex-direction: row !important; align-items: flex-start; gap: 8px;
  background: #fff8e1; border: 1px solid #ffe082; border-radius: 6px; padding: 10px 12px;
  font-weight: normal !important;
}
.dsfa-trigger-row input { width: auto; margin-top: 2px; }
</style>
