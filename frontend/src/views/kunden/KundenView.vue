    ...
    <div v-else class="kunde-form">
      ...
      <div v-if="activeTab === 'general'" class="tab-content">
        <fieldset>
          ...
          <div class="form-row">
            ...
            />
            <small v-if="!creating">Kunden-Name kann nach dem Speichern nicht mehr geändert werden.</small>
            <small v-else class="hint">
              Beim Anlegen werden Modul-Projekte mit diesem Namen automatisch in allen aktivierten Modulen erstellt.
            </small>
          </div>
          <div class="form-row">
      ...
      <div v-if="activeTab === 'modules'" class="tab-content">
        <fieldset>
          ...
          <!-- Issue #430: Bestandskunden — Modul-Projekte nachsynchronisieren -->
          <div v-if="!creating && form.name" class="form-row" style="margin-top: 12px;">
            <button class="btn-secondary" type="button" @click="onSyncProjekte" :disabled="syncing">
              {{ syncing ? '⏳ Synchronisiere…' : '🔄 Modul-Projekte jetzt anlegen' }}
            </button>
            <small class="hint">
              Erstellt in jedem aktivierten Modul ein Projekt mit dem Kunden-Namen (idempotent — bestehende bleiben).
            </small>
            <div v-if="syncResult" class="hint" :style="{ color: syncResult.error ? '#c62828' : '#2e7d32' }">
              {{ syncResult.message }}
            </div>
          </div>
        </fieldset>

        <!-- Issue #434: Kundenbezogene Module (1:1) — Status + Direkt-Link -->
        <fieldset v-if="!creating && form.name">
          <legend>Modul-Bewertungen (1 pro Kunde)</legend>
          <p class="hint">
            NIS2, AI Act und DSGVO werden pro Kunde geführt
            (1:1). Aktiviere oben das Modul, dann erscheint hier der Link
            zur Bewertung.
          </p>
          <table class="sub-table">
            ...
            <tbody>
              <tr v-for="m in customerModuleLinks" :key="m.key">
                <td><strong>{{ m.label }}</strong></td>
                <td>
                  <span v-if="m.enabled" class="badge-ok">aktiv</span>
                  <span v-else class="badge-off">nicht aktiv</span>
                </td>
                <td>
                  <router-link v-if="m.enabled" :to="m.route" class="btn-small">
                    Öffnen →
                  </router-link>
        ...
        <!-- Issue #433: RB-Projekte fuer diesen Kunden (mehrere moeglich) -->
        <fieldset v-if="!creating && form.name && form.modules.risikobewertung">
          <legend>Risikobewertung-Projekte</legend>
          <p class="hint">
            Pro Kunde sind mehrere Risikobewertungs-Projekte möglich
            (z. B. für unterschiedliche Produkte / Services).
          </p>
          <table v-if="rbProjekte.length > 0" class="sub-table">
            ...
            <tbody>
              <tr v-for="p in rbProjekte" :key="p.name">
                ...
                <td>
                  <router-link :to="`/risikobewertung?projekt=${encodeURIComponent(p.name)}`"
                               class="btn-small">
                    Öffnen →
                  </router-link>
                </td>
          ...
          <div v-else class="sub-empty">
            Noch keine Risikobewertungs-Projekte vorhanden.
          </div>
          <router-link
            :to="`/risikobewertung?neu=${encodeURIComponent(form.name)}`"
            class="btn-secondary"
            style="display: inline-block; margin-top: 8px;">
            + Risikobewertungs-Projekt für diesen Kunden anlegen
          </router-link>
        </fieldset>
      ...
      <div v-if="activeTab === 'cra'" class="tab-content">
        ...
        <fieldset>
          ...
          <p class="hint">
            Mehrere Produkte pro Kunde. Standardprodukt für CRA-Bewertungen markieren.
            Jedes Produkt erzeugt automatisch ein CRA-Projekt (Issue #435).
          </p>
          ...
          <table v-if="kunden.produkte.length > 0" class="sub-table">
            ...
            <tbody>
              <tr v-for="p in kunden.produkte" :key="p.id">
                ...
                <td>{{ p.is_default ? '★' : '' }}</td>
                <td>
                  <router-link v-if="craProjektFuerProdukt(p)"
                               :to="`/cra?projekt=${encodeURIComponent(craProjektFuerProdukt(p))}`"
                               class="btn-small">
                    CRA öffnen →
                  </router-link>
                  <span v-else class="muted">—</span>
...
// Issue #430: Sync-Modul-Projekte fuer Bestandskunden
const syncing = ref(false)
const syncResult = ref<{ message: string; error?: boolean } | null>(null)

// Issue #433: RB-Projekte pro Kunde (1:n) anzeigen
const rbProjekte = ref<Array<{
  name: string; framework: string; produkt: string; risiken_count: number;
}>>([])

// Issue #434: Direkt-Links zu den kundenbezogenen Modulen (1:1)
const customerModuleLinks = computed(() => {
  const n = encodeURIComponent(form.value.name || '')
  return [
    {
      key: 'nis2', label: 'NIS2',
      enabled: !!form.value.modules?.nis2,
      route: `/nis2?projekt=${n}`,
    },
    {
      key: 'ai_act', label: 'AI Act',
      enabled: !!form.value.modules?.ai_act,
      route: `/aiact?projekt=${n}`,
    },
    {
      key: 'dsgvo', label: 'DSGVO',
      enabled: !!form.value.modules?.dsgvo,
      route: `/dsgvo?projekt=${n}`,
    },
  ]
})

const loadRbProjekte = async (kundeName: string) => {
  if (!kundeName) { rbProjekte.value = []; return }
  try {
    const res = await apiClient.get(`/kunden/${encodeURIComponent(kundeName)}/rb-projekte`)
...
// Issue #435: CRA-Projekte pro Kunde (1 pro Produkt) — fuer die
...
const loadCraProjekte = async (kundeName: string) => {
  if (!kundeName) { craProjekte.value = []; return }
  try {
    const res = await apiClient.get(`/kunden/${encodeURIComponent(kundeName)}/cra-projekte`)
...
const craProjektFuerProdukt = (produkt: any): string => {
  // Default-Produkt → CRA-Projekt heisst wie der Kunde
  // Sonst → "<Kunde> – <Produkt>"
  const kunde = form.value.name
  if (!kunde) return ''
  if (produkt.is_default) {
    const hit = craProjekte.value.find(p => p.name === kunde)
    return hit ? hit.name : ''
  }
  const composite = `${kunde} – ${produkt.name}`
...
const onSyncProjekte = async () => {
  ...
  try {
    const res = await apiClient.post(`/kunden/${encodeURIComponent(form.value.name)}/sync-projekte`)
...
watch(() => kunden.selectedKunde, async (k) => {
  if (k) {
    ...
    await Promise.all([
      ...
      loadRbProjekte(k.name),  // #433: RB-Projekte fuer diesen Kunden
