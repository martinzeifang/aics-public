  ...
  <div class="kunden-view">
    <div v-if="!kunden.selectedKunde && !creating" class="empty-state">
      ...
      <div class="empty-actions">
        ...
        <ModuleHelpButton module="kunden" />
      </div>
    </div>
    ...
    <div v-else class="cockpit">
      ...
      <div class="cockpit-header">
        <div class="cockpit-title">
          <h2>{{ creating ? 'Neuer Kunde' : (form.name || 'Kunde') }}</h2>
      ...
      <section class="card" v-if="!creating && form.name">
        ...
        <div v-if="subTab === 'risiko'" class="subpanel">
          <p class="hint">Pro Kunde mehrere Risikobewertungs-Projekte möglich (z. B. je Produkt/Service).</p>
        ...
        <div v-if="subTab === 'produkte'" class="subpanel">
          <p class="hint">Mehrere Produkte pro Kunde. Jedes Produkt erzeugt automatisch ein CRA-Projekt.</p>
...
const subTabs = computed(() => [
  ...
  { id: 'produkte' as const, label: 'Produkte (CRA)', count: kunden.produkte.length },
  { id: 'evidence' as const, label: 'Nachweise', count: kunden.evidence.length },
...
const moduleTiles = computed(() => {
  ...
  return [
    { key: 'risikobewertung' as const, label: 'Risikobewertung', desc: 'Multi-Framework-Risiko-Editor',
      enabled: !!mods.risikobewertung, route: `/risikobewertung?kunde=${n}`,
    ...
    { key: 'cra' as const, label: 'CRA-Readiness', desc: 'Cyber Resilience Act',
      ...
      statusLabel: kunden.produkte.length ? `${kunden.produkte.length} Produkt(e)` : 'aktiv' },
    ...
    { key: 'gutachten' as const, label: 'Gutachten', desc: 'Expert Opinions',
      enabled: !!mods.gutachten, route: `/gutachten?kunde=${n}`,
...
/* ===== Cockpit (Sprint: Kundenverwaltung-UX) ===== */
