<template>
  <div class="gg-view">
    ...
    <div v-else-if="creating" class="card">
      ...
      <div class="form-grid">
        <label>Projekt-Name * <input v-model="newForm.name"
               :placeholder="newForm.gutachten_art === 'gericht' ? 'z.B. GG-2026-007' : 'z.B. PG-2026-007'" /></label>
      ...
      <div class="actions">
        ...
        <button class="btn-link" :disabled="!newForm.klaeger_name" @click="runBefangCheck">⚠ Befangenheits-Check</button>
    ...
    <div v-else-if="aktuell" class="editor">
            ...
            <tr v-for="a in store.assets" :key="a.id">
              ...
              <td>
                ...
                <a :href="`/api/gutachten/gerichts/assets/${a.id}/sicherungsprotokoll.pdf`"
                   target="_blank" class="btn-link" title="Sicherungsprotokoll">📄</a>
      ...
      <div v-if="tab === 'honorar'" class="tab-content">
        ...
        <a :href="`/api/gutachten/gerichts/${encodeURIComponent(aktuell.name)}/rechnung.pdf?auftraggeber=${encodeURIComponent(aktuell.gericht)}`"
           target="_blank" class="btn-secondary">📄 Rechnung-PDF</a>
        <table class="tbl">
    ...
    <!-- #690 DOCX-Vorschau -->
    <div v-if="previewOpen" class="modal-bg" @click.self="previewOpen = false">
      <div class="modal preview-modal">
        ...
        <div class="preview-body word-page">
          ...
          <div v-for="b in store.befunde" :key="b.id" style="margin: 12px 0;">
            ...
            <div v-html="b.beschreibung_text"></div>
          </div>
    ...
    <!-- Help-Modal (#671) -->
    <div v-if="helpModal.open" class="modal-bg" @click.self="closeHelp">
    ...
    <!-- Wizard-Modal: Beurteilung -->
    <div v-if="wizardModal.open" class="modal-bg" @click.self="closeWizard">
      <div class="modal">
        <h3>🤖 Beurteilungs-Wizard</h3>
        ...
        <label>Prompt (in ChatGPT kopieren)</label>
        <textarea readonly :value="wizardModal.prompt" rows="6" class="mono"></textarea>
        ...
        <div class="actions">
          ...
          <button class="btn-primary" :disabled="!wizardModal.response" @click="parseWizardResponse">In Editor übernehmen</button>
        </div>
      </div>
    </div>
  </div>
...
const wizardModal = ref<any>({ open: false, prompt: '', response: '' })
...
const runBefangCheck = async () => {
  if (!newForm.value.klaeger_name) return
...
const openBeurteilungsWizard = async () => {
  ...
  wizardModal.value = { open: true, prompt, response: '' }
}
const closeWizard = () => { wizardModal.value = { open: false, prompt: '', response: '' } }
const copyPrompt = () => navigator.clipboard?.writeText(wizardModal.value.prompt)
const parseWizardResponse = async () => {
  const parsed = await store.beurteilungParse(aktuell.value.name, wizardModal.value.response, newBeurt.value.norm_id, null)
  if (parsed?.parsed) {
