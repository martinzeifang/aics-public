    ...
    <div class="modal-content modal-wide">
      ...
      <div class="modal-body">
        ...
        <div v-if="step === 2">
          ...
          <!-- Kontext-Quellen (Repo / Anhänge / Doku-URLs) — #763 -->
          <RisikoKontextPanel v-if="projektName" :projekt-name="projektName" />
          <div class="form-row context-flags">
            <label class="flag"><input type="checkbox" v-model="form.use_repo" /> Repo-Kontext einbeziehen</label>
            <label class="flag"><input type="checkbox" v-model="form.use_attachments" /> Anhänge einbeziehen</label>
            <label class="flag"><input type="checkbox" v-model="form.use_doc_urls" /> Doku-URLs einbeziehen</label>
          </div>
          <!-- #1045: AI-Act-Projektkontext für Bewertungsart EU-AI-Act -->
          <div v-if="isEuAiAct" class="form-row aiact-ctx">
            <label>AI-Act-Projekt (für konkreten Systemkontext):</label>
            <input v-model="form.aiact_projekt" placeholder="Name des AI-Act-Projekts" />
            <small class="hint">Vorbelegt aus der Verknüpfung. Liefert Zweck, Architektur &amp;
              Risk-Tier in den Prompt — gegen generische Risiken.</small>
          </div>
        </div>
...
import RisikoKontextPanel from './RisikoKontextPanel.vue'
...
const form = reactive({
  ...
  beschreibung: '',
  n_risiken: 10,
  // #763 Kontext-Quellen serverseitig ergänzen
  use_repo: false,
  use_attachments: false,
  use_doc_urls: false,
  aiact_projekt: '',  // #1045: AI-Act-Projektkontext für Bewertungsart EU-AI-Act
})

const isEuAiAct = ref(false)
...
const reset = () => {
  ...
  form.n_risiken = 10
  form.use_repo = false
  form.use_attachments = false
  form.use_doc_urls = false
  form.aiact_projekt = ''
...
watch(() => props.open, async (o) => {
  if (o) {
    ...
    // #1045: Bei Bewertungsart EU-AI-Act das verknüpfte AI-Act-Projekt vorbelegen
    isEuAiAct.value = false
    if (props.projektName) {
      try {
        const pr = await apiClient.get(`/risikobewertung/projekte/${encodeURIComponent(props.projektName)}`)
        const proj = pr.data || {}
        isEuAiAct.value = proj.framework === 'EU-AI-Act'
        // #1046: linked_aiact_projekt wird direkt serialisiert (nicht über meta)
        if (isEuAiAct.value) form.aiact_projekt = proj.linked_aiact_projekt || ''
      } catch { /* ignore */ }
    }
  }
})
    ...
    try {
      const res = await apiClient.post(
        `/risikobewertung/projekte/${encodeURIComponent(props.projektName)}/risiken/discovery-prompt`,
        {
          ...
          use_repo: form.use_repo,
          use_attachments: form.use_attachments,
          use_doc_urls: form.use_doc_urls,
          aiact_projekt: form.aiact_projekt,
        },
      )
...
.context-flags { display: flex; gap: 16px; flex-wrap: wrap; margin-top: 8px; }
.context-flags .flag { display: flex; align-items: center; gap: 6px; font-weight: 500; font-size: 13px; }
.context-flags .flag input { width: auto; }
