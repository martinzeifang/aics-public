      ...
      <div class="modal-body">
        ...
        <div class="mode-tabs">
          ...
          <button :class="['mode-tab', { active: mode === 'chatgpt' }]" @click="mode = 'chatgpt'">
            📝 Manuell (1 Prompt je Risiko)
          </button>
          <button :class="['mode-tab', { active: mode === 'batch' }]" @click="mode = 'batch'">
            📋 Ein Prompt (mehrere)
          </button>
        </div>
        ...
        <!-- #1048: EIN Sammel-Prompt für mehrere Risiken, mit Review/Bestätigung -->
        <div v-if="mode === 'batch'" class="tab-content">
          <div class="info-banner">
            ℹ️ Bewertet mehrere Risiken über <strong>einen einzigen Prompt</strong>. Ablauf wie bei der
            Einzelbewertung: Prompt kopieren → in ChatGPT → die JSON-Array-Antwort einfügen →
            <strong>Vorschau prüfen → bestätigen</strong>. Es wird nichts ohne Bestätigung gespeichert.
          </div>

          <div class="step">
            <h4>Schritt 1: Sammel-Prompt generieren</h4>
            <p>Erzeugt EINEN Prompt für die <strong>{{ openCount }}</strong> offenen Risiken
              (empfohlen ≤ 10–15 pro Durchgang).</p>
            <button class="btn-secondary" @click="genBatchPrompt" :disabled="busy">
              {{ busy ? 'Generiert…' : 'Sammel-Prompt generieren' }}
            </button>
          </div>

          <div v-if="batchPrompt" class="step">
            <h4>Schritt 2: Prompt → ChatGPT → Antwort einfügen</h4>
            <div class="prompt-header">
              <strong>{{ batchCount }} Risiken in einem Prompt</strong>
              <button class="btn-small" @click="copyPrompt(batchPrompt)">📋 Prompt kopieren</button>
            </div>
            <details><summary>Prompt anzeigen</summary><pre class="prompt-text">{{ batchPrompt }}</pre></details>
            <textarea
              v-model="batchResponse"
              class="batch-response"
              rows="8"
              placeholder='JSON-Array der KI-Antwort hier einfügen, z.B. [{"nr":1,"felder":{...},"bewertung":"..."}]'
            ></textarea>
            <div class="batch-actions">
              <button class="btn-secondary" @click="previewBatch" :disabled="busy || !batchResponse.trim()">
                {{ busy ? 'Prüft…' : '🔍 Vorschau anzeigen' }}
              </button>
            </div>
          </div>

          <div v-if="batchPreview.length > 0" class="step">
            <h4>Schritt 3: Vorschau prüfen &amp; bestätigen</h4>
            <p>{{ selectedBatchCount }} von {{ batchPreview.length }} ausgewählt.
              <button class="btn-small" @click="toggleAllBatch(true)">Alle</button>
              <button class="btn-small" @click="toggleAllBatch(false)">Keine</button>
            </p>
            <table class="results">
              <thead>
                <tr><th></th><th>Nr</th><th>Risiko</th><th>Score</th><th>Level</th><th>Bewertung</th></tr>
              </thead>
              <tbody>
                <tr v-for="a in batchPreview" :key="a.risk_id">
                  <td><input type="checkbox" v-model="a._sel" /></td>
                  <td>{{ a.nr }}</td>
                  <td>{{ a.risk_name }}</td>
                  <td>{{ a.risikowert ?? '—' }}</td>
                  <td>{{ a.risiko_label || '—' }}</td>
                  <td class="bew-cell">{{ (a.bewertung_text || '').slice(0, 140) }}</td>
                </tr>
              </tbody>
            </table>
            <p v-if="batchUnmatched.length" class="hint">⚠️ Nicht zugeordnet (nr): {{ batchUnmatched.join(', ') }}</p>
            <button class="btn-primary" @click="applyBatch" :disabled="busy || selectedBatchCount === 0">
              {{ busy ? 'Übernimmt…' : `${selectedBatchCount} Bewertungen übernehmen` }}
            </button>
            <div v-if="batchApplyResult" class="apply-result">
              <strong>{{ batchApplyResult.applied_count }} übernommen.</strong>
              <span v-if="batchApplyResult.errors.length > 0">{{ batchApplyResult.errors.length }} Fehler.</span>
            </div>
          </div>
        </div>
...
const mode = ref<'ollama' | 'chatgpt' | 'batch'>('ollama')
...
// #1048: Sammel-Prompt-Modus (ein Prompt, mehrere Bewertungen, mit Bestätigung)
const batchPrompt = ref('')
const batchCount = ref(0)
const batchResponse = ref('')
const batchPreview = ref<any[]>([])
const batchUnmatched = ref<number[]>([])
const batchApplyResult = ref<any | null>(null)
const selectedBatchCount = computed(() => batchPreview.value.filter(a => a._sel).length)

const _pjBase = () => `/risikobewertung/projekte/${encodeURIComponent(props.projektName)}/risiken`
...
// ── #1048: Sammel-Prompt-Flow (Prompt → Vorschau → bestätigen) ──────────
const genBatchPrompt = async () => {
  busy.value = true; error.value = ''
  batchPrompt.value = ''; batchResponse.value = ''
  batchPreview.value = []; batchUnmatched.value = []; batchApplyResult.value = null
  try {
    const res = await apiClient.post(`${_pjBase()}/mass-assess-prompt`, { only_open: true })
    batchPrompt.value = res.data.prompt
    batchCount.value = res.data.count
  } catch (e: any) {
    error.value = e?.response?.data?.error || 'Fehler beim Generieren des Sammel-Prompts'
  } finally {
    busy.value = false
  }
}
...
const previewBatch = async () => {
  ...
  batchPreview.value = []; batchUnmatched.value = []; batchApplyResult.value = null
  try {
    const res = await apiClient.post(`${_pjBase()}/mass-assess-preview`, { raw: batchResponse.value })
    batchPreview.value = (res.data.assessments || []).map((a: any) => ({ ...a, _sel: true }))
    batchUnmatched.value = res.data.unmatched || []
    if (batchPreview.value.length === 0) {
      error.value = 'Keine zuordenbaren Bewertungen in der Antwort gefunden.'
    }
  } catch (e: any) {
    error.value = e?.response?.data?.error || 'Fehler bei der Vorschau (gültiges JSON-Array eingefügt?)'
  } finally {
    busy.value = false
  }
}

const toggleAllBatch = (v: boolean) => batchPreview.value.forEach(a => { a._sel = v })

const applyBatch = async () => {
  busy.value = true; error.value = ''; batchApplyResult.value = null
  const chosen = batchPreview.value
    .filter(a => a._sel)
    .map(a => ({ risk_id: a.risk_id, felder: a.felder, bewertung_text: a.bewertung_text }))
  try {
    const res = await apiClient.post(`${_pjBase()}/mass-assess-apply`, { assessments: chosen })
...
/* #1052: Batch-Sammelprompt — JSON-Feld volle Breite, Button sauber darunter */
.batch-response {
  display: block;
  width: 100%;
...
.bew-cell { color: #555; font-size: 11px; max-width: 260px; }
