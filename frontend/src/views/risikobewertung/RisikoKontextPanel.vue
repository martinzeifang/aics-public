      ...
      <div class="grid">
        ...
        <label>Access-Token (optional)
          <input v-model="tokenInput" type="password" autocomplete="off"
                 :placeholder="hasToken ? '•••••• gespeichert — leer lassen zum Beibehalten' : 'Personal Access Token'" />
        </label>
      </div>
      <p class="hint">
        Der Token wird <em>verschlüsselt</em> gespeichert und für den Repo-Zugriff verwendet
        (nötig für GitHub, sonst Rate-Limit).
        <span v-if="hasToken" class="ok-inline">✓ Token gespeichert</span>
      </p>
      <div class="actions">
...
const vcs = reactive<VcsPublish>({ provider: 'github', repo: '', base_url: '' })
const software = reactive<SoftwareMeta>({ description: '', doc_urls: [] })
const tokenInput = ref('')      // #784: eingegebener Token-Wert (wird verschlüsselt gespeichert)
const hasToken = ref(false)     // ob bereits ein Token hinterlegt ist
...
async function load() {
  ...
  try {
    ...
    const v = p.data.vcs_publish || {}
    Object.assign(vcs, { provider: 'github', repo: '', base_url: '', ...v })
...
async function saveRepo() {
  busy.value = true
  try {
    // Token nur mitschicken, wenn neu eingegeben (leer = bestehenden beibehalten).
    const payload: any = { ...vcs }
    if (tokenInput.value.trim()) payload.token = tokenInput.value.trim()
    const r = await apiClient.put(`${base()}/repo-config`, { vcs_publish: payload })
...
.ok-inline { color: #166534; font-weight: 600; margin-left: 6px; }
