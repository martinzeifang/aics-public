...
const newProjektForm = ref({ name: '', framework: 'STRIDE', beschreibung: '' })
...
const startNewProjekt = () => {
  newProjektForm.value = { name: '', framework: 'STRIDE', beschreibung: '' }
...
const onCreateProjekt = async () => {
  ...
  const result = await rb.createProjekt(newProjektForm.value)
  if (result) {
