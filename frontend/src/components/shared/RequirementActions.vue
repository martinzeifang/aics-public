...
const doAutoBewertung = async () => {
  if (!props.requirement?.id) return
  autoConfirm.value.open = false
  busy.value = 'auto'
  autoError.value = ''
  try {
    const res = await apiClient.post(`${baseUrl.value}/auto-bewertung`)
    emit('saved', res.data)
  } catch (e: any) {
    autoError.value = e?.response?.data?.error || 'Automatische Bewertung fehlgeschlagen'
    emit('error', autoError.value)
  } finally {
    busy.value = ''
  }
}
