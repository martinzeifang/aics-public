  ...
  <div class="app-layout">
    ...
    <header class="app-header">
      <div class="header-left">
        ...
        <div class="header-title" :class="{ 'header-title-soc': portalStore.isSoc }">
          <h1>{{ portalStore.isSoc ? portalStore.portalName : 'AI Compliance Suite' }}</h1>
          <p>{{ portalStore.isSoc ? 'Security Operations Center' : 'Compliance Management Platform' }}</p>
        </div>
      </div>
    ...
    <div class="main-container">
      ...
      <main class="app-main">
        <!-- #1412: Portal-Modus, aber kein SOC-Zugriff → freundlicher Hinweis statt Redirect-Schleife -->
        <div v-if="portalNoAccess" class="portal-noaccess">
          <div class="pna-icon" aria-hidden="true">🛡️</div>
          <h2>Kein Zugriff auf das SOC</h2>
          <p>Ihr Konto hat keine Berechtigung für das Security Operations Center.
             Bitte wenden Sie sich an Ihre Administration, um die Rolle
             <code>SOC-Operator</code> (oder die Berechtigung <code>soc:read</code>) zu erhalten.</p>
          <button class="pna-logout" @click="logout">Abmelden</button>
        </div>
...
import FirmenSidebar from './sidebars/FirmenSidebar.vue'
import RisikobewertungSidebar from './sidebars/RisikobewertungSidebar.vue'
import CRASidebar from './sidebars/CRASidebar.vue'
import WiBASidebar from './sidebars/WiBASidebar.vue'
import SOCSidebar from './sidebars/SOCSidebar.vue'
...
// #1412: Portal-Modus + angemeldet, aber keine SOC-Berechtigung → Hinweis statt leerer/Loop-Ansicht.
const portalNoAccess = computed(() =>
  portalStore.isSoc && authStore.isAuthenticated && !authStore.hasPermission('soc:read'))

const visibleModules = computed(() => {
  let list = modules
  // #1412: SOC-Portal — höchstprioritäres Gating: NUR das SOC-Modul, alles andere aus.
  if (portalStore.isSoc) {
    return list.filter(m => m.id === 'soc')
  }
  ...
  //   - 'firmen' ist immer sichtbar (Master-Modul)
  ...
  //   - leere Liste / nicht-Array (= keine Lizenz aktiv): nur 'firmen'
...
const modules = [
  ...
  { id: 'wiba', label: 'WiBA', tooltip: 'BSI Weg in die Basis-Absicherung' },
  ...
  { id: 'soc', label: 'SOC', tooltip: 'Security Operations Center — Wazuh-Alarme & Incidents' },
]
...
const currentSidebarComponent = computed(() => {
  return sidebarComponents[currentModule.value] || FirmenSidebar
})
...
/* #1469: Icons in den runden Header-Buttons (Dark-Mode, Benutzer) wurden als
   Flex-Items horizontal zerquetscht (Firefox: flex-shrink:1 + min-width:auto →
   SVG-Breite kollabierte auf ~5px). flex:none + feste Maße halten sie bei 20px. */
.btn-icon svg, .btn-user svg { flex: none; width: 20px; height: 20px; }
...
/* #1412: SOC-Portal — „kein Zugriff"-Hinweis */
.portal-noaccess { max-width: 480px; margin: 8vh auto 0; text-align: center; padding: 32px 24px; }
.portal-noaccess .pna-icon { font-size: 48px; margin-bottom: 12px; }
.portal-noaccess h2 { color: #b71c1c; margin: 0 0 12px; }
.portal-noaccess p { color: #546e7a; line-height: 1.55; margin: 0 0 18px; }
.portal-noaccess .pna-logout { background: #1565c0; color: #fff; border: none; border-radius: 6px; padding: 8px 18px; cursor: pointer; font-size: 14px; }
