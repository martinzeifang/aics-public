...
export const useAuthStore = defineStore('auth', () => {
  ...
  const _finishLogin = async (data: any) => {
    ...
    // FOLLOW-UP #740 (WP-07): Auth-Token liegt derzeit in sessionStorage und ist
    // damit für JS lesbar. Migration auf httpOnly-Cookie ist als separater, größerer
    // Schritt geplant (berührt Auth-Flow + Tests) und bewusst NICHT Teil dieses PRs.
  ...
  const logout = async () => {
    // #1049: Lokale Session ZUERST synchron verwerfen, damit App.vue
    // (v-if="isAuthenticated") AppLayout sofort ausblendet — sonst bleiben
    // Menü + Inhalt während des await sichtbar, obwohl die Anmeldemaske erscheint.
    ...
    // Backend best-effort benachrichtigen (Token-Blacklist) — nicht blockierend fürs UI.
    if (prevToken) {
      try {
        ...
        // Ignore — lokaler Logout ist bereits erfolgt
      }
    }
  }
