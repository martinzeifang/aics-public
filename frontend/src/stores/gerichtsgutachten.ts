...
export const useGerichtsgutachtenStore = defineStore('gerichtsgutachten', () => {
  ...
  // ── Befangenheits-Check ────────────────────────────────
  const befangenheitsCheck = async (firma: string, system = '', parteien: string[] = [], svUser = '') => {
    try {
      const res = await apiClient.post('/gutachten/befangenheits-check', { firma, system, partei_namen: parteien, sv_user: svUser })
      return res.data
    } catch (e) { _err(e); return null }
  }
  ...
  // ── #983/#984 Glossar ──────────────────────────────────
  const glossar = ref<any[]>([])
  const _gl = (n: string) => `/gutachten/gerichts/${encodeURIComponent(n)}/glossar`
  const fetchGlossar = async (n: string) => {
    try { glossar.value = (await apiClient.get(_gl(n))).data?.glossar || [] } catch (e) { _err(e) }
  }
  const generateGlossar = async (n: string) => {
    try { const r = await apiClient.post(`${_gl(n)}/generate`); glossar.value = r.data?.glossar || []; return r.data?.stats }
    catch (e) { _err(e); return null }
  }
  const saveGlossarEntry = async (n: string, data: any) => {
    try {
      if (data.id) await apiClient.put(`${_gl(n)}/${data.id}`, data)
      else await apiClient.post(_gl(n), data)
      await fetchGlossar(n); return true
    } catch (e) { _err(e); return false }
  }
  const deleteGlossarEntry = async (n: string, id: number) => {
    try { await apiClient.delete(`${_gl(n)}/${id}`); await fetchGlossar(n); return true } catch (e) { _err(e); return false }
  }
  ...
  const downloadDocx = async (name: string, includeAnhang = true, templateId: number | null = null,
                              opts: { methodeWerkzeug?: boolean; beurteilungSubheadings?: boolean; verfahrenDatum?: boolean; glossar?: boolean } = {}) => {
    try {
      ...
      if (opts.glossar === false) params.set('include_glossar', 'false')
      ...
      a.href = url
      // #1000: <Datum>_<Projekt>_<PG|GG>(_vorlage|_ohne-anhang).docx — wie der Server
      const today = new Date().toISOString().slice(0, 10)
      const safe = (name || 'Gutachten').replace(/[^A-Za-z0-9._-]+/g, '_').replace(/^_+|_+$/g, '') || 'Gutachten'
      const art = aktuell.value?.gutachten_art === 'privat' ? 'PG' : 'GG'
      const sfx = templateId ? '_vorlage' : (includeAnhang ? '' : '_ohne-anhang')
      a.download = `${today}_${safe}_${art}${sfx}.docx`
      a.click()
  ...
  return {
    ...
    glossar, fetchGlossar, generateGlossar, saveGlossarEntry, deleteGlossarEntry,
