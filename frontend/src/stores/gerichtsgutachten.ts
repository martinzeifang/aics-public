...
export const useGerichtsgutachtenStore = defineStore('gerichtsgutachten', () => {
  ...
  // ── DOCX + Archiv ──────────────────────────────────────
  const downloadDocx = async (name: string) => {
    try {
      const res = await apiClient.get(`/gutachten/gerichts/${encodeURIComponent(name)}/docx`, { responseType: 'blob' })
      const url = URL.createObjectURL(res.data)
      const a = document.createElement('a')
      a.href = url
      a.download = `Gerichtsgutachten_${name}.docx`
      a.click()
  ...
  return {
    ...
    saveBeweisfrage, saveBefund, saveBeurteilung, saveAsset, saveVerfahren,
