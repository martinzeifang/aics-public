...
export const useDsgvoStore = defineStore('dsgvo', () => {
  ...
  // #1101: VVT mit optionalem Rollen-Filter (Art. 30(1)/30(2))
  const vvtRolleFilter = ref<'' | 'verantwortlicher' | 'auftragsverarbeiter'>('')
  const fetchVvt = async () => {
    try {
      const q = vvtRolleFilter.value ? `?rolle=${encodeURIComponent(vvtRolleFilter.value)}` : ''
      vvt.value = (await apiClient.get(_pjUrl(`/vvt${q}`))).data || []
    } catch (e: any) { error.value = e?.response?.data?.error || 'Fehler' }
  }
  const setVvtRolleFilter = async (r: '' | 'verantwortlicher' | 'auftragsverarbeiter') => {
    vvtRolleFilter.value = r
    await fetchVvt()
  }
  ...
  // DSFA ↔ Risikobewertung (#1084/#1085): Risiken(c)+Maßnahmen(d) READ-ONLY
  // aus dem verknüpften rb_projekt. Art. 35(7)a+b bleiben lokal.
  const fetchDsfaRiskLink = async (dpiaRowId: number) => {
    try {
      return (await apiClient.get(_pjUrl(`/dpia/${dpiaRowId}/risk-link`))).data
    } catch (e: any) {
      error.value = e?.response?.data?.error || 'Fehler beim Laden der DSFA-Verknüpfung'
      return null
    }
  }

  // ── Sprint #23 DSFA-Rework (DS5 #1105 / DS6 #1106 / DS7 #1107) ──────────
  const dsfaSchwellwertKriterien = ref<any | null>(null)

  // DS5: Kriterienkatalog (Art. 35 Abs. 1/3/4) — statisch
  const fetchSchwellwertKriterien = async () => {
    try {
      dsfaSchwellwertKriterien.value =
        (await apiClient.get('/dsgvo/dsfa/schwellwert-kriterien')).data
      return dsfaSchwellwertKriterien.value
    } catch (e: any) {
      error.value = e?.response?.data?.error || 'Fehler beim Laden der Schwellwert-Kriterien'
      return null
    }
  }

  // DS5: Schwellwert für ein DSFA auswerten + persistieren
  const saveDsfaSchwellwert = async (dpiaRowId: number, payload: any) => {
    try {
      return (await apiClient.post(_pjUrl(`/dpia/${dpiaRowId}/schwellwert`), payload)).data
    } catch (e: any) {
      error.value = e?.response?.data?.error || 'Fehler beim Speichern der Schwellwertanalyse'
      return null
    }
  }

  // DS6: Stufenwechsel im DSFA-Prozess
  const setDsfaStage = async (dpiaRowId: number, stage: string) => {
    try {
      return (await apiClient.post(_pjUrl(`/dpia/${dpiaRowId}/stage`), { stage })).data
    } catch (e: any) {
      error.value = e?.response?.data?.error || 'Fehler beim Wechsel der DSFA-Stufe'
      return null
    }
  }
  ...
  return {
    ...
    vvtRolleFilter, setVvtRolleFilter,
    ...
    fetchDsfaRiskLink,
    // Sprint #23 DSFA-Rework (DS5/DS6/DS7)
    dsfaSchwellwertKriterien,
    fetchSchwellwertKriterien, saveDsfaSchwellwert, setDsfaStage,
    // Phase B
    branchenTemplates,
