...
export const useNis2Store = defineStore('nis2', () => {
  ...
  const getWizardPrompt = async (wizard: 'klassifikator' | 'incident-notification' | 'supply-chain', body: any = {}) => {
    try {
  ...
  const parseWizardResponse = async (wizard: 'klassifikator' | 'incident-notification' | 'supply-chain', response: string, extra: any = {}, apply = true) => {
    try {
