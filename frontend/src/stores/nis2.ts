...
export const useNis2Store = defineStore('nis2', () => {
  ...
  const getWizardPrompt = async (wizard: 'klassifikator' | 'incident-notification' | 'supply-chain' | 'incident-24h' | 'incident-72h' | 'incident-final' | 'cyberhygiene-quiz' | 'vendor-tiering', body: any = {}) => {
    try {
      const url = _pjUrl(`/wizards/${wizard}/prompt`)
      const res = wizard === 'klassifikator'
        ? await apiClient.get(url)
        : await apiClient.post(url, body)
      return res.data?.prompt || ''
    } catch (e: any) { error.value = e?.response?.data?.error || 'Fehler'; return '' }
  }
  const parseWizardResponse = async (wizard: 'klassifikator' | 'incident-notification' | 'supply-chain' | 'incident-24h' | 'incident-72h' | 'incident-final' | 'cyberhygiene-quiz' | 'vendor-tiering', response: string, extra: any = {}, apply = true) => {
    try {
