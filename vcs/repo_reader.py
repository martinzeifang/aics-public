...
def _gh_api(path: str) -> dict | list:
    args = ["gh", "api", path]
    p = subprocess.run(args, capture_output=True, text=True, encoding="utf-8", timeout=30)
    if p.returncode != 0:
...
def _fetch_github(owner_repo: str, *, max_readme_chars: int = 3000, max_tree_entries: int = 120) -> RepoContext:
    ...
    readme = ""
    try:
        rm = _gh_api(f"repos/{owner}/{repo}/readme")
    ...
    file_tree: list[str] = []
    try:
        tree_data = _gh_api(f"repos/{owner}/{repo}/git/trees/{default_branch}?recursive=1")
