...
def _resolve_github_token(token: str | None = None) -> str | None:
    """Expliziter Token > GITHUB_TOKEN > GH_TOKEN env."""
    return token or os.environ.get("GITHUB_TOKEN") or os.environ.get("GH_TOKEN")


def _github_http_api(path: str, token: str | None = None) -> dict | list:
    """GitHub-API via HTTPS (api.github.com).

    Fester Host → SSRF-unkritisch. Ohne Token ist die API hart rate-limitet
    (von Server-/RZ-IPs oft sofort HTTP 403) — #782: dann klare Meldung.
    """
    ...
    try:
        ...
        if exc.code in (403, 429) and not token:
            raise RuntimeError(
                "GitHub-API-Limit erreicht (unauthentifizierte Anfrage). Bitte einen "
                "GitHub-Token hinterlegen — als Umgebungsvariable (Name = im Projekt "
                "konfiguriertes token_env, z.B. GH_TOKEN/GITHUB_TOKEN)."
            ) from exc
        raise


def _github_api(path: str, token: str | None = None) -> dict | list:
    """Mit Token: deterministische HTTP-API (nicht rate-limitet) bevorzugen.
    Ohne Token: gh-CLI versuchen (falls vorhanden+authentifiziert), sonst HTTP."""
    import shutil
    ...
    return _github_http_api(path, token=token)


def _fetch_github(owner_repo: str, *, token: str | None = None,
                  max_readme_chars: int = 3000, max_tree_entries: int = 120) -> RepoContext:
    owner, repo = owner_repo.split("/", 1)

    # Repo metadata
    meta = _github_api(f"repos/{owner}/{repo}", token=token)
    ...
    readme = ""
    try:
        rm = _github_api(f"repos/{owner}/{repo}/readme", token=token)
    ...
    file_tree: list[str] = []
    try:
        tree_data = _github_api(f"repos/{owner}/{repo}/git/trees/{default_branch}?recursive=1", token=token)
...
def _fetch_gitlab(owner_repo: str, base_url: str, *, token: str | None = None,
                  max_readme_chars: int = 3000, max_tree_entries: int = 120) -> RepoContext:
    token = token or os.environ.get("GITLAB_TOKEN") or os.environ.get("CI_JOB_TOKEN")
...
def fetch_repo_context(raw: str, *, token: str | None = None) -> RepoContext:
    """Fetch repository context from GitHub or GitLab.

    token: optionaler API-Token (z.B. aus dem konfigurierten token_env). Wird an
    den jeweiligen Provider durchgereicht (#782). Ohne Token greifen die
    GITHUB_TOKEN/GH_TOKEN- bzw. GITLAB_TOKEN-Env-Variablen.

    Raises ValueError on bad input, RuntimeError on fetch failure.
    """
    provider, owner_repo, canon_url = detect_provider(raw)
    if provider == "github":
        return _fetch_github(owner_repo, token=token)
    else:
        return _fetch_gitlab(owner_repo, canon_url, token=token)
