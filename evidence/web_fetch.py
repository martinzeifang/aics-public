...
def fetch_page(url: str, *, timeout: int = 15) -> FetchResult:
    ...
    # #741 (SSRF): URL + jeden Redirect-Hop validieren (kein Zugriff auf
    # Loopback/RFC1918/Link-Local/Cloud-Metadata 169.254.169.254).
    from shared.net_validation import SSRFError, safe_get
    try:
        resp = safe_get(url, headers=_HEADERS, timeout=timeout)
        resp.raise_for_status()
    except SSRFError as e:
        raise WebFetchError(f"URL aus Sicherheitsgründen blockiert: {e}")
