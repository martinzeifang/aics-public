...
def ensure_db(db_path: Path = DEFAULT_DB_PATH) -> None:
    with connect_sqlite(db_path, anchor=Path(__file__)) as con:
        ...
        for _tbl in ("evidence_documents", "evidence_mappings_approved"):
            cols = {r[1] for r in con.execute(f"PRAGMA table_info({_tbl})")}
            if "kunden_id" in cols and "firmen_id" not in cols:
                try:
        ...
        for _col_sql in [
            ...
            "ALTER TABLE evidence_documents ADD COLUMN firmen_id TEXT NOT NULL DEFAULT ''",
            "ALTER TABLE evidence_mappings_approved ADD COLUMN firmen_id TEXT NOT NULL DEFAULT ''",
        ]:
            try:
