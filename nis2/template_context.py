...
NIS2_VARIABLES: list[dict[str, Any]] = [
    ...
    {"key": "dokumente", "typ": "liste",
     "beschreibung": "Finalisierte/freigegebene gemanagte Dokumente des Projekts "
                     ...
                     "doc_mode, extern, external_url, external_label).",
     "pflicht": False},
    {"key": "dokument_verweise", "typ": "liste",
     "beschreibung": "Gebündelte interne Verweise/Links der Pflichtdokumente "
                     "(Playbook-URL, CSIRT-Kontakt, BCP-URL, Vendor-SLA/DPA); "
                     "je Eintrag label, url, quelle.", "pflicht": False},
]
...
def _dokumente(db_path: Path, projekt_name: str) -> list[dict[str, Any]]:
    ...
    #1241: Web-verlinkte Dokumente (``doc_mode='extern'``) werden mit ihrer URL
    als Verweis im Bericht referenzierbar (statt In-App-Inhalt).
    """
    try:
        ...
        for d in list_documents(db_path, _DOC_MODUL, projekt_name):
            ...
            doc_mode = _s(d.get("doc_mode")) or "inapp"
            out.append({
                ...
                # #1241: Web-Verknüpfung als Bericht-Referenz.
                "doc_mode": doc_mode,
                "extern": doc_mode == "extern",
                "external_url": _s(d.get("external_url")),
                "external_label": _s(d.get("external_label")),
            })
        return out
    except Exception:
        return []
...
def _dokument_verweise(incident_response: dict[str, Any],
                       bcp: dict[str, Any],
                       vendors: list[dict[str, Any]]) -> list[dict[str, Any]]:
    """#1241: gebündelte interne Verweise/Links der NIS2-Pflichtdokumente.

    Reuse bereits erfasster Felder (N3 Playbook/CSIRT, N5 BCP-URL, N4
    Vendor-SLA/DPA) — keine Neu-Erfassung. Jeder Eintrag hat ``label``,
    ``url`` und ``quelle`` (Pflichtdokument-Bezug).
    """
    out: list[dict[str, Any]] = []

    def _add(label: str, url: Any, quelle: str) -> None:
        u = _s(url).strip()
        if u:
            out.append({"label": label, "url": u, "quelle": quelle})

    ir = incident_response or {}
    _add("Incident-Playbook", ir.get("playbook_url"),
         "Incident-Handling-Konzept (Art. 21(2)b)")
    _add("CSIRT-Kontakt", ir.get("csirt_email"),
         "Incident-Handling-Konzept (Art. 21(2)b)")
    _add("BCP-Dokument", (bcp or {}).get("bcp_url"),
         "BCM-/DR-Plan (Art. 21(2)c)")
    for v in vendors or []:
        name = _s(v.get("vendor_name")) or "Lieferant"
        _add(f"SLA — {name}", v.get("sla_url"),
             "Lieferketten-Sicherheitsrichtlinie (Art. 21(2)d)")
        _add(f"DPA — {name}", v.get("dpa_url"),
             "Lieferketten-Sicherheitsrichtlinie (Art. 21(2)d)")
    return out
...
def build_nis2_context(db_path: Path | str, projekt_name: str) -> dict[str, Any]:
    ...
    return {
        ...
        # #1241: gebündelte interne Verweise/Links der Pflichtdokumente.
        "dokument_verweise": _dokument_verweise(incident_response, bcp, vendors),
    }
