...
def update_projekt_meta(db_path: Path, name: str, meta: dict[str, Any]) -> None:
    ...
    try:
        con.execute(
            "UPDATE nis2_projekte SET meta_json=?, updated_at=datetime('now') WHERE name=?",
        ...
        if aid:
            con.execute(
                """UPDATE nis2_asset_inventory SET asset_name=?, asset_typ=?, kritikalitaet=?,
                          ...
                          schutzbedarf_i=?, schutzbedarf_a=?, notizen=?, updated_at=datetime('now')
                   WHERE id=?""",
        ...
        if rid:
            con.execute(
                """UPDATE nis2_risiko_register SET titel=?, asset_ref=?, bedrohung=?,
                          ...
                          massnahmen=?, status=?, review_datum=?, notizen=?, updated_at=datetime('now')
                   WHERE id=?""",
        ...
        if vid:
            con.execute(
                """UPDATE nis2_supply_chain SET vendor_name=?, leistung=?, kritikalitaet=?,
                          ...
                          dpa_url=?, review_datum=?, status=?, notizen=?, updated_at=datetime('now')
                   WHERE id=?""",
