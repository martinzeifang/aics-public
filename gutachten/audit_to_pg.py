...
CREATE TABLE IF NOT EXISTS gutachten_audit_to_pg_log (
    ...
    konvertiert_am  TEXT NOT NULL DEFAULT (datetime('now')),
...
def get_audit_summary(db_path: Path, audit_projekt_name: str) -> dict[str, Any]:
    ...
    try:
        ...
        return {
            "name": projekt["name"],
            "kunde": projekt.get("meta", {}).get("unternehmen") or projekt.get("name", ""),
...
VORBEFASSUNG_WARNING = (
    ...
    "UND PG-Ersteller für diesen Kunden.\n\n"
...
def convert_audit_to_pg(
    ...
    _gdb.save_gerichts_projekt(
        ...
        name=pg_name,
        gutachten_art="privat",
        auftraggeber=summary.get("kunde", ""),
    ...
    _gdb.save_verfahrensereignis(
        ...
        beschreibung=(
            ...
            f"Auftraggeber: {summary.get('kunde', '')}. "
...
PG_TEMPLATES_PER_AUFTRAG = {
    "Tauglichkeitsprüfung": {
        "compliance": "Ist das Unternehmen {kunde} compliant mit {frameworks}?",
...
{{
  "fragen": [
    {{
      ...
      "frage_text": "Ist das Unternehmen '{audit_summary.get('kunde', 'X')}' compliant mit den Anforderungen von ...?"
    }}
  ]
}}
```
...
def generate_pg_thema_from_audit(audit_summary: dict[str, Any],
    ...
    kunde = audit_summary.get("kunde", "") or "(unbekannt)"
    ...
    return (
        f"{auftrags_art} der IT-Compliance des Unternehmens '{kunde}' "
