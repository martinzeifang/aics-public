...
- VII. Schlussformel
- VIII. Anhang (Asservaten + Werkzeuge)
...
def build_gerichtsgutachten_docx(projekt_name: str, db_path: Path) -> Document:
    ...
    _add_inhaltsverzeichnis(doc)
    _add_kapitel_2(doc, beweisfragen)
    _add_kapitel_3(doc, verfahren)
    _add_kapitel_4(doc, befunde)
    _add_kapitel_5(doc, beurteilungen)
...
def _add_deckblatt(doc: Document, projekt: dict[str, Any]) -> None:
    ...
    if projekt.get("thema"):
        _p(doc, "Thema des Gutachtens:" if not is_privat else "Thema:", bold=True)
...
def _add_inhaltsverzeichnis(doc: Document) -> None:
...
def _add_kapitel_2(doc: Document, beweisfragen: list[dict[str, Any]]) -> None:
    _h1(doc, "II. Untersuchungsauftrag")
    if not beweisfragen:
        _p(doc, "(Keine Beweisfragen erfasst — Pflichtfeld)", bold=True)
        return
    _p(doc, "Der Sachverständige wurde durch das Gericht zur Klärung folgender Fragen beauftragt:")
    for f in beweisfragen:
        p = doc.add_paragraph(style="List Number")
        p.add_run(f.get("frage_text", "") or "(leer)")


def _add_kapitel_3(doc: Document, verfahren: list[dict[str, Any]]) -> None:
...
def _add_kapitel_4(doc: Document, befunde: list[dict[str, Any]]) -> None:
    _h1(doc, "IV. Befunderhebung")
    _p(doc, "Disjunkter Befundbericht — objektive Darstellung der Messfakten, ohne Bewertung.")
    if not befunde:
...
def _add_kapitel_5(doc: Document, beurteilungen: list[dict[str, Any]]) -> None:
    _h1(doc, "V. Technische Beurteilung")
    _p(doc, "Gutachterliche Würdigung der Befunde anhand des anerkannten Standes der Technik.")
    ...
    for u in beurteilungen:
        ...
        if u.get("soll_text"):
            _p(doc, "Soll (Stand der Technik):", bold=True)
            _html2docx.render_to_docx(doc, u["soll_text"])
        if u.get("ist_text"):
            _p(doc, "Ist (Befund-Vergleich):", bold=True)
            _html2docx.render_to_docx(doc, u["ist_text"])
        if u.get("kausalitaet_text"):
            _p(doc, "Kausalität:", bold=True)
            _html2docx.render_to_docx(doc, u["kausalitaet_text"])
        if u.get("bewertung_text"):
            _p(doc, "Würdigung:", bold=True)
            _html2docx.render_to_docx(doc, u["bewertung_text"])
        if u.get("non_liquet"):
...
def _add_kapitel_7(doc: Document, projekt: dict[str, Any]) -> None:
    ...
    eigenversicherung = (
        f"Der Unterzeichnende versichert, das Gutachten unparteiisch, "
        f"nach bestem Wissen und Gewissen, persönlich und nach dem aktuellen "
        f"Stand der Technik erstellt zu haben."
    )
...
def _add_kapitel_8(doc: Document, assets: list[dict[str, Any]]) -> None:
    ...
    if not assets:
        _p(doc, "(Keine Asservaten erfasst)")
    else:
