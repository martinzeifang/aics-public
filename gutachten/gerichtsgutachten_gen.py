...
def build_gerichtsgutachten_docx(projekt_name: str, db_path: Path,
    ...
    if opts["include_glossar"]:
        _add_glossar(doc, _gdb.list_glossar(db_path, projekt_name))
    _add_footer(doc, projekt)
    return doc


def _add_glossar(doc: Document, glossar: list[dict[str, Any]]) -> None:
    """Glossar im Anhang — alphabetisch, Begriff fett + Erklärung (#985)."""
    if not glossar:
        return
    _h1(doc, "Glossar")
    _p(doc, "Erläuterung der verwendeten Fachbegriffe, Normen und Werkzeuge (alphabetisch):")
    for g in glossar:  # bereits alphabetisch aus list_glossar
        p = doc.add_paragraph()
        run = p.add_run(f"{g.get('begriff', '')}: ")
        run.bold = True
        p.add_run(g.get("erklaerung", "") or "—")
