...
CREATE TABLE IF NOT EXISTS gerichtsgutachten_peer_review (
    ...
    angefordert_am  TEXT NOT NULL DEFAULT (datetime('now')),
...
CREATE TABLE IF NOT EXISTS gerichtsgutachten_aufbewahrung (
    ...
    eingereicht_am    TEXT NOT NULL DEFAULT (datetime('now')),
    ...
    try:
        con.execute(
            """UPDATE gerichtsgutachten_peer_review
               SET status='abgeschlossen', abgeschlossen_am=datetime('now')
               WHERE id=?""",
            (review_id,),
        )
        con.commit()
