...
def _migrate_schema(con: Any) -> None:
    ...
    con.execute("ALTER TABLE gutachten_projects ADD COLUMN IF NOT EXISTS meta_json TEXT NOT NULL DEFAULT '{}' ")
    # Issue #427/#436: Firmen-Bezug fuer Reassign
    con.execute("ALTER TABLE gutachten_projects ADD COLUMN IF NOT EXISTS unternehmen TEXT NOT NULL DEFAULT ''")
...
def save_project(
    ...
    try:
        ...
        con.execute(
            """
            INSERT INTO gutachten_projects (name, frameworks_json, pruefungsfokus, unternehmen, meta_json, updated_at)
            VALUES (?, ?, ?, ?, ?, aics_now())
            ON CONFLICT(name) DO UPDATE SET
                ...
                updated_at      = aics_now()
            """,
            (name, _json.dumps(frameworks, ensure_ascii=False), pruefungsfokus, unternehmen, meta_json),
        )
        con.commit()
    finally:
        con.close()
