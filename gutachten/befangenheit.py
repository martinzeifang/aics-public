...
Risiko-Levels:
- hoch     → derselbe SV hat den Kunden vorher als Audit-Bericht-Ersteller bedient
- mittel   → derselbe Kunde wurde von einem anderen SV bedient (Kontext erklären)
...
CREATE INDEX IF NOT EXISTS idx_befang_kunde ON gutachten_befangenheits_log(kunde);
"""
...
def check(db_path: Path, kunde: str, system: str = "", parteien: list[str] | None = None,
    ...
    try:
        # 1. Audit-Berichte zum gleichen Kunden — RISIKO HOCH
        ...
        try:
            ...
            for r in rows:
                treffer.append({
                    ...
                    "projekt_name": r["name"],
                    "risiko": "hoch",
                    "grund": f"Audit-Bericht '{r['name']}' enthält den Kunden — Vorbefassung möglich (§ 406 ZPO)",
                })
        except sqlite3.OperationalError:
            pass  # Tabelle existiert noch nicht

        # 2. Andere Gerichtsgutachten zum gleichen Kunden — RISIKO MITTEL/HOCH
        try:
            rows = con.execute(
            ...
            for r in rows:
                treffer.append({
                    ...
                    "projekt_name": r["name"],
                    "risiko": "hoch",
                    "grund": f"Anderes Gerichtsgutachten '{r['name']}' nennt den Kunden — Vorbefassung möglich",
                })
        except sqlite3.OperationalError:
    ...
    try:
        cur = con.execute(
            """INSERT INTO gutachten_befangenheits_log
                 (sv_user, kunde, system, treffer_json, entscheidung)
               VALUES (?, ?, ?, ?, ?)
               RETURNING id""",
            (sv_user, kunde, system, treffer_json, entscheidung),
        )
