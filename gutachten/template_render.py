...
def _paragraph_has_field(paragraph) -> bool:
    """True, wenn der Absatz Word-Feldcodes (PAGE/NUMPAGES/…) enthält (#1007).

    Solche Absätze dürfen NICHT per Run-Reset (``_replace_in_paragraph``) neu
    geschrieben werden, sonst gehen die ``w:fldChar``/``w:instrText``-Paare
    verloren und „Seite n von N" kollabiert zu „Seite 1 von 1".
    """
    from docx.oxml.ns import qn
    el = paragraph._p
    for tag in ("w:fldChar", "w:instrText", "w:fldSimple"):
        if next(el.iter(qn(tag)), None) is not None:
            return True
    return False


def _replace_in_runs_fieldsafe(paragraph, replacements: dict[str, str]) -> None:
    """Feldcode-sichere Ersetzung (#1007): ersetzt ``[token]`` nur INNERHALB
    einzelner Text-Runs, ohne Runs zu konsolidieren — erhält fldChar/instrText.

    Tokens, die sich über mehrere Runs erstrecken, werden in feldhaltigen
    Absätzen bewusst nicht ersetzt (seltener Briefkopf-Fall; Feld-Erhalt hat
    Vorrang). Plain-Absätze laufen weiter über ``_replace_in_paragraph``.
    """
...
_BLOCK_KEYS = {"befunde", "beurteilungen", "beweisfragen", "hilfspersonen", "glossar", "gutachten_volltext"}
...
def _build_blocks(key: str, context: dict) -> list[tuple[str, tuple[str, ...]]]:
    ...
    def add_glossar(items):  # #1001 — „Glossar" (H2) + Tabelle (Begriff | Erklärung)
        rows = [[g.get("begriff", ""), g.get("erklaerung", "") or "—"]
                for g in items if isinstance(g, dict) and g.get("begriff")]
        if not rows:
            return
        blocks.append(("Glossar", H2))
        blocks.append((rows, ("__table__",), ["Begriff", "Erklärung"]))

    def add_assets(items):  # #1001 — Asservaten im Anhang als Tabelle
        rows = [[a.get("bezeichnung", ""), (a.get("sha256", "") or "")[:16],
                 f"{a.get('werkzeug_name', '')} {a.get('werkzeug_version', '')}".strip()]
                for a in items if isinstance(a, dict)]
        blocks.append(("Asservaten (Chain of Custody nach ISO/IEC 27037)", H2))
        if rows:
            blocks.append((rows, ("__table__",), ["Bezeichnung", "SHA-256", "Werkzeug"]))
        else:
            blocks.append(("(Keine Asservaten erfasst)", BODY))
    ...
    if key == "befunde":
        ...
        add_glossar(context.get("glossar") or [])
        ...
        # #1004: „Gegenstand des Gutachtens" als Heading 2 (→ TOC + Navigation),
        # gespiegelt vom Standard-Deckblatt.
        if p.get("thema"):
            blocks.append(("Gegenstand des Gutachtens", H2))
            blocks.append((p["thema"], BODY))
        ...
        # Anhang (#1001/#1005) — H1 „Anhang" mit Asservaten + Glossar (jeweils H2),
        # respektiert include_anhang / include_glossar. #1005: Die H1 „Anhang" wird
        # bei include_anhang IMMER gedruckt (auch ohne Asservaten — dann
        # „(Keine Asservaten erfasst)"), konsistent zum Standard-Renderer.
        assets = context.get("assets") or []
        glossar = context.get("glossar") or []
        want_anhang = context.get("include_anhang", True)
        want_glossar = glossar and context.get("include_glossar", True)
        if want_anhang or want_glossar:
            blocks.append(("Anhang", H1))
            if want_anhang:
                add_assets(assets)
            if want_glossar:
                add_glossar(glossar)
...
def _insert_blocks_after(anchor, blocks, doc, style_map: dict[str, str]) -> None:
    ...
    for block in blocks:
        ...
        # #1001: Tabellen-Block — (rows, ("__table__",), header)
        if "__table__" in styles:
            header = bold_label if isinstance(bold_label, (list, tuple)) else None
            rows = text or []
            ...
            for row in rows:
                ...
                for i, val in enumerate(row[:ncols]):
                    rc[i].text = str(val or "")
            cur._p.addnext(tbl._tbl)               # Tabelle hinter den aktuellen Anker
            after = OxmlElement("w:p")              # Leerabsatz nach der Tabelle als neuer Cursor
            tbl._tbl.addnext(after)
            cur = Paragraph(after, anchor._parent)
            continue
...
def render_with_bracket_mapping(template_path: Path, mapping: dict, context: dict,
    ...
    if scalar:
        for p in _iter_all_paragraphs(doc):
            if "[" in (p.text or ""):
                # #1007: feldhaltige Absätze (Fußzeile mit PAGE/NUMPAGES) feld-sicher
                # behandeln, sonst regulärer Run-Reset.
                if _paragraph_has_field(p):
                    _replace_in_runs_fieldsafe(p, scalar)
                else:
    ...
    if blocks:
        ...
        for p in list(_iter_all_paragraphs(doc)):
            txt = p.text or ""
            # #1007: keine Block-Einfügung in feldhaltige Absätze (Header/Footer).
            if "[" not in txt or _paragraph_has_field(p):
                continue
    ...
    # #985/#1001-Fix: Glossar (als Tabelle unter „Anhang") auch dann anhängen,
    # wenn die Vorlage KEINEN [glossar]/[gutachten_volltext]-Platzhalter enthält.
    glossar = context.get("glossar") or []
    handled = any(spec in ("glossar", "gutachten_volltext") for spec in (mapping or {}).values())
    if glossar and context.get("include_glossar", True) and not handled:
        H1 = ("Überschrift 1", "Heading 1")
        body_paras = doc.paragraphs
        if body_paras:
            sm = _build_style_id_map(doc)
            g_blocks = [("Anhang", H1)] + _build_blocks("glossar", context)
            _insert_blocks_after(body_paras[-1], g_blocks, doc, sm)
...
def build_template_context(db_path: Path, projekt_name: str) -> dict[str, Any]:
    ...
    return {
        ...
        "assets": gdb.list_assets(db_path, projekt_name),
        "glossar": gdb.list_glossar(db_path, projekt_name),
