...
def _build_blocks(key: str, context: dict) -> list[tuple[str, tuple[str, ...]]]:
    ...
    if key == "befunde":
        ...
        for e in (context.get("verfahren") or []):
            ...
            blocks.append((f"• {datum} — {titel}".strip(" —"), BODY))
            if e.get("beschreibung"):
                blocks.append((_strip_html(e["beschreibung"]), BODY))
        ...
        for i, f in enumerate(bf, 1):
            ...
            if f.get("antwort_kurz"):
                blocks.append((f"Antwort (kurz): {f['antwort_kurz']}", BODY))
            ans = _strip_html(f.get("antwort_text", ""))
            if ans:
                blocks.append((ans, BODY))

        # VII. Schlussformel
        blocks.append(("VII. Schlussformel", H1))
        blocks.append(("Der Unterzeichnende versichert, das Gutachten unparteiisch, nach bestem "
                       "Wissen und Gewissen, persönlich und nach dem aktuellen Stand der Technik "
                       "erstellt zu haben.", BODY))
        blocks.append((context.get("datum", ""), BODY))
        blocks.append(("_______________________________", BODY))
        if p.get("sv_name"):
            blocks.append((p["sv_name"], BODY))
