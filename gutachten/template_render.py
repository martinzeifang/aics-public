...
def _build_blocks(key: str, context: dict) -> list[tuple[str, tuple[str, ...]]]:
    ...
    def add_glossar(items):  # #985 — Begriff (fett) + Erklärung, alphabetisch
        for g in items:
            if isinstance(g, dict) and g.get("begriff"):
                blocks.append((g.get("erklaerung", "") or "—", BODY, f"{g['begriff']}: "))
    ...
    if key == "befunde":
        ...
        # Glossar (#985) — am Ende, alphabetisch; respektiert Export-Toggle.
        glossar = context.get("glossar") or []
        if glossar and context.get("include_glossar", True):
            blocks.append(("Glossar", H1))
            add_glossar(glossar)
    return blocks
...
def render_with_bracket_mapping(template_path: Path, mapping: dict, context: dict,
    ...
    # #985-Fix: Glossar auch dann anhängen, wenn die Vorlage KEINEN
    # [glossar]/[gutachten_volltext]-Platzhalter enthält — sonst fehlt es im Anhang.
    glossar = context.get("glossar") or []
    ...
    if glossar and context.get("include_glossar", True) and not handled:
        ...
        if body_paras:
            ...
            g_blocks = [("Glossar", H1)] + _build_blocks("glossar", context)
