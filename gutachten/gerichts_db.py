...
def list_glossar(db_path: Path, projekt_name: str) -> list[dict[str, Any]]:
    ...
    try:
        rows = con.execute(
            "SELECT * FROM gerichtsgutachten_glossar WHERE projekt_name=? ORDER BY begriff COLLATE NOCASE",
            (projekt_name,),
        ).fetchall()
        return [dict(r) for r in rows]
    finally:
...
def upsert_glossar_eintrag(db_path: Path, projekt_name: str, begriff: str,
    ...
    try:
        row = con.execute(
            "SELECT id, quelle, erklaerung FROM gerichtsgutachten_glossar WHERE projekt_name=? AND begriff=? COLLATE NOCASE",
