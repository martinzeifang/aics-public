...
# ── Geplante Berichte (GUI-konfigurierbar, #1405) ───────────────────────────
# Einfache Presets: periode (quartal|jahr) → abgeleiteter Cron. Persistiert in der
# DB (statt soc.config.json, das einen Integritäts-Sidecar hat).

_SCHEDULE_SCHEMA = """
CREATE TABLE IF NOT EXISTS soc_bericht_schedule (
    id         BIGINT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
    typ        TEXT NOT NULL,
    periode    TEXT NOT NULL DEFAULT 'quartal',   -- quartal | jahr
...
PERIODE_CRON = {"quartal": "0 6 1 1,4,7,10 *", "jahr": "0 7 1 1 *"}


def ensure_schedule(db_path: Path) -> None:
    con = _connect(db_path)
    try:
        con.executescript(_SCHEDULE_SCHEMA)
        con.commit()
    finally:
        con.close()
...
def list_schedules(db_path: Path) -> list[dict[str, Any]]:
    ensure_schedule(db_path)
    con = _connect(db_path)
    try:
        return [dict(r) for r in con.execute(
            "SELECT * FROM soc_bericht_schedule ORDER BY id").fetchall()]
    finally:
        con.close()


def add_schedule(db_path: Path, *, typ: str, periode: str = "quartal",
                 fmt: str = "docx") -> int:
    if typ not in BERICHT_TYPEN:
        raise ValueError("Unbekannter Berichtstyp")
    periode = periode if periode in PERIODE_CRON else "quartal"
    fmt = fmt if fmt in ("docx", "pdf") else "docx"
    ensure_schedule(db_path)
    con = _connect(db_path)
    try:
        cur = con.execute(
            "INSERT INTO soc_bericht_schedule(typ, periode, format) VALUES(?,?,?) RETURNING id",
            (typ, periode, fmt))
        con.commit()
        return int(cur.lastrowid or 0)
    finally:
...
def set_schedule_aktiv(db_path: Path, sched_id: int, aktiv: bool) -> None:
    ensure_schedule(db_path)
    con = _connect(db_path)
    try:
        con.execute("UPDATE soc_bericht_schedule SET aktiv=? WHERE id=?",
                    (1 if aktiv else 0, int(sched_id)))
        con.commit()
    finally:
        con.close()


def delete_schedule(db_path: Path, sched_id: int) -> None:
    ensure_schedule(db_path)
    con = _connect(db_path)
    try:
        con.execute("DELETE FROM soc_bericht_schedule WHERE id=?", (int(sched_id),))
        con.commit()
    finally:
        con.close()
