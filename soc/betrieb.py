...
# Prozess-weiter Guard: die Standard-Eskalationsmatrix muss nur einmal je Prozess
# angelegt werden (nicht bei jedem Read im Incident-Detail → #1454).
_SEEDED_DBS: set[str] = set()


def seed_defaults(db_path: Path, *, force: bool = False) -> None:
    """Legt eine sinnvolle Standard-Eskalationsmatrix an, falls leer (idempotent).

    Per Prozess-Guard wird höchstens einmal je DB tatsächlich geprüft/geschrieben;
    so verursacht der Lese-Pfad (Incident-Detail) keine wiederholten Schreib-/
    COUNT-Zugriffe mehr (#1454). ``force=True`` umgeht den Guard (z. B. beim
    expliziten Konfigurieren der Eskalationsmatrix).
    """
    key = str(Path(db_path))
    if not force and key in _SEEDED_DBS:
        return
    ensure_db(db_path)
    con = _connect(db_path)
    try:
