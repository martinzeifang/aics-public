...
def _build_agg_body(hours: int) -> dict[str, Any]:
    return {
        ...
        "aggs": {
            # Primär gruppiert nach Hostname; je Bucket Top-Absender-IP (aus
            # data.srcip, Fallback agent.ip), Programm, Trefferzahl + last_seen.
            "by_host": {
                "terms": {"field": "predecoder.hostname.keyword", "size": 500,
                          "missing": "(ohne Hostname)"},
                "aggs": {
                    "srcip": {"terms": {"field": "data.srcip", "size": 1}},
                    "agentip": {"terms": {"field": "agent.ip", "size": 1}},
                    "program": {"terms": {"field": "predecoder.program_name.keyword", "size": 1}},
                    "last_seen": {"max": {"field": "timestamp"}},
                },
            },
            # Fallback: Quellen ganz ohne predecoder.hostname rein nach Absender-IP
            # (data.srcip — NICHT location, das alles auf den Manager kollabiert),
            # damit hostname-lose Geräte nicht verloren gehen.
            "by_ip": {
                "terms": {"field": "data.srcip", "size": 500},
                "aggs": {
                    "program": {"terms": {"field": "predecoder.program_name.keyword", "size": 1}},
                    "last_seen": {"max": {"field": "timestamp"}},
                },
...
def discover_syslog_sources(db_path: Path, *, hours: int = 2,
    ...
    for b in (aggs.get("by_host", {}) or {}).get("buckets", []) or []:
        ...
        if hostname == "(ohne Hostname)":
            hostname = ""
        ip = _first_bucket_key(b.get("srcip", {})) or _first_bucket_key(b.get("agentip", {}))
