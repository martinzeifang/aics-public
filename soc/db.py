...
CREATE TABLE IF NOT EXISTS soc_meldetracks (
    id            BIGINT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
    incident_id   INTEGER NOT NULL,
    regime        TEXT NOT NULL,                           -- dsgvo|nis2|cra|aiact
...
_ASSET_FLAGS = ("personenbezogen", "nis2_scope", "cra_produkt", "ki_hochrisiko")
...
def upsert_asset(db_path: Path, asset: dict[str, Any]) -> int:
    ...
    try:
        ...
        # #1464: Umbenennen NUR beim expliziten Edit (id gesetzt) — agent_name ist
        # sonst der Import-Schlüssel und darf vom Re-Import nicht überschrieben werden.
        if asset.get("id") and "agent_name" in asset:
            present["agent_name"] = asset["agent_name"]
        if "meta" in asset:
...
def _asset_to_dict(r: Any) -> dict[str, Any]:
    d = dict(r)
    for f in ("personenbezogen", "nis2_scope", "cra_produkt", "ki_hochrisiko"):
        d[f] = bool(d.get(f))
    try:
