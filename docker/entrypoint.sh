...
if [ ! -f "$CERT_FILE" ]; then
    ...
    openssl req -x509 -nodes -days 365 -newkey rsa:2048 \
        ...
        2>/dev/null
    # #1476: Key für den nginx-User (anderer Container, UID 101) lesbar machen —
    # er mountet dasselbe Volume read-only. 0644 ist für ein internes Self-Signed-
    # Zertifikat vertretbar; behebt den "Permission denied"-Restart-Loop dauerhaft
    # (restart-fest, kein manueller chmod nötig). Für ein echtes CA-Cert ggf. den
    # Key über eine geteilte Gruppe mit 0640 bereitstellen.
    chmod 644 "$KEY_FILE"
    chmod 644 "$CERT_FILE"
