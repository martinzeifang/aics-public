...
# LibreOffice headless für DOCX→PDF in der Template-Engine (#990).
# Nur Writer-Kern (kein Calc/Impress/JRE) — hält den Layer kleiner (~ einige 100 MB).
RUN apt-get update && apt-get install -y --no-install-recommends \
        libreoffice-core libreoffice-writer libreoffice-common --no-install-recommends \
