    ...
    if layout == "system":
        ...
        for u in updates:
            ...
            if u.get("umsetzung") is not None:
                ws.cell(r, cols["Umsetzung"]).value = safe_cell_value(u["umsetzung"])
            if u.get("schutzziel") is not None:
                ws.cell(r, cols["Zuordnung zu Schutzzielen"]).value = safe_cell_value(u["schutzziel"])
            if u.get("bemerkung_umsetzung") is not None:
                ws.cell(r, cols["Bemerkung zur Umsetzung"]).value = safe_cell_value(u["bemerkung_umsetzung"])
        ...
        for u in updates:
            ...
            if u.get("contract_assured") is not None:
                ws.cell(r, c_contract_assured).value = safe_cell_value(u["contract_assured"])
            if u.get("ops_met") is not None:
                ws.cell(r, c_ops_met).value = safe_cell_value(u["ops_met"])
            if u.get("bemerkung") is not None:
                ws.cell(r, c_bem).value = safe_cell_value(u["bemerkung"])
            if u.get("evaluated_by") is not None:
                ws.cell(r, c_contract_by).value = safe_cell_value(u["evaluated_by"])
                ws.cell(r, c_ops_by).value = safe_cell_value(u["evaluated_by"])
            if u.get("evaluated_at") is not None:
                ws.cell(r, c_contract_date).value = safe_cell_value(u["evaluated_at"])
                ws.cell(r, c_ops_date).value = safe_cell_value(u["evaluated_at"])
